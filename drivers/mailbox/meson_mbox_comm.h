...
struct aml_mbox_wq_rx_header {
	int chan_id;
	u32 cmd;
	u32 size;
} __packed;

struct aml_mbox_wq_rx_packet {
	struct aml_mbox_wq_rx_header rx_header;
	char buf[MBOX_DATA_SIZE];
} __packed;

struct aml_mbox_rx_work {
	struct device *dev;
...
struct aml_mbox_rx_msg {
	...
	struct aml_mbox_wq_rx_packet *msg_data; /* hook for rx data packet */
	unsigned int rx_queue_len;              /* queue length of rx data packet */
	spinlock_t lock;                        /* lock for message buffer */
	struct workqueue_struct *wq;            /* work queue for rx message */
	struct aml_mbox_rx_work work_items;     /* rx message work items */
};
