...
#define IS_READ_CACHE_CMD(cmd) ({	\
	...
	(_cmd) == CMD_READ ||			\
	(_cmd) == CMD_READ_FAST ||		\
	(_cmd) == CMD_READ_DUAL_OUT ||	\
	(_cmd) == CMD_READ_QUAD_OUT ||	\
	(_cmd) == CMD_READ_DUAL_IO ||	\
	(_cmd) == CMD_READ_QUAD_IO;		\
})

#define IS_WRITE_CACHE_CMD(cmd)	 ({	\
	__typeof__(cmd) _cmd = (cmd);	\
	(_cmd) == CMD_PROG_LOAD ||			\
	(_cmd) == CMD_PROG_LOAD_RDM_DATA ||	\
	(_cmd) == CMD_PROG_LOAD_X4 ||		\
	(_cmd) == CMD_PROG_LOAD_RDM_DATA_X4;	\
})
			    ...
			    u8 *user_buf, int len, bool read)
{
	...
	SPI_NFC_DEBUG("flags = %lx user_buf = %p len = 0x%x disable_host_ecc = %d mode = 0x%x %s\n",
		       flags, user_buf, len, spi_nfc->disable_host_ecc, spi->mode,
		       (read) ? "read" : "write");
				    ...
				    struct spi_transfer *xfer)
{
	...
	case TRANSFER_STATE_CMD:
		if (IS_READ_CACHE_CMD(p[0]) || IS_WRITE_CACHE_CMD(p[0]))
			cache_op_running = 1;
