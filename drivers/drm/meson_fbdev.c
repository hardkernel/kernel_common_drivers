...
static int am_meson_fbdev_open(struct fb_info *info, int arg)
{
	...
	MESON_DRM_FBDEV("IN,pid[%s %d],plane_id[%d],screen_size[%ld],res_virtual[%d %d]\n",
		current->comm, current->pid, osdplane->plane_index,
		info->screen_size, info->var.xres_virtual, info->var.yres_virtual);
	ret = am_meson_fbdev_alloc_fb_gem(info);
	MESON_DRM_FBDEV("OUT, ret=%d\n", ret);
	return ret;
}
...
static int am_meson_fbdev_release(struct fb_info *info, int arg)
{
	...
	MESON_DRM_FBDEV("IN, pid[%s %d], plane_id[%d]\n",
		current->comm, current->pid, osdplane->plane_index);
...
static int am_meson_drm_fbdev_setcmap(struct fb_cmap *cmap, struct fb_info *info)
{
	...
	transp  = cmap->transp;
	index   = cmap->start;
	MESON_DRM_FBDEV("visual = %d, len = %d\n", info->fix.visual, cmap->len);
			     ...
			     unsigned int blue, unsigned int transp, struct fb_info *info)
{
	...
	MESON_DRM_FBDEV("pixel is %d bits, palette index-%d\n",
		var->bits_per_pixel, regno);
					    ...
					    struct fb_info *info)
{
	...
	if (var->bits_per_pixel != fb->format->depth ||
		...
		MESON_DRM_FBDEV("need realloc buffer\n");
...
static void am_meson_drm_wait_one_vblank(struct fb_info *info)
{
	struct drm_fb_helper *fb_helper = info->par;
	...
	if (fbdev->plane->crtc)
		crtc_index = fbdev->plane->crtc->index;
	else if (fbdev->modeset.crtc)
		crtc_index = fbdev->modeset.crtc->index;
	else
		crtc_index = 0;

	vblank = &fb_helper->dev->vblank[crtc_index];
	if (vblank->enabled)
		drm_wait_one_vblank(fb_helper->dev, crtc_index);
	else
		MESON_DRM_FBDEV("crtc-%d is disabled, can not wait vblank.\n",
			crtc_index);
}
...
int am_meson_drm_fb_helper_set_par(struct fb_info *info)
{
	...
	unsigned long old_screen_size = info->screen_size;
	...
	DRM_INFO("pid[%s %d], plane_id[%d], fb_gem[%px]\n"
		"bpp[%d %d], fb[%d %d], res[%d %d], res_virtual[%d %d]\n",
		current->comm, current->pid, plane->index, fb_gem,
		var->bits_per_pixel, fb->format->depth,
		fb->width, fb->height, var->xres, var->yres,
		var->xres_virtual, var->yres_virtual);
	...
	if (var->bits_per_pixel != fb->format->depth ||
		...
		MESON_DRM_FBDEV("length[%d %d %d %d]offset[%d %d %d %d]format:%c%c%c%c\n",
		...
		pitches = ALIGN(sizes.fb_width * bytes_per_pixel, 64);
		screen_size = pitches * sizes.fb_height;

		if (fb_gem && fb_gem->size < info->screen_size) {
			DRM_INFO("%s, GEM SIZE is not enough: %ld < %ld\n",
				__func__, (unsigned long)fb_gem->size, screen_size);
		...
		if (!fb_gem) {
			...
			if (am_meson_fbdev_alloc_fb_gem(info)) {
				DRM_ERROR("realloc fb fail, screen_size[%ld->%ld]\n",
					screen_size, old_screen_size);
				info->screen_size = old_screen_size;
				return -ENOMEM;
			}
		}
	...
	am_meson_drm_wait_one_vblank(info);
				       ...
				       struct fb_info *info)
{
	...
	MESON_DRM_FBDEV("IN, pid[%s %d], plane_id[%d], plane_type[%d], fb[%p]\n",
		current->comm, current->pid, plane->index,
...
int am_meson_drm_fb_blank(int blank, struct fb_info *info)
{
	...
	if (blank == 0) {
		MESON_DRM_FBDEV("IN, pid[%s %d] meson_fbdev[%s]fb[%p] goto unblank.\n",
		...
		MESON_DRM_FBDEV("IN, pid[%s %d] meson_fbdev[%s]fb[%p] goto blank.\n",
			current->comm, current->pid,
			fbdev->plane->name, fbdev->plane->fb);

		if (fbdev->plane->crtc) {
			crtc = fbdev->plane->crtc;
		} else if (fbdev->modeset.crtc) {
			crtc = fbdev->modeset.crtc;
		}
		if (!crtc) {
			DRM_ERROR("crtc is null, skip %s blank\n", fbdev->plane->name);
			return 0;
		}
	...
	MESON_DRM_FBDEV("OUT, meson_fbdev[%s], ret=%d\n",
		fbdev->plane->name, ret);
	return ret;
}
				    ...
				    unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == FBIOGET_OSD_DMABUF) {
		...
		if (i == MAX_RETRY_CNT)
			DRM_ERROR("timeout, frame seq %u-%u\n",
		...
		if (ret) {
			DRM_ERROR("set dst region error and not change\n");
			memset(&fbdev->dst_changed, 0, sizeof(struct region));
		} else if ((dst.w == 0) || (dst.h == 0)) {
			MESON_DRM_FBDEV("restore mode resolution by clearing w(%d) or h(%d)\n",
				dst.w, dst.h);
			memset(&fbdev->dst_changed, 0, sizeof(struct region));
		} else {
			MESON_DRM_FBDEV("set dst region is [%d, %d, %d x %d]\n",
		...
		MESON_DRM_FBDEV("get plane %d zorder and value is%d(%d)\n",
			plane->index, zorder, fbdev->zorder);
	} else if (cmd == FBIOSET_OSD_ZORDER) {
		ret = copy_from_user(&zorder, argp, sizeof(zorder)) ? -EFAULT : 0;
		if (ret) {
			DRM_ERROR("set plane %d zorder error and not change\n",
				plane->index);
		} else if ((zorder < 65) || (zorder > 128)) {
			DRM_ERROR("plane %d zorder value %d is out of range\n",
				plane->index, zorder);
			ret = -EFAULT;
		} else {
			MESON_DRM_FBDEV("set plane %d zorder and value is %d\n",
				plane->index, zorder);
			fbdev->zorder = zorder;
			ret = am_meson_drm_fb_pan_display(&info->var, info);
		}
				     ...
				     struct drm_fb_helper_surface_size *sizesxx)
{
	...
	DRM_INFO("fb=%dx%d, surface=%dx%d, bpp=%d\n",
		sizes.fb_width, sizes.fb_height, sizes.surface_width,
		sizes.surface_height, sizes.surface_bpp);
	DRM_INFO("mode_cmd_size=%dx%d,pixel_format=%d-%d-%d,pitches=%d,fb_gem=%p\n",
		mode_cmd.width, mode_cmd.height, mode_cmd.pixel_format,
		DRM_FORMAT_ARGB8888, DRM_FORMAT_ABGR8888,
		mode_cmd.pitches[0], fbdev->fb_gem);
	...
	DRM_INFO("xres=%dx%d, xres_virtual=%dx%d, bpp=%d, screen_size=%ld\n",
		fbi->var.xres, fbi->var.yres,
		fbi->var.xres_virtual, fbi->var.yres_virtual,
		fbi->var.bits_per_pixel, fbi->screen_size);
	return 0;
	...
	if (drmdev->primary_plane) {
		...
		if (fbdev) {
			...
			MESON_DRM_FBDEV("create fbdev(%p) for primary plane, zorder=(%d %d)\n",
				fbdev, drmdev->fbdev_zorder[i], fbdev->zorder);
		} else {
	...
	for (i = 0; i < MESON_MAX_OSD; i++) {
		...
		if (fbdev) {
			...
			MESON_DRM_FBDEV("create fbdev(%p) for plane (%d %d), zorder=(%d %d)\n",
				fbdev, i, osd_plane->plane_index,
				drmdev->fbdev_zorder[i], fbdev->zorder);
		} else {
