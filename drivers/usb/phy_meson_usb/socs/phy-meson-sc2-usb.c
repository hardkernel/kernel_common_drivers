...
static void meson_usb2phy_sc2_cali(struct amlogic_usb_v2 *mphy)
{
	meson_usb2phy_legacy_cali(mphy);
	meson_usb2phy_legacy_cali_disc_squelch(mphy);
}

static struct meson_u2phy_priv meson_u2phy_sc2_priv = {
	.set_mode = meson_u2phy_legacy_set_mode,
	.cali = meson_usb2phy_sc2_cali,
	.set_pll = meson_usb2phy_legacy_set_pll,
};

static int meson_u2phy_sc2_set_mode(void *phy, enum meson_uphy_mode mode)
{
	return meson_u2phy_legacy_set_mode((struct amlogic_usb_v2 *)phy, mode);
}

static int meson_u2phy_sc2_init(void *phy)
{
	return meson_u2phy_aml_init((struct amlogic_usb_v2 *)phy, &meson_u2phy_sc2_priv);
}

static int meson_u2phy_sc2_exit(void *phy)
{
 ...
 */
static int meson_u2phy_sc2_power_on(void *phy)
{
...
static int meson_u2phy_sc2_power_off(void *phy)
{
	struct amlogic_usb_v2 *mphy = (struct amlogic_usb_v2 *)phy;
...
int meson_u2phy_sc2_configure(void *phy, struct meson_uphy_configure_opts *opts)
{
	int ret = 0;
...
static int meson_u2phy_sc2_validate(void *phy, enum meson_uphy_mode mode,
...
static int meson_u3phy_sc2_init(void *phy)
{
	return meson_synopsis_u3phy_init((struct amlogic_usb_v2 *)phy);
}

static int meson_u3phy_sc2_exit(void *phy)
{
