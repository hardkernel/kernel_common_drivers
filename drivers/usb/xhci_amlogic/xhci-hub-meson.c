...
/**
 * aml_xhci_port_state_to_neutral() - Clean up read portsc value back into writeable
...
static void xhci_disable_port(struct aml_xhci_hcd *xhci, struct aml_xhci_port *port)
{
	struct usb_hcd *hcd;
	u32 portsc;
	...
	if (hcd->speed >= HCD_USB3) {
		aml_xhci_dbg(xhci, "Ignoring request to disable SuperSpeed port.\n");
		return;
	}
 ...
 * method.
 */
static void xhci_set_port_power(struct aml_xhci_hcd *xhci, struct aml_xhci_port *port,
		...
		case USB_PORT_FEAT_LINK_STATE:
			...
			if (link_state == USB_SS_PORT_LS_RX_DETECT) {
				aml_xhci_dbg(xhci, "Enable port %d-%d\n",
					 hcd->self.busnum, portnum1);
				aml_xhci_set_link_state(xhci, port,	link_state);
				temp = readl(port->addr);
				break;
			}
			...
			if (link_state == USB_SS_PORT_LS_U0) {
				...
				if (pls <= XDEV_U3) /* U1, U2, U3 */
					aml_xhci_set_link_state(xhci, port, USB_SS_PORT_LS_U0);
				if (!wait_u0) {
					if (pls > XDEV_U3)
						goto error;
					break;
...
EXPORT_SYMBOL_GPL(aml_xhci_hub_control);
