...
static void trb_to_noop(union aml_xhci_trb *trb)
{
...
{
	memset(dbc->eps, 0, sizeof_field(struct aml_xhci_dbc, eps));
}
...
static void
handle_ep_halt_changes(struct aml_xhci_dbc *dbc, struct aml_dbc_ep *dep, bool halted)
{
	if (halted) {
...
static void dbc_handle_xfer_event(struct aml_xhci_dbc *dbc, union aml_xhci_trb *event)
{
	...
	ep_ctx		= (ep_id == EPID_OUT) ?
				aml_dbc_bulkout_ctx(dbc) : aml_dbc_bulkin_ctx(dbc);
	ring		= dep->ring;
