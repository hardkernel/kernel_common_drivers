...
{
	if (direct) {
		if (!strncmp(param_name, "debug_ts_clone", strlen("debug_ts_clone")))
			debug_ts_clone = *param_value;
		else if (!strncmp(param_name, "clone_buf_size", strlen("clone_buf_size")))
			clone_buf_size = *param_value;
		else
			return -EINVAL;
	} else {
		if (!strncmp(param_name, "debug_ts_clone", strlen("debug_ts_clone")))
			*param_value = debug_ts_clone;
		else if (!strncmp(param_name, "clone_buf_size", strlen("clone_buf_size")))
			*param_value = clone_buf_size;
		else
			return -EINVAL;
	}
