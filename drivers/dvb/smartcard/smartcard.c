			  ...
			  struct device_attribute *attr, char *buf)
{
	...
	pr_inf("debug : enable(1/n)|disable|dump|reset\n");
	pr_inf("\t dump file: kernel 5.15 not support file read or write\n");
	return 0;
}
...
static int smc_hw_set_param(struct smc_dev *smc)
{
	...
	reg5->cwt_detect_en = cwt_det_enable;
...
static int smc_hw_setup(struct smc_dev *smc)
{
	...
	reg1->atr_holdoff_en = atr_hold_off;	/*ATR_HOLDOFF_EN; */
	...
	reg5 = (struct SMCCARD_HW_REG5 *)&v;
	reg5->cwt_detect_en = cwt_det_enable;
	reg5->btw_detect_en = btw_det_enable;
	reg5->etu_msr_en = etu_msr_enable;
	reg5->bwt_base_time_gnt = BWT_BASE_DEFAULT;
	SMC_WRITE_REG(REG5, v);
	pr_error("REG5: 0x%08lx\n", v);
