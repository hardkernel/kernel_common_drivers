...
struct videodisplay_dev {
	u32 index;
	struct videodisplay_port_s *port;
	u32 enable;
	DECLARE_KFIFO(ready_q, struct vframe_s *, COMPOSER_READY_POOL_SIZE);
	DECLARE_KFIFO(receive_q, struct received_frames_t *, FRAMES_INFO_POOL_SIZE);
	...
	DECLARE_KFIFO(vc_prepare_data_q, struct vd_prepare_s *, COMPOSER_READY_POOL_SIZE);
...
struct mediaproxy_info_t *videodisplay_get_mediaproxy(u32 index);
