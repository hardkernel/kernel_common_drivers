...
static void aml_dma_put(void *buf_priv)
{
	...
	if (!atomic_dec_and_test(&buf->refcount)) {
		pr_debug("aml_dma_buf=0x%p, refcount=%d\n",
				  ...
				  unsigned long flags)
{
	...
	atomic_inc(&buf->refcount);
	gdc_log(LOG_DEBUG, "get_dmabuf, aml_dma_buf=0x%p, refcount=%d\n",
		buf, atomic_read(&buf->refcount));

	return dbuf;
error:
	gdc_log(LOG_ERR, "get_dmabuf fail! ret:%d\n", ret);
	return NULL;
}
			  ...
			  struct gdc_dmabuf_exp_s *gdc_exp_buf)
{
	...
	return 0;
error:
	gdc_log(LOG_ERR, "gdc_dma_buffer_export fail! ret:%d\n", ret);
	return (-EINVAL);
}
...
int gdc_dma_buffer_map(struct aml_dma_cfg *cfg)
{
	...
	cfg->attach = d_att;
	cfg->sg = sg;
	gdc_log(LOG_DEBUG, "dbuf map=0x%p\n", dbuf);
...
error:
	gdc_log(LOG_ERR, "gdc_dma_buffer_map fail! ret:%d\n", ret);
	return -1;
}
			    ...
			    struct aml_dma_cfg *cfg, unsigned long *addr)
{
	...
	return ret;
error:
	gdc_log(LOG_ERR, "gdc_dma_buffer_get_phys fail! ret:%d\n", ret);
	return -1;
}
...
void gdc_dma_buffer_unmap(struct aml_dma_cfg *cfg)
{
	...
	dma_buf_put(dbuf);
	gdc_log(LOG_DEBUG, "dbuf unmap=0x%p\n", dbuf);
}
...
void gdc_dma_buffer_dma_flush(struct device *dev, int fd)
{
	...
	if (!is_module_dmabuf(dmabuf)) {
		gdc_log(LOG_DEBUG, "dma flush, dmabuf=0x%p is not allocated by this module\n",
			dmabuf);
		goto put;
	}
...
error:
	gdc_log(LOG_ERR, "gdc_dma_buffer_dma_flush fail! ret: %d\n", ret);
	return;
}
...
void gdc_dma_buffer_cache_flush(struct device *dev, int fd)
{
	...
	if (!is_module_dmabuf(dmabuf)) {
		gdc_log(LOG_DEBUG, "cache_flush, dmabuf=0x%p is not allocated by this module\n",
			dmabuf);
		goto put;
	}
...
error:
	gdc_log(LOG_ERR, "gdc_dma_buffer_cache_flush fail! ret: %d\n", ret);
	return;
}
