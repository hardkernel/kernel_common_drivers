...
struct gc2145_device {
	...
	/* platform device data from board init. */
			      ...
			      struct gc2145_buffer *buf)
{
	...
	dprintk(dev, 1, "free vmall buffer: freed\n");
			    ...
			    struct gc2145_buffer *buf)
{
	...
	dprintk(dev, 1, "free res buffer: freed\n");
	buf->vb.state = VIDEOBUF_NEEDS_INIT;
}
...
int __init gc2145_i2c_driver_init(void)
{
