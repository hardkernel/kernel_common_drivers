 ...
 *Input	:	*config_file
 *Output	:	file size
 *function :	open the firmware file, and return total size
 ***************************************
 */
	...
	while (check < sum) {
		...
		for (i = 0; i <= cf->aet.aet[check].info->tbl_max_step; i++) {
			if (i == cf->aet.aet[check].info->tbl_max_step) {
				ret = parse_last_aet_element_tbl(&iter,
		  ...
		  struct configure_s *cf)
{
	...
	/**init hw**/
	if (cf->hw_valid == 1) {
		para->xml_regs_map = vmalloc(sizeof(*para->xml_regs_map));
		if (!para->xml_regs_map) {
//			pr_err("alloc mem failed\n");
			return -ENOMEM;
		}
	...
	if (cf->capture_valid == 1) {
		para->xml_capture = vmalloc(sizeof(*para->xml_capture));
		if (!para->xml_capture) {
//			pr_err("alloc mem failed\n");
			return -ENOMEM;
		}
		capture = para->xml_capture;
		capture->ae_try_max_cnt =
			(unsigned int)(cf->capture.capture[pindex.capture_index].export[0]);
		capture->single_count =
			(unsigned int)(cf->capture.capture[pindex.capture_index].export[3]);
		capture->skip_step =
			(unsigned int)(cf->capture.capture[pindex.capture_index].export[4]);
		capture->multi_capture_num =
			(unsigned int)(cf->capture.capture[pindex.capture_index].export[5]);
		capture->af_mode =
			(enum cam_scanmode_e)(cf->capture.capture[pindex.capture_index].export[2]);
		capture->eyetime =
			(unsigned int)(cf->capture.capture[pindex.capture_index].export[6]);
		capture->pretime =
			(unsigned int)(cf->capture.capture[pindex.capture_index].export[7]);
		capture->postime =
			(unsigned int)(cf->capture.capture[pindex.capture_index].export[8]);
	} else {
		para->xml_capture = NULL;
	}
	/**init wave**/
	if (cf->wave_valid == 1) {
		para->xml_wave = kmalloc(sizeof(*para->xml_wave), 0);
		if (!para->xml_wave) {
//			pr_err("alloc mem failed\n");
			return -ENOMEM;
		}
