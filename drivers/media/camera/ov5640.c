...
static inline uint64_t getTimestamp(void)
{
...
static int OV5640_AutoFocus(struct ov5640_device *dev, int focus_mode)
{
	...
	default:
		...
		pr_info("release focus to infinite\n");
		break;
	}
	return ret;
}
...
static struct resolution_param *get_resolution_param(struct ov5640_device *dev,
		...
		struct resolution_param *res_param)
{
	...
	while (1) {
		if (res_param->reg_script[i].val == 0xff &&
			...
			pr_info("setting resolution param complete\n");
			break;
		}
...
static int ov5640_setting(struct ov5640_device *dev, int PROP_ID, int value)
{
	...
	case V4L2_CID_BRIGHTNESS:
		mutex_lock(&firmware_mutex);
		dprintk(dev, 1, "setting brighten:%d\n", v4l_2_ov5640(value));
...
static void free_buffer(struct videobuf_queue *vq, struct ov5640_buffer *buf)
{
	...
	dprintk(dev, 1, "free buffer: freed\n");
...
static int ov5640_probe(struct i2c_client *client KV_I2C_PROBE_ID)
{
...
*/
	...
	pr_info("ov5640 probe successful.\n");

	return 0;
}

static KV_I2C_REMOVE_TYPE ov5640_remove(struct i2c_client *client)
{
