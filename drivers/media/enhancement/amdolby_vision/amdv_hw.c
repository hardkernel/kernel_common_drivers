			     ...
			     bool reset)
{
	...
	if (is_aml_t3() || is_aml_t5w() || is_aml_t5m()) {
		VSYNC_WR_DV_REG_BITS(VPP_TOP_CTRL, 0, 0, 1); //AMDV TV select
	...
	if (is_aml_txlx()) {/*old chips, need check*/
		...
		if (1/*!is_video_effect_bypass*/) {
...
#else
			{
				...
				if (is_amdv_stb_mode() && g_dst_format == FORMAT_DOVI) {
					...
					if (debug_dolby & 2)
						pr_dv_dbg("g_dst_format %d, eye_pr_bypass %d\n",
							g_dst_format, eye_pr_bypass);
				}
				if (eye_pr_bypass)
					dv_pq_ctl(DV_PQ_EP_BYPASS);
				else
