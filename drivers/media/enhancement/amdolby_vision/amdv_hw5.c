...
u32 test_dv;// = DEBUG_SW_TRIGGER_TOP2_RST | MANUAL_DPE_START;
...
/*5:case1080p 1920x1080 + 960x540 444-10 read from file*/
u32 fix_data;
...
void fixed_buf_config(void)
{
	...
	else if (fix_data == CASE1080p_TOP1_READFROM_FILE)
		y_buf_size = 960 * 540 * 4;
	...
	if (fixed_y_buf_paddr)
		codec_mm_free_for_dma("amdv", fixed_y_buf_paddr);
	if (fixed_uv_buf_paddr)
		codec_mm_free_for_dma("amdv", fixed_uv_buf_paddr);
...
void dump_pyramid_buf(unsigned int idx)
{
	...
	for (i = 0; i < 2; i++) {
		...
		switch (idx) {
		case 1:
			data_addr = (u8 *)py_addr[0].py_vaddr[0];
			data_size = py_addr[0].top1_py_size[0];
			break;
		case 2:
			data_addr = (u8 *)py_addr[0].py_vaddr[1];
			data_size = py_addr[0].top1_py_size[1];
			break;
		case 3:
			data_addr = (u8 *)py_addr[0].py_vaddr[2];
			data_size = py_addr[0].top1_py_size[2];
			break;
		case 4:
			data_addr = (u8 *)py_addr[0].py_vaddr[3];
			data_size = py_addr[0].top1_py_size[3];
			break;
		case 5:
			data_addr = (u8 *)py_addr[0].py_vaddr[4];
			data_size = py_addr[0].top1_py_size[4];
			break;
		case 6:
			data_addr = (u8 *)py_addr[0].py_vaddr[5];
			data_size = py_addr[0].top1_py_size[5];
			break;
		case 7:
			data_addr = (u8 *)py_addr[0].py_vaddr[6];
			data_size = py_addr[0].top1_py_size[6];
			break;
		default:
			break;
		}
		...
		if (data_size > 1280)
			data_size = 1280;
		if (idx < 4) { /*no stride*/
			for (i = 0; i < data_size - 4; i += 5) {/*5x8bit=>4x10bit*/
				temp = data_addr[i + 1] & 0x3;
				data_addr_16_1 = (temp << 8) | data_addr[i];
				temp = data_addr[i + 2] & 0xf;
				data_addr_16_2 = (temp << 6) | (data_addr[i + 1] >> 2);
				temp = data_addr[i + 3] & 0x3f;
...
void top1_crc_ro(void)
{
	...
	if (top1_crc_rd && (debug_dolby & 0x80000)) {
		if (is_aml_t3x()) {
			...
			VSYNC_WR_DV_REG_BITS(T6W_DOLBY5_CORE1_CRC_CNTRL, 1, 0, 1);
			in_crc = READ_VPP_DV_REG(T6W_DOLBY5_CORE1_CRC_IN_FRM);
			out_crc = READ_VPP_DV_REG(T6W_DOLBY5_CORE1_CRC_OUT_FRM);
		}
	...
	int stride[3])
{
	...
	case 0x02:
		...
		if (fix_data == CASE5344_TOP1_READFROM_FILE ||
			...
			fix_data == CASE1080p_TOP1_READFROM_FILE) {
			cntl_bits_mode = 3;
			cntl_pixel_bytes = 1;
			cntl_bit16_mode = 0;
		}
	...
	if (fix_data) {
		...
		if (fix_data != CASE5344_TOP1_READFROM_FILE &&
			fix_data != CASE5363_TOP1_READFROM_FILE &&
			fix_data != CASE1080p_TOP1_READFROM_FILE)/*p444,one plane*/
...
void enable_amdv_hw5(int enable)
{
	...
	if (debug_dolby & 8)
		pr_dv_dbg("enable %d,dv on %d %d %d,mode %d %d,video %d %d,dpss %d\n",
			...
			if (!(amdv_mask & 1) ||
				...
				if (is_aml_t3x()) {
					...
					if (is_amdv_dpss_path() &&
						!(test_dv & DEBUG_FORCE_VCBUS_REG)) {
						dpss_v_rdma_config_a();/*clear rdma buf*/
						dpss_v_rdma_config_b();/*clear rdma buf*/
					}
#endif
					if (is_amdv_dpss_path()) {
						/*top1 int disable*/
						WRITE_VPP_DV_REG_BITS
						(T6W_VPU_DOLBY_IRQ_CTRL0, 0, 0, 17);
						/*top2 int disable*/
						WRITE_VPP_DV_REG_BITS
		...
		if (dolby_vision_on) {
			if (is_aml_tvmode()) {
				if (is_aml_t3x()) {
					...
					/*avoid conflict with vpp rdma write*/
					if (is_aml_t6w() && delay_update_path)
						update_dd_axi_path_mode(1);
					else if (is_aml_t6w())
						update_dd_axi_path_mode(0);
				}
				vpu_module_clk_disable(0, DV_TVCORE, 0);
				...
				bool toggle)
{
	...
	if (debug_dolby & 1)
		pr_dv_dbg("top1_set:on %d %d,reset %d,toggle %d,video %d,py_id %d\n",
				   top1_info.core_on, top1_info.core_on_cnt,
				   reset, toggle, video_enable, top1_info.py_id);
	...
	for (i = 0; i < 2; i++) {
		...
		if (fix_data == CASE5344_TOP1_READFROM_FILE)
			...
			top1_type.rdmif_stride[i] = 240;/*(960x32+127)/128*/
	...
	for (i = 0; i < 7; i++)
		top1_type.py_baddr[i] = py_addr[top1_info.py_id].top1_py_paddr[i];
	...
	if ((top1_vd_info.type & VIDTYPE_VIU_NV21) || (top1_vd_info.type & VIDTYPE_VIU_NV12)) {
		...
		top1_type.fmt_mode = 2;
		if ((debug_dolby & 0x80000) && !fix_data)
			     ...
			     bool pr_done)
{
	...
	dma_addr_t py_baddr[7] = {py_addr[top2_info.py_id].top1_py_paddr[0],
					py_addr[top2_info.py_id].top1_py_paddr[1],
					py_addr[top2_info.py_id].top1_py_paddr[2],
					py_addr[top2_info.py_id].top1_py_paddr[3],
					py_addr[top2_info.py_id].top1_py_paddr[4],
					py_addr[top2_info.py_id].top1_py_paddr[5],
					py_addr[top2_info.py_id].top1_py_paddr[6]};
	...
	if (reset || toggle) {/*no need set these regs when no toggle*/
		...
		if (enable_detunnel) {
			if (vdin_vd1_detunnel_tunnel_mode)
				VSYNC_WR_DV_REG(VPU_DOLBY_WRAP_DTNL, (hsize << 18) | 0x8762);
			else
...
void amdv_sw_reset(u64 *top2_reg)
{
	...
	if (top2_reg && p_reg[5] == 4) {
		...
		if (is_aml_t3x()) {
			...
			VSYNC_WR_DV_REG(T6W_DOLBY5_CORE2_REG_BASE0 + 1, cntrl_value);
			cntrl_value = cntrl_value & 0x3FFFFFFF;
			VSYNC_WR_DV_REG(T6W_DOLBY5_CORE2_REG_BASE0 + 1, cntrl_value);
		}
	} else {
			     ...
			     bool pr_done)
{
	...
	if (!wait_first_frame_top1 ||
		...
		tv_top2_set(top2_reg, hsize, vsize, video_enable,
				enable_detunnel, reset || sw_reset, toggle, pr_done);
		if (video_enable && toggle)
...
void print_dv_ro(void)
{
	...
	if ((debug_dolby & 0x4000) && (enable_top1 || force_enable_top12_lut)) {
		if (is_aml_t3x()) {
			...
			pr_info("vsync: corep RO %x, %x %x %x %x %x\n",
	...
	if ((debug_dolby & 0x4000) && top2_info.core_on) {
		if (is_aml_t3x())
			...
			pr_info("vysnc: top2 RO %x %x %x %x, frame st %x %x\n",
					READ_VPP_DV_REG(T6W_DOLBY_TOP2_RO_3),
					READ_VPP_DV_REG(T6W_DOLBY_TOP2_RO_2),
					READ_VPP_DV_REG(T6W_DOLBY_TOP2_RO_1),
					READ_VPP_DV_REG(T6W_DOLBY_TOP2_RO_0),
					READ_VPP_DV_REG(T6W_DOLBY5_CORE2_INP_FRM_ST),
					READ_VPP_DV_REG(T6W_DOLBY5_CORE2_OP_FRM_ST));
	}
