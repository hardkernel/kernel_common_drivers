...
static ssize_t debug_reg_en_show(const struct class *cla,
			const struct class_attribute *attr, char *buf)
{
	return snprintf(buf, 80, "current debug_reg_enable is %d.\n", debug_reg_en);
}

static ssize_t debug_reg_en_store(const struct class *cla,
				const struct class_attribute *attr,
				const char *buf, size_t count)
{
	long tmp;
...
static ssize_t enhance_sec_en_show(const struct class *class,
		const struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "current enhance_sec_en is %d.\n", enhance_sec_en);
}

static ssize_t enhance_sec_en_store(const struct class *class,
...
static int vicp_probe(struct platform_device *pdev)
{
	...
	dev->coherent_dma_mask = ULLONG_MAX;
