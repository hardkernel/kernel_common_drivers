...
struct vinfo_s *get_current_vinfo2(void)
{
	...
	if (!IS_ERR_OR_NULL(p_module->curr_vout_server[VOUT_VIU1])) {
		data = p_module->curr_vout_server[VOUT_VIU1]->data;
		if (p_module->curr_vout_server[VOUT_VIU1]->op.get_vinfo)
			vinfo = p_module->curr_vout_server[VOUT_VIU1]->op.get_vinfo(data);
	}
	if (!vinfo) /* avoid crash mistake */
		vinfo = get_invalid_vinfo(2, p_module->init_flag[VOUT_VIU1]);
...
{
	return vout_func_validate_vmode(2, name, type, frac);
}
...
int set_vframe2_rate_policy(int policy)
{
	...
	p_module->fr_policy[VOUT_VIU1] = policy;
...
int get_vframe2_rate_policy(void)
{
	...
	return p_module->fr_policy[VOUT_VIU1];
}
