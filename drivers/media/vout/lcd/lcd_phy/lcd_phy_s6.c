...
static void lcd_mipi_phy_set(struct aml_lcd_drv_s *pdrv, int status)
{
	...
	unsigned short CHECK_HS_ability;
	int HSTX_50R_cali_val;
	...
	mipi_dsi_ctl0 = (0x0 << 15) | //serializer reset
			(0x0 << 14) | // ccp reset;
	...
	mipi_dsi_ctl1 = (0x0 << 14) | // CH 0/1/2/3 hstx slew setting (2bit)
			(0x7 << 11) | // ChannelCLK /CH:0/1/2/3/ lptx slew setting (3bit)
			(0x1 << 8)  | // CH0/1/2/3 hstx post setting (3bit)
			(0x3 << 4)  | // CH0/1/2/3 hstx driver ability setting (3bit)
			(HSTX_50R_cali_val << 0); // hstx 50Ω resistance calibration setting(4bit)
	mipi_dsi_ctl2 = (0x0 << 15) | // LPRX ref voltage: 1=0.74V 0=0.88V
			(0x0 << 12) | // ChannelCLK hstx post setting (3bit)
			(0x0 << 11) | // ChannelCLK half clk inversion: 1=inversion 0=no inversion
			(CHECK_HS_ability << 8) | // CCLK hstx driver ability setting(3bit)
			(0x0 << 7)  | // CCLK input full clk inversion:0=inversion 1=no inversion
			(0x0 << 5)  | // ChannelCLK HSTX slew setting (2bit)
			(0x1 << 4)  | // ChannelCLK channel enable
	...
	mipi_dsi_ctl3 = (0x1 << 15) | // full clk enable
			...
			(DSI_PHY_CLK_HS_400mV  << 5) | // Channel CLK 0.4V setting
	...
	lcd_ana_write(ANACTRL_MIPIDSI_CTRL1_S6, mipi_dsi_ctl3 << 16 | mipi_dsi_ctl2);
	lcd_delay_us(20);
	mipi_dsi_ctl0 |= (0x1 << 15); //serializer reset
	lcd_ana_write(ANACTRL_MIPIDSI_CTRL0_S6, mipi_dsi_ctl1 << 16 | mipi_dsi_ctl0);
}
