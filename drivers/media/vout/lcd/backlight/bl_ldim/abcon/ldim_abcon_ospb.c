...
static int abcon_ospb_smr(struct aml_ldim_driver_s *ldim_drv, unsigned int *buf,
		      unsigned int len)
{
	int ret = 0;
...
static int abcon_ospb_smr_dummy(struct aml_ldim_driver_s *ldim_drv)
{
	return 0;
}

static int abcon_ospb_fault_handler(struct aml_ldim_driver_s *ldim_drv)
{
	int ret = 0;
...
static ssize_t abcon_ospb_show(struct class *class, struct class_attribute *attr, char *buf)
{
	struct aml_ldim_driver_s *ldim_drv = aml_ldim_get_driver();
...
static ssize_t abcon_ospb_store(struct class *class, struct class_attribute *attr,
...
static int abcon_ospb_dev_update(struct ldim_dev_driver_s *dev_drv)
{
	...
	dev_drv->dev_smr = abcon_ospb_smr;
	dev_drv->dev_smr_dummy = abcon_ospb_smr_dummy;
	dev_drv->dev_err_handler = abcon_ospb_fault_handler;
