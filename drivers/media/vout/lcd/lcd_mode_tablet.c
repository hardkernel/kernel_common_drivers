...
static int lcd_set_vframe_rate_hint(int duration, void *data)
{
	...
	if (duration == 0) { /* end hint */
		...
		LCDPR("[%d]: %s: duration: %d, frame_rate: %d, fr_adj_type: %d\n",
		      pdrv->index, __func__, duration, duration / 100,
		      pdrv->config.timing.act_timing.fr_adjust_type);
...
void lcd_tablet_vout_server_init(struct aml_lcd_drv_s *pdrv)
{
	...
	if (!strcmp(lcd_connector, curr_vout_connector) ||
		...
		pdrv->viu_sel = 1;
		LCDPR("%s: panel_mode:%u, connector0=%s, mode:%s\n", __func__,
			lcd_legacy_panel_disp_mode, curr_vout_connector, curr_vout_mode);
	}
#endif
#ifdef CONFIG_AMLOGIC_VOUT2_SERVE
...
static void lcd_tablet_config_post_update(struct aml_lcd_drv_s *pdrv)
{
