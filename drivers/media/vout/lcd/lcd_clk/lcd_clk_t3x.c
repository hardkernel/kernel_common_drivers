...
static void lcd_pll_frac_set(struct aml_lcd_drv_s *pdrv, unsigned int frac)
{
	...
	LCD_DBG(pdrv, "%s: phyconf reg 0x%x: 0x%08x->0x%08x, pll_frac=0x%x\n", __func__,
		ANACTRL_TCON_PLL0_CNTL1 + offset[0], val[0],
		lcd_ana_read(ANACTRL_TCON_PLL0_CNTL1 + offset[0]), phyconf->pll_frac);
	...
	if (cconf->pll_mode & LCD_PLL_MODE_DUAL_PLL) {
		...
		LCD_DBG(pdrv, "%s: pixconf reg 0x%x: 0x%08x->0x%08x, pll_frac=0x%x", __func__,
			ANACTRL_TCON_PLL0_CNTL1 + offset[1], val[1],
			lcd_ana_read(ANACTRL_TCON_PLL0_CNTL1 + offset[1]), pixconf->pll_frac);
	}
...
static void lcd_set_tcon_clk_t3x(struct aml_lcd_drv_s *pdrv)
{
	...
	LCD_DBG(pdrv, "lcd clk: set_tcon_clk");
