...
static unsigned int pll_ss_reg_table[][2] = {
	/* dep_sel,  str_m  */
	{ 0,          0}, /* 0: disable */
	{ 4,          1}, /* 1: +/-0.1% */
	{ 4,          2}, /* 2: +/-0.2% */
...
static void lcd_pll_ss_init(struct lcd_clk_config_s *cconf)
{
	...
	if (cconf->ss_level > 0) {
		cconf->ss_en = 1;
		cconf->ss_dep_sel = pll_ss_reg_table[cconf->ss_level][0];
		cconf->ss_str_m = pll_ss_reg_table[cconf->ss_level][1];
...
static void lcd_set_pll_ss(struct aml_lcd_drv_s *pdrv, unsigned int ss_flag)
{
	...
	if (ss_flag & LCD_SSC_LEVEL) {
		...
		if (level > 0) {
			cconf->ss_en = 1;
			cconf->ss_dep_sel = pll_ss_reg_table[level][0];
			cconf->ss_str_m = pll_ss_reg_table[level][1];
			cconf->ss_ppm =
...
static void lcd_set_pll(struct aml_lcd_drv_s *pdrv)
{
	...
	pll_ctrl = ((0x3 << 17) | /* gate ctrl */
		...
		(cconf->pll_config->pll_n << LCD_PLL_N_TL1) |
		(cconf->pll_config->pll_m << LCD_PLL_M_TL1) |
		(cconf->pll_config->pll_od3_sel << LCD_PLL_OD3_TL1) |
		(cconf->pll_config->pll_od2_sel << LCD_PLL_OD2_TL1) |
		(cconf->pll_config->pll_od1_sel << LCD_PLL_OD1_TL1));
	pll_ctrl1 = (1 << 28) |
...
set_pll_retry_tl1:
	...
	ret = lcd_pll_wait_lock(cconf->pll_config->pll_id, HHI_TCON_PLL_CNTL0, LCD_PLL_LOCK_TL1);
	if (ret) {
...
static struct lcd_pll_data_s lcd_pll_data_tm2 = {
	.pll_od_fb = 0,
	.pll_m_max = 511,
	.pll_m_min = 2,
	.pll_n_max = 1,
...
struct lcd_clk_config_s *lcd_clk_config_chip_init_tm2(struct aml_lcd_drv_s *pdrv)
{
	...
	cconf->pll_config[0].pll_od_fb = lcd_pll_data_tm2.pll_od_fb;
