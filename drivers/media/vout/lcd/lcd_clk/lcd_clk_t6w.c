...
static unsigned int tcon_div_t6w[][3] = {
...
static void lcd_set_pll_ss_t6w(struct aml_lcd_drv_s *pdrv, unsigned int ss_flag)
{
	struct lcd_clk_config_s *cconf;
...
static void lcd_set_pll_t6w(struct aml_lcd_drv_s *pdrv)
{
	...
	pll_ctrl3 = 0x180 |
		...
		(tcon_div_sel << 24) | /*tcon_div without phase shift*/
		(1 << 9); /*tcon_od_en*/
	if (pconf->basic.lcd_type != LCD_MLVDS) {
		...
		pll_ctrl3 |= (1 << 27) |
			(tcon_div_t6w[tcon_div_sel][0] << 28) |
			(tcon_div_t6w[tcon_div_sel][1] << 29) |
			(tcon_div_t6w[tcon_div_sel][2] << 30);
	}
	...
	do {
		...
		lcd_vx1_lvds_ctrl_write(pdrv, ANACTRL_TCON_PLL0_CNTL2, 0x02000000);
	...
	if (ret)
		LCD_ERR(pdrv, "vx1 pll lock failed");
	...
	if (cconf->ss_level > 0)
		lcd_set_pll_ss_t6w(pdrv, (LCD_SSC_LEVEL | LCD_SSC_FREQ | LCD_SSC_MODE));
}
...
static void lcd_disable_pll_t6w(struct aml_lcd_drv_s *pdrv)
{
	...
	lcd_vx1_lvds_ctrl_setb(pdrv, ANACTRL_TCON_PLL0_CNTL0, 0, 29, 1);  //lock resetn
	lcd_vx1_lvds_ctrl_setb(pdrv, ANACTRL_TCON_PLL0_CNTL0, 0, 30, 1);  //pll resetn
...
static void lcd_set_vid_pll_div_t6w(struct aml_lcd_drv_s *pdrv)
{
	...
	if (cconf->pll_mode & LCD_PLL_MODE_DUAL_PLL) {
		LCD_DBG(pdrv, "no need to set clk divider for gp2");
		return;
	}
...
static void lcd_clk_set_dummy_t6w(struct aml_lcd_drv_s *pdrv)
{
...
static void lcd_clk_prbs_test_t6w(struct aml_lcd_drv_s *pdrv,
				 unsigned int ms, unsigned int mode_flag)
{
	...
	for (i = 0; i < LCD_PRBS_MODE_MAX; i++) {
		...
		while (lcd_prbs_flag) {
			...
			val = lcd_vx1_lvds_ctrl_getb(pdrv, reg_phy_tx_ctrl1, 0, 12);
			if (val != 0xfff) {
...
lcd_prbs_test_err_t3:
	...
	for (j = 0; j < 12; j++) {
		bit = j & 0x1 ? 16 : 0;
		lcd_vx1_lvds_ctrl_setb(pdrv, chdig_reg[j >> 1],
...
static struct lcd_pll_data_s lcd_pll_data_tcon_t6w = {
	.pll_od_fb = 0,
	.pll_0_5_div_en = 0,
	.pll_m_max = 511,
	.pll_m_min = 2,
...
static struct lcd_pll_data_s lcd_pll_data_gp2_t6w = {
	.pll_od_fb = 0,
	.pll_0_5_div_en = 0,
	.pll_m_max = 511,
	.pll_m_min = 2,
...
static struct lcd_clk_data_s lcd_clk_data_t6w = {
	.pll_data[0] = &lcd_pll_data_tcon_t6w,
	.pll_data[1] = &lcd_pll_data_gp2_t6w,
	...
	.clk_set_dummy = lcd_clk_set_dummy_t6w,
	.clk_disable = lcd_clk_disable_t6w,
	.mlvds_clk_phase_set = lcd_set_mlvds_clk_phase_t6w,
