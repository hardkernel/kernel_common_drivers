...
static void dsi_phy_init(struct aml_lcd_drv_s *pdrv, struct dsi_dphy_s *dphy)
{
	...
	/* 0x8d40  //1E848-> 48 to reduce sim time. */
...
static void set_dsi_phy_config(struct aml_lcd_drv_s *pdrv)
{
	...
	dsi_host_write(pdrv, MIPI_DSI_DWC_PHY_TST_CTRL1_OS, 0x00010044);/*test code*/
	dsi_host_write(pdrv, MIPI_DSI_DWC_PHY_TST_CTRL0_OS, 0x2);
	dsi_host_write(pdrv, MIPI_DSI_DWC_PHY_TST_CTRL0_OS, 0x0);
	dsi_host_write(pdrv, MIPI_DSI_DWC_PHY_TST_CTRL1_OS, 0x00000074);/*test write*/
	dsi_host_write(pdrv, MIPI_DSI_DWC_PHY_TST_CTRL0_OS, 0x2);
	dsi_host_write(pdrv, MIPI_DSI_DWC_PHY_TST_CTRL0_OS, 0x0);
...
/* bit_rate is confirm by clk_gen_rate, so internal clk config must after that */
