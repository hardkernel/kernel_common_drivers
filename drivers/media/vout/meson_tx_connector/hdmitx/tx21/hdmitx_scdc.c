...
void scdc21_config(struct hdmitx21_dev *hdev)
{
	...
	scdc21_wr_sink(SCDC_TMDS_CFG,
		hdev->tx_comm.fmt_para.tx_hw_para.hdmitx_hw_para.tmds_clk_div40 ? 0x3 : 0);
}
...
int scdc21_status_flags(struct hdmitx21_dev *hdev)
{
	...
	if (st & STATUS_UPDATE) {
		...
		hdev->tx_comm.ch_locked_st.clock_detected = locked_st & BIT(0);
		hdev->tx_comm.ch_locked_st.ch0_locked = !!(locked_st & BIT(1));
		hdev->tx_comm.ch_locked_st.ch1_locked = !!(locked_st & BIT(2));
		hdev->tx_comm.ch_locked_st.ch2_locked = !!(locked_st & BIT(3));
		hdev->tx_comm.ch_locked_st.ch3_locked = !!(locked_st & BIT(4));
	}
	if (st & CED_UPDATE)
		scdc_ced_cnt(hdev);
	if (st & RSED_UPDATE)
	...
	if (st & STATUS_UPDATE) {
		if (!frl_rate && !hdev->tx_comm.ch_locked_st.clock_detected)
			HDMITX_INFO("ced: clock undetected\n");
		if (!hdev->tx_comm.ch_locked_st.ch0_locked)
			HDMITX_INFO("ced: ch0 unlocked\n");
		if (!hdev->tx_comm.ch_locked_st.ch1_locked)
			HDMITX_INFO("ced: ch1 unlocked\n");
		if (!hdev->tx_comm.ch_locked_st.ch2_locked)
			HDMITX_INFO("ced: ch2 unlocked\n");
		if (frl_rate && !hdev->tx_comm.ch_locked_st.ch3_locked)
