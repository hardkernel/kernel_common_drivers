...
static void config_tv_enc_calc(struct hdmitx21_dev *hdev, enum hdmi_vic vic)
{
	...
	if (hdev->tx_comm.fmt_para.cs == HDMI_COLORSPACE_YUV420)
		y420_mode = 1;
	tp = meson_tx_mode_vic_to_timing(vic);
	if (!tp) {
	...
	if (hdev->tx_comm.fmt_para.tx_hw_para.hdmitx_hw_para.frl_rate && y420_mode == 1)
		hpara_div = 4;
	if (hdev->tx_comm.fmt_para.tx_hw_para.hdmitx_hw_para.frl_rate && y420_mode == 0)
		hpara_div = 2;
	...
	u32 enable)
{
	...
	else if (hdev->tx_comm.fmt_para.tx_hw_para.hdmitx_hw_para.frl_rate &&
...
void hdmitx21_pbist_config(struct hdmitx21_dev *hdev, enum hdmi_vic vic, int reg_pbist_en)
{
	...
	if (reg_pbist_en && (hdev->tx_comm.tx_hw->chip_data->chip_type == MESON_CPU_ID_S7 ||
