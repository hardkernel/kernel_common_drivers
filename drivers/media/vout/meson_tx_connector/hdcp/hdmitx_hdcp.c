...
static void hdcptx_update_failures(struct hdcptx21_core *p_hdcp, enum hdcp_fail_types_t types);
static bool hdcp1x_ds_ksv_fifo_ready(u8 intr_reg);
static void hdcp2x_auth_stop(struct hdcptx21_core *p_hdcp);
static void hdcptx_send_csm_msg(struct hdcptx21_core *p_hdcp);
static void hdcptx_reset(struct hdcptx21_core *p_hdcp);
static void hdcptx_get_ds_bksv_list(struct hdcptx21_core *p_hdcp);
static void hdcptx_get_ds_rcv_id(struct hdcptx21_core *p_hdcp);
static void hdcptx_update_csm(struct hdcptx21_core *p_hdcp);
...
void hdcptx_mode_set(struct hdcptx21_core *p_hdcp, unsigned int mode)
{
	if (!p_hdcp) {
...
static void hdcptx_enable_mode(struct hdcptx21_core *p_hdcp, int hdcp_type)
{
	...
	hdcptx_comm = p_hdcp->bind_hdcptx_comm;
	if (hdcp_type == HDCP_START_NONE) {
		hdcptx_comm->hdcp_mode = 0x0;
		hdcptx_mode_set(p_hdcp, hdcp_type);
		hdmitx21_ctrl_hdcp_gate(hdcptx_comm->chip_type, 0, false);
	} else if (hdcp_type == HDCP_START_HDCP1X) {
		if (hdcptx_comm->efuse_dis_hdcp_tx14) {
			HDMITX_ERROR("warning, efuse disable hdcptx14\n");
			return;
		}
		hdcptx_comm->hdcp_mode = hdcp_type;
		hdcptx_mode_set(p_hdcp, hdcp_type);
		hdmitx21_ctrl_hdcp_gate(hdcptx_comm->chip_type, hdcp_type, true);
	} else if (hdcp_type == HDCP_START_HDCP2X) {
		if (hdcptx_comm->efuse_dis_hdcp_tx22) {
...
static void hdmitx21_enable_hdcp(struct hdcptx21_core *p_hdcp)
{
	struct hdcptx_common *hdcp_comm = NULL;

	if (!p_hdcp || !p_hdcp->bind_hdcptx_comm) {
		HDMITX_ERROR("%s NULL tx_comm or NULL hdcp_private instance\n", __func__);
		return;
	}
	hdcp_comm = p_hdcp->bind_hdcptx_comm;
	/*
	...
	if (p_hdcp->hdcptx_start_type == HDCP_START_AUTO) {
		if (hdcp_comm->hdcp_mode != 0) {
			HDMITX_HDCP_INFO("[%s] hdcp %d already enabled, exit\n",
				__func__, hdcp_comm->hdcp_mode);
			mutex_unlock(&p_hdcp->hdcptx_auth_mutex);
			return;
		}

		if (hdcp_comm->hdcp_lstore == 0) {
			if (get_hdcptx_lstore(p_hdcp->bind_hdcptx_comm, HDCP_VER_HDCP2X) &&
				hdcp_comm->dw_hdcp22_cap) {
				/* enable hdcp gate */
				hdmitx21_ctrl_hdcp_gate(hdcp_comm->chip_type, 2, true);
				hdcp_comm->hdcp_mode = 2;
				hdcptx_mode_set(p_hdcp, 2);
			} else {
				if (p_hdcp->frl_rate > FRL_NONE &&
					p_hdcp->frl_rate < FRL_RATE_MAX) {
					hdcp_comm->hdcp_mode = 0;
					HDMITX_HDCP_INFO("[%s] not enable hdcp1x under FRL\n",
						__func__);
				} else if (get_hdcptx_lstore(hdcp_comm, HDCP_VER_HDCP1X)) {
					hdmitx21_ctrl_hdcp_gate(hdcp_comm->chip_type,
						1, true);
					hdcp_comm->hdcp_mode = 1;
		...
		} else if (hdcp_comm->hdcp_lstore & 0x2) {
			if (get_hdcptx_lstore(p_hdcp->bind_hdcptx_comm, HDCP_VER_HDCP2X) &&
				hdcp_comm->dw_hdcp22_cap) {
				hdmitx21_ctrl_hdcp_gate(hdcp_comm->chip_type,
					2, true);
		...
		} else if (hdcp_comm->hdcp_lstore & 0x1) {
			if (p_hdcp->frl_rate > FRL_NONE &&
				p_hdcp->frl_rate < FRL_RATE_MAX) {
				hdcp_comm->hdcp_mode = 0;
				...
				hdmitx21_ctrl_hdcp_gate(hdcp_comm->chip_type, 1, true);
				hdcp_comm->hdcp_mode = 1;
				hdcptx_mode_set(p_hdcp, 1);
			} else {
				hdmitx21_ctrl_hdcp_gate(hdcp_comm->chip_type, 0, false);
				hdcp_comm->hdcp_mode = 0;
			}
		} else {
		...
		/* for drm start hdcp or debug node */
		hdcptx_enable_mode(p_hdcp, p_hdcp->hdcptx_start_type);
	}
	mutex_unlock(&p_hdcp->hdcptx_auth_mutex);
}

static void hdcptx_enable(void *hdcptx_instance, int hdcp_type, unsigned long delay)
{
	struct hdcptx21_core *p_hdcp = (struct hdcptx21_core *)hdcptx_instance;

	if (!p_hdcp) {
		HDMITX_ERROR("%s NULL hdcp instance\n", __func__);
		return;
	}
	p_hdcp->hdcptx_start_type = hdcp_type;
	schedule_delayed_work(&p_hdcp->work_start_hdcp, msecs_to_jiffies(delay));
}

/* use hdcptx_stop_work_sync outside of hdcp auth handlers */
static void hdcptx_cancel_works(struct hdcptx21_core *p_hdcp)
{
	u8 stop_work_max = 3;
 ...
 * there are below cases need to disable hdcp:
 ...
 */
static void hdmitx21_disable_hdcp(void *hdcptx_instance)
{
	struct hdcptx21_core *p_hdcp = (struct hdcptx21_core *)hdcptx_instance;
	struct hdcptx_common *hdcp_comm = NULL;
	...
	cancel_delayed_work(&p_hdcp->work_start_hdcp);
	if (hdcp_comm->hdcp_mode != 0) {
...
static u32 hdmitx21_get_hdcptx_mode(void *hdcptx_instance)
{
	u32 hdcp_mode = 0;
	struct hdcptx21_core *p_hdcp = (struct hdcptx21_core *)hdcptx_instance;

	if (!p_hdcp || !p_hdcp->bind_hdcptx_comm) {
		HDMITX_ERROR("NULL p_hdcp or NULL hdcp_comm instance\n");
		return hdcp_mode;
	}

	mutex_lock(&p_hdcp->hdcptx_auth_mutex);
	hdcp_mode = p_hdcp->bind_hdcptx_comm->hdcp_mode;
	mutex_unlock(&p_hdcp->hdcptx_auth_mutex);
	return hdcp_mode;
}
...
static void hdcptx_update_state(struct hdcptx21_core *p_hdcp, enum hdcp_stat_t state)
{
	if (!p_hdcp) {
...
static void hdcp2x_reauth_start(struct hdcptx21_core *p_hdcp)
{
	if (!p_hdcp) {
		HDMITX_ERROR("%s NULL private_hdcp struct\n", __func__);
		return;
...
static void hdcptx_topology_update(struct hdcptx21_core *p_hdcp)
{
	struct hdcp_topo_t *topo = &p_hdcp->hdcp_topology;
	struct hdcptx_common *hdcp_comm = NULL;

	if (!p_hdcp || !p_hdcp->bind_hdcptx_comm) {
		HDMITX_ERROR("%s NULL hdcp_comm or NULL private_hdcp struct\n", __func__);
		return;
	}
	hdcp_comm = p_hdcp->bind_hdcptx_comm;
	...
	if (p_hdcp->hdcp_type == HDCP_VER_HDCP2X) {
		...
		if (p_hdcp->ds_repeater) {
			u8 bstatus[2];
			u8 max_devices = hdcp_comm->hdcp_rpt_en ?
...
static void hdcptx_encryption_update(struct hdcptx21_core *p_hdcp, bool en)
{
	if (!p_hdcp) {
		HDMITX_ERROR("%s NULL private_hdcp struct\n", __func__);
		return;
...
static void hdcptx_check_ds_csm_status(struct hdcptx21_core *p_hdcp)
{
	if (!p_hdcp) {
		HDMITX_ERROR("%s NULL private_hdcp struct\n", __func__);
		return;
...
static void hdcptx_authenticated_handle(struct hdcptx21_core *p_hdcp)
{
	if (!p_hdcp || !p_hdcp->bind_hdcptx_comm) {
	...
	if (p_hdcp->hdcp_type == HDCP_VER_HDCP1X) {
		...
		/* ds is hdcp2.2 TV */
		hdcptx22_topo_ctrl(p_hdcp->bind_hdcptx_comm,
			SET_HDCP22_TOPO, 1);
		hdcptx_check_ds_csm_status(p_hdcp);
	}
}

static bool hdcptx_query_ds_repeater(struct hdcptx21_core *p_hdcp)
{
	if (!p_hdcp) {
		HDMITX_ERROR("%s NULL private_hdcp struct\n", __func__);
		return false;
...
static void hdcp1x_check_bksv_done(struct hdcptx21_core *p_hdcp)
{
	u8 copp_data1;
...
static void hdcptx_reset_ksv_fifo(struct hdcptx21_core *p_hdcp)
{
	if (!p_hdcp) {
		HDMITX_ERROR("%s NULL private_hdcp struct\n", __func__);
		return;
...
static bool hdcptx_is_topology_correct(struct hdcptx21_core *p_hdcp)
{
	...
	max_device_count = p_hdcp->bind_hdcptx_comm->hdcp_rpt_en ?
		HDCP1X_MAX_TX_DEV_RPT : HDCP1X_MAX_TX_DEV_SRC;
...
static void hdcptx_get_ds_rcvid_lists(struct hdcptx21_core *p_hdcp, u8 count)
{
	if (!p_hdcp) {
		HDMITX_ERROR("%s NULL private_hdcp struct\n", __func__);
		return;
...
static void hdcptx_assemble_ds_ksv_lists(struct hdcptx21_core *p_hdcp)
{
	if (!p_hdcp) {
		HDMITX_ERROR("%s NULL private_hdcp struct\n", __func__);
		return;
...
static bool hdcptx_process_repeater_fail(struct hdcptx21_core *p_hdcp)
{
	if (!p_hdcp) {
		HDMITX_ERROR("%s NULL private_hdcp struct\n", __func__);
		return false;
...
static void hdcptx_rpt_ready_process(struct hdcptx21_core *p_hdcp, bool ksv_read_status)
{
	struct hdcptx_common *hdcp_comm = NULL;
	...
	hdcp_comm = p_hdcp->bind_hdcptx_comm;
	bool hdcp_rpt_en = hdcp_comm->hdcp_rpt_en;
...
static void hdcptx_notify_rpt_info(struct work_struct *work)
{
	...
	unsigned int i = 0;
	unsigned int j = 0;
	struct hdcptx21_core *p_hdcp = container_of((struct delayed_work *)work,
		struct hdcptx21_core, ksv_notify_wk);
	struct hdcp_topo_t *topo = &p_hdcp->hdcp_topology;
	...
	hdcp_comm = p_hdcp->bind_hdcptx_comm;
	hdcp_rpt_en = hdcp_comm->hdcp_rpt_en;
	/*
...
{
	struct hdcptx21_core *p_hdcp = container_of((struct delayed_work *)work,
			struct hdcptx21_core, req_reauth_wk);
	struct hdcptx_common *hdcptx_comm = NULL;
	...
	if (hdcp_reauth_dbg == 1) {
		...
		if (hdcptx_comm->video_mute_op)
			hdcptx_comm->video_mute_op(hdcptx_comm->bind_hw_comm,
				0, VIDEO_MUTE_PATH_3);
		if (hdcptx_comm->avmute_op)
			hdcptx_comm->avmute_op(hdcptx_comm->bind_tx_comm,
				SET_AVMUTE, AVMUTE_PATH_2);
		...
		if (hdcptx_comm->avmute_op)
			hdcptx_comm->avmute_op(hdcptx_comm->bind_tx_comm,
				CLR_AVMUTE, AVMUTE_PATH_2);
	} else if (hdcp_reauth_dbg == 4) {
		if (hdcptx_comm->hdcp_mode) {
			if (p_hdcp->req_reauth_ver == 0 ||
				p_hdcp->req_reauth_ver == hdcptx_comm->hdcp_mode) {
	...
	if (p_hdcp->req_reauth_ver == 0) {
		...
		hdmitx21_enable_hdcp(p_hdcp);
	} else if (p_hdcp->req_reauth_ver == 1) {
		/* force hdcp1.x mode */
		mutex_lock(&p_hdcp->hdcptx_auth_mutex);
		if (get_hdcptx_lstore(hdcptx_comm, HDCP_VER_HDCP1X)) {
			hdcptx_comm->hdcp_mode = 1;
			hdcptx_mode_set(p_hdcp, 1);
		} else {
			hdcptx_comm->hdcp_mode = 0;
		}
		mutex_unlock(&p_hdcp->hdcptx_auth_mutex);
	} else if (p_hdcp->req_reauth_ver == 2) {
		/* force hdcp2.x mode */
		mutex_lock(&p_hdcp->hdcptx_auth_mutex);
		if (hdcptx22_topo_ctrl(hdcptx_comm, GET_HDCP22_TOPO, 0) &&
			hdcptx_comm->dw_hdcp22_cap) {
			hdcptx_comm->hdcp_mode = 2;
			hdcptx_mode_set(p_hdcp, 2);
		} else {
			hdcptx_comm->hdcp_mode = 0;
		}
...
void hdmitx21_rst_stream_type(struct hdcptx21_core *p_hdcp)
{
	if (!p_hdcp)
		return;
...
{
	struct hdcptx21_core *p_hdcp = container_of((struct delayed_work *)work,
		struct hdcptx21_core, stream_mute_wk);
	struct hdcptx_common *hdcptx_comm = NULL;
	...
	if (hdcptx_comm->video_mute_op)
		hdcptx_comm->video_mute_op(hdcptx_comm->bind_hw_comm,
			!p_hdcp->stream_mute, VIDEO_MUTE_PATH_2);
	if (hdcptx_comm->audio_mute_op)
		hdcptx_comm->audio_mute_op(hdcptx_comm->bind_tx_comm,
			!p_hdcp->stream_mute, AUDIO_MUTE_PATH_3);
}
...
{
	struct hdcptx21_core *p_hdcp = container_of((struct delayed_work *)work,
		struct hdcptx21_core, stream_type_wk);
	u8 update_scene = 0;
	u8 upstream_type = 0;
	...
	if (!p_hdcp || !p_hdcp->bind_hdcptx_comm) {
		HDMITX_ERROR("%s NULL hdcp_comm or NULL hdcp_private instance\n", __func__);
		return;
	}
	hdcp_comm = p_hdcp->bind_hdcptx_comm;
	...
	if (update_scene == STREAMTYPE_UPDATE) {
		...
		if (hdmitx21_get_hdcptx_mode(p_hdcp) == 0) {
...
static u8 hdcptx_reauth_request(void *hdcptx_priv, u8 hdcp_version)
{
	u8 upstream_type = 0;
	u8 update_scene = 0;
	struct hdcptx21_core *p_hdcp;
	...
	if (update_scene == 0x0) {
		 ...
		 * there are some special source devices, firstly do
...
static void hdcp1x_auth_start(struct hdcptx21_core *p_hdcp)
{
	bool key_valid;
	...
	if (!p_hdcp->bind_hdcptx_comm->hdcp14_key_loaded) {
		key_valid = hdcptx1_load_key();
		if (!key_valid)
			HDMITX_ERROR("%s: hdcp1.4 key load failed!\n", __func__);
		else
			p_hdcp->bind_hdcptx_comm->hdcp14_key_loaded = true;
	} else {
...
static void hdcp2x_auth_start(struct hdcptx21_core *p_hdcp)
{
	u8 content_type;
...
static enum hdcp_ver_t hdcp_check_ds_hdcp2ver(struct hdcptx21_core *p_hdcp)
{
...
static void hdcp1x_auth_stop(struct hdcptx21_core *p_hdcp)
{
	hdcptx1_auth_stop();
	hdcptx1_protection_enable(false);
	hdcptx_ddc_toggle_sw_tpi();
}
...
static void hdcp_reset_hw(struct hdcptx21_core *p_hdcp)
{
	hdcptx_init_reg();
...
static void hdcp2x_validate_csm_msg(struct hdcptx21_core *p_hdcp)
{
...
static void hdcptx_auth_start(struct hdcptx21_core *p_hdcp)
{
...
static void hdcptx_check_ds_auth_whandler(struct work_struct *w)
{
	...
	struct hdcptx21_core *p_hdcp = NULL;
	...
	if (strcmp(work->name, "hdcp_rcv_auth") == 0)
		p_hdcp = container_of(work, struct hdcptx21_core, timer_hdcp_rcv_auth);
	else if (strcmp(work->name, "hdcp_rpt_auth") == 0)
		p_hdcp = container_of(work, struct hdcptx21_core, timer_hdcp_rpt_auth);
	if (!p_hdcp) {
...
static void hdcptx_check_bksv_done_whandler(struct work_struct *w)
{
	...
	struct hdcp_work *work = container_of((struct delayed_work *)w,
		struct hdcp_work, dwork);
	struct hdcptx21_core *p_hdcp = container_of(work, struct hdcptx21_core,
...
static int hdmitx_validate_hdcp_key(struct hdcptx_common *hdcptx_comm, int hdcp_type)
{
	int ret = 0;
	int set_mode_ret = 0;
	...
	if (!hdcptx_comm) {
		HDMITX_ERROR("%s NULL hdcp_comm instance\n", __func__);
		return 0;
	}

	if (hdcp_type == HDCP_VER_HDCP2X) {
		/* consider hdcp2.2 key always valid if key length match */
		if (hdcptx22_topo_ctrl(hdcptx_comm, GET_HDCP22_TOPO, 0))
			ret = 1;
	} else if (hdcp_type == HDCP_VER_HDCP1X) {
		if (get_hdcptx_lstore(hdcptx_comm, HDCP_VER_HDCP1X)) {
			/* enable tmds clk */
			if (hdcptx_comm->tmds_enable)
				set_mode_ret = hdcptx_comm->tmds_enable(hdcptx_comm->bind_tx_comm);
			if (set_mode_ret < 0)
				return -1;
			/* enable hdcp1.4 gate if gate is disabled */
			if (hdcptx_comm->chip_type >= MESON_CPU_ID_S7) {
				hdcp14_gate_status =
					hdmitx21_get_gate_status() & BIT_HDMITX_TOP_CLK_GATE_HDCP1X;
				if (!hdcp14_gate_status)
					hdmitx21_ctrl_hdcp_gate(hdcptx_comm->chip_type, 1, true);
			}
			/* load key into HW for crc check */
			ret = hdcptx1_load_key();
			/* recover disable tmds clk */
			if (set_mode_ret == 0) {
				if (hdcptx_comm->tmds_disable)
					hdcptx_comm->tmds_disable(hdcptx_comm->bind_tx_comm);
			}
			/* recover hdcp1.4 gate status */
			if (hdcptx_comm->chip_type >= MESON_CPU_ID_S7) {
				if (!hdcp14_gate_status)
					hdmitx21_ctrl_hdcp_gate(hdcptx_comm->chip_type,
						1, false);
			}
		}
	}
...
{
	struct hdcptx21_core *p_hdcp = container_of((struct delayed_work *)work,
		struct hdcptx21_core, work_start_hdcp);
	struct hdcptx_common *hdcptx_comm = NULL;
	...
	if (hdcptx_need_ctrl_by_upstream(hdcptx_comm->hdcp_rpt_en)) {
		...
		if (p_hdcp->is_passthrough_switch) {
			...
			if (!hdcptx_comm->ready || !hdcptx_comm->hpd_state) {
				HDMITX_INFO("signal ready: %d, hpd_state: %d, eixt hdcp\n",
					hdcptx_comm->ready, hdcptx_comm->hpd_state);
...
{
	struct hdcptx21_core *p_hdcp = container_of((struct delayed_work *)work,
		struct hdcptx21_core, work_up_hdcp_timeout);
	struct hdcptx_common *hdcptx_comm = NULL;
...
static void hdcptx_reset_param(void *hdcptx_instance)
{
	struct hdcptx21_core *p_hdcp = (struct hdcptx21_core *)hdcptx_instance;
	...
	p_hdcp->is_passthrough_switch = 0;
	p_hdcp->bind_hdcptx_comm->dw_hdcp22_cap = false;
	/* clear audio/video mute flag of stream type */
	if (p_hdcp->bind_hdcptx_comm->video_mute_op)
		p_hdcp->bind_hdcptx_comm->video_mute_op(p_hdcp->bind_hdcptx_comm->bind_hw_comm,
			1, VIDEO_MUTE_PATH_2);
	if (p_hdcp->bind_hdcptx_comm->audio_mute_op)
		p_hdcp->bind_hdcptx_comm->audio_mute_op(p_hdcp->bind_hdcptx_comm->bind_tx_comm,
			1, AUDIO_MUTE_PATH_3);
}

static void hdcptx_debug_func(void *hdcptx_instance, const char *buf)
{
	struct hdcptx21_core *p_hdcp = (struct hdcptx21_core *)hdcptx_instance;
	struct hdcptx_common *hdcptx_comm = NULL;
	int ret;
	...
	hdcptx_comm = p_hdcp->bind_hdcptx_comm;
	if (strncmp(buf, "reauth_dbg", 10) == 0) {
		ret = kstrtoul(buf + 10, 10, &value);
		hdcp_reauth_dbg = value;
		HDMITX_INFO("set hdcp_reauth_dbg :%lu\n", hdcp_reauth_dbg);
	} else if (strncmp(buf, "streamtype_dbg", 14) == 0) {
		ret = kstrtoul(buf + 14, 10, &value);
		streamtype_dbg = value;
		HDMITX_INFO("set streamtype_dbg :%lu\n", streamtype_dbg);
	} else if (strncmp(buf, "en_fake_rcv_id", 14) == 0) {
		ret = kstrtoul(buf + 14, 10, &value);
		en_fake_rcv_id = value;
		HDMITX_INFO("set en_fake_rcv_id :%lu\n", en_fake_rcv_id);
	} else if (strncmp(buf, "hdcp_timeout", 12) == 0) {
		ret = kstrtoul(buf + 12, 10, &value);
		if (p_hdcp)
			p_hdcp->up_hdcp_timeout_sec = value;
	} else if (strncmp(buf, "avmute_ms", 9) == 0) {
		ret = kstrtoul(buf + 9, 10, &value);
		avmute_ms = value;
		HDMITX_INFO("set avmute_ms :%lu\n", avmute_ms);
	} else if (strncmp(buf, "vid_mute_ms", 10) == 0) {
		ret = kstrtoul(buf + 10, 10, &value);
		vid_mute_ms = value;
		HDMITX_INFO("set vid_mute_ms :%lu\n", vid_mute_ms);
	} else if (strncmp(buf, "hdcp_result", 11) == 0) {
		HDMITX_INFO("hdcp filtered result: hdcp22: %d topo: %d, hdcp14: %d\n",
		get_hdcptx_result(hdcptx_comm, 2),
		hdcptx22_topo_ctrl(hdcptx_comm, GET_HDCP22_TOPO, 0),
		get_hdcptx_result(hdcptx_comm, 1));
	} else if (strncmp(buf, "propagate_stream_type", 21) == 0) {
		if (p_hdcp->ds_repeater && p_hdcp->hdcp_type == HDCP_VER_HDCP2X) {
	...
	} else if (strncmp(buf, "cont_smng_method", 16) == 0) {
		/*
		...
		if (isdigit(buf[16])) {
			...
			hdcptx_comm->not_restart_hdcp = !!value;
		}
	} else if (strncmp(buf, "hdcp_delay", 10) == 0) {
		if (!p_hdcp)
			return;
		ret = kstrtoul(buf + 10, 10, &value);
		HDMITX_INFO("hdcp_delay :%d\n", value);
		hdcptx_comm->hdcp_debug_delay = value;
	} else if (strncmp(buf, "load_14key", 10) == 0) {
		HDMITX_INFO("hdcp1.4 key load result: %d\n",
			hdmitx_validate_hdcp_key(hdcptx_comm, 0x1));
	}
}

static int hdcp_communicate_open(struct inode *inode, struct file *file)
{
	struct hdcptx21_core *p_hdcp = container_of(file->private_data,
		struct hdcptx21_core, hdcp_misc_dev);

	file->private_data = p_hdcp;
	return 0;
}
	...
	unsigned long arg)
{
	...
	int hdcp_key = 0;
	int hdcprx_ver = 0;
	struct hdcptx21_core *p_hdcp = file->private_data;
	void __user *argp = (void __user *)arg;
	void *cb_data;

	if (!p_hdcp || !p_hdcp->bind_hdcptx_comm) {
		HDMITX_ERROR("%s NULL p_hdcp instance\n", __func__);
		return -EINVAL;
	}

	cb_data = p_hdcp->bind_hdcptx_comm->tx_hdcp_cb.data;
	...
	case TEE_HDCP_IOC_START:
		...
		if (get_hdcptx_lstore(p_hdcp->bind_hdcptx_comm, HDCP_VER_HDCP1X))
		...
		if (p_hdcp->bind_hdcptx_comm->hdcp_ctl_lvl > 0) {
			if (p_hdcp->bind_hdcptx_comm->tx_hdcp_cb.hdcp_notify) {
				p_hdcp->bind_hdcptx_comm->tx_hdcp_cb.hdcp_notify(cb_data,
					HDCP_KEY_UPDATE, HDCP_AUTH_UNKNOWN);
				HDMITX_DEBUG("notify hdcp key load done to drm\n");
			}
			return ret;
		}
		if (p_hdcp->bind_hdcptx_comm->hpd_state == 1 &&
			p_hdcp->bind_hdcptx_comm->ready &&
			hdmitx21_get_hdcptx_mode(p_hdcp) == 0) {
			HDMITX_HDCP_INFO("hdmi ready but hdcp not enabled, enable now\n");
			if (hdcptx_need_ctrl_by_upstream(p_hdcp->bind_hdcptx_comm->hdcp_rpt_en)) {
				HDMITX_HDCP_INFO("currently hdcp should started by upstream\n");
			} else {
				if (hdcp_key_store & BIT(1)) {
					hdcprx_ver =
						hdcptx_get_hdcprx_ver(p_hdcp->bind_hdcptx_comm);
					p_hdcp->bind_hdcptx_comm->dw_hdcp22_cap =
						(hdcprx_ver == 2) ? true : false;
				}
				hdmitx21_enable_hdcp(p_hdcp);
			}
		}
	...
	case TEE_HDCP_IOC_VALIDATE_KEY:
		...
		if (hdcp_key == 14 || hdcp_key == 1)
			key_valid = hdmitx_validate_hdcp_key(p_hdcp->bind_hdcptx_comm, 0x1);
		else if (hdcp_key == 22 || hdcp_key == 2)
			key_valid = hdmitx_validate_hdcp_key(p_hdcp->bind_hdcptx_comm, 0x2);
		else
			key_valid = 0;
...
static void hdmitx21_hdcp_uninit(void *hdcptx_instance);
static struct hdcptx_api hdcptx_api_init = {
	.get_hdcptx_lstore = get_hdcptx_lstore,
	.get_hdcptx_result = get_hdcptx_result,
	.hdcptx22_topo_ctrl = hdcptx22_topo_ctrl,
	.hdcptx_reset_param = hdcptx_reset_param,
	.hdcptx_enable = hdcptx_enable,
	.hdcptx_disable = hdmitx21_disable_hdcp,
	.get_hdcprx_ver = hdcptx_get_hdcprx_ver,
	.get_hdcptx_mode = hdmitx21_get_hdcptx_mode,
...
void *hdmitx21_hdcp_init(void *hdcp_data)
{
	struct hdcptx21_core *p_hdcp = NULL;
	struct hdcptx_common *hdcptx_comm = (struct hdcptx_common *)hdcp_data;
	...
	p_hdcp->bind_hdcptx_comm = hdcptx_comm;
	p_hdcp->hdcp_state = HDCP_STAT_NONE;
	p_hdcp->hdcp_type = HDCP_VER_NONE;
	p_hdcp->hdcp_cap_ds = HDCP_VER_NONE;
	p_hdcp->ds_auth = false;
	...
	p_hdcp->bind_hdcptx_comm->not_restart_hdcp = false;
	/* wait for upstream start hdcp auth 5S */
	p_hdcp->up_hdcp_timeout_sec = 5;
	p_hdcp->bind_hdcptx_comm->hdcptx_api = &hdcptx_api_init;
	...
	INIT_DELAYED_WORK(&p_hdcp->work_start_hdcp, hdmitx_start_hdcp_handler);
	INIT_DELAYED_WORK(&p_hdcp->work_up_hdcp_timeout, hdmitx_up_hdcp_timeout_handler);
	mutex_init(&p_hdcp->stream_type_mutex);

	tee_comm_dev_reg(&p_hdcp->hdcp_misc_dev, &hdcp_comm_file_operations);
...
static void hdmitx21_hdcp_uninit(void *hdcptx_instance)
{
	...
	if (!p_hdcp || !p_hdcp->bind_hdcptx_comm) {
		HDMITX_ERROR("NULL p_hdcp or NULL hdcptx_comm instance\n");
		return;
	}
	tee_comm_dev_unreg(&p_hdcp->hdcp_misc_dev);
	hdmitx21_disable_hdcp(p_hdcp);
