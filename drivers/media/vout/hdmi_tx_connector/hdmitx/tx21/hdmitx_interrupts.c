...
void hdmitx_top_intr_handler(struct work_struct *work)
{
	...
	if (pint->st_data) {
		...
		if (!strcmp(disablehpd, "true")) {
			...
			hdev->tx_comm.last_hpd_handle_done_stat |= HDMI_TX_HPD_PLUGIN;
		...
		if (dat_top & (1 << 1)) {
			...
			if (!ret) {
				HDMITX_INFO("too much plugin, send HDMITX_LINK_UNSTABLE uevent\n");
		...
		if (dat_top & (1 << 2)) {
			...
			if (!ret) {
				HDMITX_INFO("too much plugout, send HDMITX_LINK_UNSTABLE uevent\n");
				hdmitx_set_uevent(tx_comm, HDMITX_LINK_UNSTABLE, 1);
			}
		}
	}
...
int hdmitx_setup_irqs(struct hdmitx_hw_common *tx_hw)
{
	...
	if (!strcmp(disablehpd, "true")) {
		HDMITX_INFO(HW "disablehpd = %s\n", disablehpd);
		phdev->tx_comm.last_hpd_handle_done_stat |= HDMI_TX_HPD_PLUGIN;
		phdev->tx_comm.last_hpd_handle_done_stat &= ~HDMI_TX_HPD_PLUGOUT;
		queue_delayed_work(phdev->tx_comm.hdmi_hpd_wq,
			&phdev->tx_comm.work_hpd_plugin, HZ / 3);
	}
#endif
