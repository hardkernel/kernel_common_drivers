...
static void dump_dnlp_table(int value, int index)
{
	...
	pr_inf(lev_mod, "dump dnlp value:%d index:%d start\n", value, index);
	if (index == 255)
		return;

	for (i = 0; i < VPQ_DNLP_CURVE_LEN; i++) {
		pr_inf(lev_mod, "scurv_low/mid1/mid2/hgh1/hgh2[%d]:%d %d %d %d %d\n", i,
...
static void dump_aipq_table(int index)
{
	...
	pr_inf(lev_mod, "dump aipq index:%d start\n", index);
	if (index == 255)
		return;

	pr_inf(lev_mod, "height/width:%d %d\n",
	...
	for (i = 0; i < AIPQ_ROWS_MAX; i++) {
		for (j = 0; j < AIPQ_COLS_MAX; j++) {
			pr_inf(lev_mod, "aipq_table[%d][%d]:%d\n", i, j,
				pq_table_param.aipq_table[index].aipq_table[i][j]);
		}
	}

	pr_inf(lev_mod, "dump aipq end\n");
}
...
static void dump_lc_table(int value, int index)
{
	...
	pr_inf(lev_mod, "dump lc value:%d index:%d start\n", value, index);
	if (index == 255)
		return;

	for (i = 0; i < 63; i++)
		pr_inf(lev_mod, "lc_saturation[%d]:%d\n", i,
			pq_table_param.lc_table[index][value].curve.lc_saturation[i]);
	for (i = 0; i < 16; i++) {
		pr_inf(lev_mod, "lc_yminval/ypkbv_ymaxval/ymaxval/ypkbv_lmt[%d]:%d %d %d %d\n", i,
...
static void dump_ccoring_table(int value, int index)
{
	...
	pr_inf(lev_mod, "dump chroma coring value:%d index:%d start\n", value, index);
	if (index == 255)
		return;

	for (i = 0; i < VPQ_CCORING_MAX; i++)
		pr_inf(lev_mod, "param[%d]:%d\n", i,
			pq_table_param.ccoring_table[index][value].param.param[i]);

	pr_inf(lev_mod, "dump chroma coring end\n");
}
...
static void dump_blackext_table(int value, int index)
{
	...
	pr_inf(lev_mod, "dump blackext value:%d index:%d start\n", value, index);
	if (index == 255)
		return;
	...
	pr_inf(lev_mod, "dump blackext end\n");
}
...
static void dump_blue_stretch_table(int value, int index)
{
	...
	pr_inf(lev_mod, "dump blue stretch value:%d index:%d start\n", value, index);
	if (index == 255)
		return;
	...
	pr_inf(lev_mod, "dump blue stretch end\n");
}
...
static void dump_cm_table(int value, int index)
{
	...
	pr_inf(lev_mod, "dump cm value:%d index:%d start\n", value, index);
	if (index == 255)
		return;

	for (i = 0; i < 9; i++)
		pr_inf(lev_mod, "sat_by_y_0[%d]:%d\n", i,
			pq_table_param.cm_base_table[index][value].sat_by_y_0[i]);
	for (i = 0; i < 32; i++)
		pr_inf(lev_mod, "luma_by_hue_0[%d]:%d\n", i,
			pq_table_param.cm_base_table[index][value].luma_by_hue_0[i]);
	for (i = 0; i < 32; i++) {
		pr_inf(lev_mod, "sat_by_hs_0/1/2[%d]:%d %d %d\n", i,
	...
	for (i = 0; i < 32; i++)
		pr_inf(lev_mod, "hue_by_hue_0[%d]:%d\n", i,
			pq_table_param.cm_base_table[index][value].hue_by_hue_0[i]);
	for (i = 0; i < 32; i++) {
		pr_inf(lev_mod, "hue_by_hy_0/1/2/3/4[%d]:%d %d %d %d %d\n", i,
	...
	for (i = 0; i < 32; i++) {
		pr_inf(lev_mod, "hue_by_hs_0/1/2/3/4[%d]:%d %d %d %d %d\n", i,
	...
	for (i = 0; i < 32; i++) {
		pr_inf(lev_mod, "sat_by_hy_0/1/2/3/4[%d]:%d %d %d %d %d\n", i,
...
static void dump_nr_table(int value, int index)
{
	...
	pr_inf(lev_mod, "dump nr value:%d index:%d start\n", value, index);
	if (index == 255)
		return;
...
static void dump_deblock_table(int value, int index)
{
	...
	pr_inf(lev_mod, "dump deblock value:%d index:%d start\n", value, index);
	if (index == 255)
		return;
	...
	pr_inf(lev_mod, "dump deblock end\n");
}
...
static void dump_demosquito_table(int value, int index)
{
	...
	pr_inf(lev_mod, "dump demosquito value:%d index:%d start\n", value, index);
	if (index == 255)
		return;
	...
	pr_inf(lev_mod, "dump demosquito end\n");
}
...
static void dump_decontour_table(int value, int index)
{
	...
	pr_inf(lev_mod, "dump decontour value:%d index:%d start\n", value, index);
	if (index == 255)
		return;
	...
	pr_inf(lev_mod, "dump decontour end\n");
}
...
static void dump_di_table(int index)
{
	...
	pr_inf(lev_mod, "dump di index:%d start\n", index);
	if (index == 255)
		return;

	for (i = 0; i < VPQ_REG_XLR_MAX; i++)
		pr_inf(lev_mod, "di xlr param[%d]:%d\n", i,
			pq_table_param.di_table[index].param.param[i]);

	pr_inf(lev_mod, "dump di end\n");
}
...
int vpq_module_dump_pq_table(int module)
{
	...
	case 24:
		...
		break;
	default:
		pr_inf(lev_mod, "unknown module:%d, skip dump table\n", module);
		break;
	}
