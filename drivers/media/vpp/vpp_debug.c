...
ssize_t vpp_debug_show(const struct class *class,
	...
	char *buf)
{
	PR_DRV("Current dbg_lvl value = 0x%x\n", pr_lvl);
	PR_DRV("--> 0x1: PR_SYS\n");
	PR_DRV("--> 0x2: PR_IOC\n");
	PR_DRV("--> 0x4: PR_DEBUG\n");
	PR_DRV("--> 0x8: PR_DEBUG_CM\n");
	PR_DRV("--> 0x10: PR_DEBUG_DNLP\n");
	PR_DRV("--> 0x20: PR_DEBUG_GAMMA\n");
	PR_DRV("--> 0x40: PR_DEBUG_GO\n");
	PR_DRV("--> 0x80: PR_DEBUG_HDR\n");
	PR_DRV("--> 0x100: PR_DEBUG_LC\n");
	PR_DRV("--> 0x200: PR_DEBUG_LUT3D\n");
	PR_DRV("--> 0x400: PR_DEBUG_MATRIX\n");
	PR_DRV("--> 0x800: PR_DEBUG_METER\n");
	PR_DRV("--> 0x1000: PR_DEBUG_SR\n");
	PR_DRV("--> 0x2000: PR_DEBUG_VADJ\n");
	PR_DRV("--> 0x4000: PR_DEBUG_VE\n");
	PR_DRV("--> 0x8000: PR_DEBUG_AI\n");
	PR_DRV("Current alg_dbg_lvl value = 0x%x\n", pr_lvl_alg);
	PR_DRV("--> 0x1: PR_ALG_DNLP\n");
	...
	const char *buf, size_t count)
{
	...
	if (!strncmp(param[0], "dbg_lvl", 7)) {
		if (kstrtouint(param[1], 16, &pr_lvl) < 0)
			goto fr_bf;

		PR_DRV("pr_lvl = 0x%x\n", pr_lvl);
	} else if (!strncmp(param[0], "alg_dbg_lvl", 11)) {
		if (kstrtouint(param[1], 16, &pr_lvl_alg) < 0)
			goto fr_bf;

		PR_DRV("pr_lvl_alg = 0x%x\n", pr_lvl_alg);
	} else if (!strncmp(param[0], "dbg_bypass_top_set", 18)) {
		if (kstrtouint(param[1], 10, &val) < 0)
			goto fr_bf;

		vpp_pq_mgr_get_settings()->bypass_top_set = val;
		PR_DRV("bypass_top_set = %d\n", val);
	} else if (!strncmp(param[0], "dbg_restore_default", 19)) {
		vpp_pq_mgr_set_default_settings();
	} else if (!strncmp(param[0], "dbg_dump_mgr", 12)) {
		_dump_pq_mgr_settings();
	} else if (!strncmp(param[0], "dbg_dump_reg", 12)) {
		if (kstrtouint(param[1], 10, &val) < 0)
			goto fr_bf;
	...
	} else if (!strncmp(param[0], "dbg_dump_data", 13)) {
		if (kstrtouint(param[1], 10, &val) < 0)
			goto fr_bf;
	...
	const char *buf, size_t count)
{
	...
	if (!strncmp(param[0], "pre_gamma", 9)) {
	...
	} else if (!strncmp(param[0], "lcd_gamma", 9)) {
	...
	if (param[0][1] == 'g') {
		if (param[0][0] == 's') {
			...
			if (!strncmp(param[1], "all", 3)) {
				...
				for (i = 0; i < len; i++)
					_d_convert_str(ptmp[i], i, stemp, 3, 16);
				PR_DRV("[%s] gamma_%d str: %s\n",
					__func__, param[0][2], stemp);
	...
	char *buf)
{
	...
	PR_DRV("--> return value_0~8 including *_pre_offset/*_gain/*_post_offset\n");
	PR_DRV("--> * means r/g/b\n");
	...
	const char *buf, size_t count)
{
	...
	if (!strncmp(param[0], "wb_en", 5)) {
		vpp_module_go_en((bool)val, EN_MODE_DIR);
...
ssize_t vpp_debug_dnlp_show(const struct class *class,
	...
	char *buf)
{
	if (vpp_alg_fw_dnlp_get_insmod_status())
		PR_DRV("[%s] DNLP insmod ok.\n", __func__);
	else
		PR_DRV("[%s] DNLP no insmod.\n", __func__);

	PR_DRV("echo r param all > /sys/class/aml_vpp/vpp_dnlp\n");
	PR_DRV("echo r str_xxx > /sys/class/aml_vpp/vpp_dnlp\n");
	PR_DRV("--> read one param...str_xxx is string.\n");

	PR_DRV("echo w str_xxx val_xxx > /sys/class/aml_vpp/vpp_dnlp\n");
	PR_DRV("--> val_xxx is dec value.\n");

	PR_DRV("echo rc str_xxx all > /sys/class/aml_vpp/vpp_dnlp\n");
	PR_DRV("echo rc str_xxx str_all > /sys/class/aml_vpp/vpp_dnlp\n");

	PR_DRV("echo wc str_xxx str_all str_val_xxx > /sys/class/aml_vpp/vpp_dnlp\n");
	PR_DRV("echo wc str_xxx idx_xxx val_xxx > /sys/class/aml_vpp/vpp_dnlp\n");

	PR_DRV("echo ro str_xxx > /sys/class/aml_vpp/vpp_dnlp\n");
	...
	const char *buf, size_t count)
{
	...
	char *param[8];
	char *stemp = NULL;
	struct dnlp_alg_dbg_parse_cmd_s *pdnlp_dbg_parse_cmd;
	int *curve_val;
	...
	pdnlp_dbg_parse_cmd = vpp_alg_fw_dnlp_get_dbg_info();

	if (!strncmp(param[0], "r", 1)) {/*read param*/
		if (!strncmp(param[1], "param", 5)) {
			if (!strncmp(param[2], "all", 3)) {
	...
	} else if (!strncmp(param[0], "w", 1)) {/*write param*/
		for (i = 0; pdnlp_dbg_parse_cmd[i].value; i++) {
			if (!strcmp(param[1], pdnlp_dbg_parse_cmd[i].parse_string)) {
	...
	} else if (!strncmp(param[0], "rc", 2)) {/*read curve*/
		...
		pdnlp_dbg_parse_cmd = vpp_alg_fw_dnlp_get_dbg_cv_info();
		...
		for (i = 0; pdnlp_dbg_parse_cmd[i].value; i++) {
			if (!strcmp(param[1], pdnlp_dbg_parse_cmd[i].parse_string)) {
				...
				if (!param[2]) {
					...
					kfree(stemp);
					goto free_buf;
				} else if (strncmp(param[2], "all", 3)) {
					PR_DRV("[%s] %s\n", __func__,
						pdnlp_dbg_parse_cmd[i].parse_string);
					for (j = 0; j < data_len; j++)
						PR_DRV("[%d] %d\n",
							j, pdnlp_dbg_parse_cmd[i].value[j]);
				} else if (strncmp(param[2], "str_all", 7)) {
					for (j = 0; j < data_len; j++)
						_d_convert_str(pdnlp_dbg_parse_cmd[i].value[j],
							j, stemp, 4, 10);

					PR_DRV("[%s] %s\n",
						pdnlp_dbg_parse_cmd[i].parse_string, stemp);
				} else {
					PR_DRV("[%s] error cmd\n", __func__);
				}
			} else {
				PR_DRV("[%s] no match rc string.\n", __func__);
			}
		}

		kfree(stemp);
	} else if (!strncmp(param[0], "wc", 2)) {/*write curve*/
		pdnlp_dbg_parse_cmd = vpp_alg_fw_dnlp_get_dbg_cv_info();
		...
		for (i = 0; pdnlp_dbg_parse_cmd[i].value; i++) {
			if (!strcmp(param[1], pdnlp_dbg_parse_cmd[i].parse_string)) {
				...
				if (!param[2]) {
					...
					curve_val = kmalloc(data_len * sizeof(int), GFP_KERNEL);
					if (!curve_val) {
						PR_DRV("[%s] error buffer malloc.\n", __func__);
						goto free_buf;
					}

					_str_sapr_to_d(param[3], curve_val, data_len);
					...
					PR_DRV("[%s] set point %d to %ld\n",
						pdnlp_dbg_parse_cmd[i].parse_string,
						j, val);
				}
			}
		}
	} else if (!strncmp(param[0], "ro", 2)) {
		pdnlp_dbg_parse_cmd = vpp_alg_fw_dnlp_get_dbg_ro_info();
	...
	char *buf)
{
	...
	if (lc_dbg_rd_flag & LC_DBG_RD_UPDATE_CV) {
		...
		for (i = 0; i < len; i++)
			_d_convert_str(lut_data[i], i, stemp, 4, 10);
	...
	const char *buf, size_t count)
{
	...
	if (!strncmp(param[0], "lc_ctrl", 7)) {
		if (!strncmp(param[1], "enable", 6))
			vpp_module_lc_en(true);
		else if (!strncmp(param[1], "disable", 7))
			vpp_module_lc_en(false);
	} else if (!strncmp(param[0], "lc_demo_mode", 12)) {
		if (!strncmp(param[1], "enable", 6))
			vpp_module_lc_set_demo_mode(true, true);
		else if (!strncmp(param[1], "disable", 7))
			vpp_module_lc_set_demo_mode(false, true);
	} else if (!strncmp(param[0], "lc_curve_isr", 12)) {
		if (kstrtoul(param[1], 10, &val) < 0)
			goto free_buf;
		vpp_module_lc_set_isr_use(val);
	} else if (!strncmp(param[0], "lc_wr_lut", 9)) {
	...
	} else if (!strncmp(param[0], "dump_lut_data", 13)) {
		if (kstrtoul(param[1], 16, &val) < 0)
			goto free_buf;
		lc_lut_type = val;
		if (lc_lut_type > EN_LC_MAX)
		...
		for (i = 0; i < len; i++)
			PR_DRV("lut[%d] = %4d.\n", i, lut_data[i]);
	} else if (!strncmp(param[0], "get_blk_region", 14)) {
		if (!strncmp(param[1], "htotal", 6)) {
			lc_dbg_rd_param = vpp_module_lc_get_cfg_param(EN_LC_BLK_HNUM);
			lc_dbg_rd_flag |= LC_DBG_RD_UPDATE_PARAM;
		} else if (!strncmp(param[1], "vtotal", 6)) {
			lc_dbg_rd_param = vpp_module_lc_get_cfg_param(EN_LC_BLK_VNUM);
			lc_dbg_rd_flag |= LC_DBG_RD_UPDATE_PARAM;
		} else {
			PR_DRV("[%s] error cmd\n", __func__);
		}
	} else if (!strncmp(param[0], "dump_lut_str", 12)) {
		if (kstrtoul(param[1], 16, &val) < 0)
			goto free_buf;
		lc_lut_type = val;
		if (lc_lut_type > EN_LC_MAX)
		...
		lc_dbg_rd_flag |= LC_DBG_RD_UPDATE_CV;
	} else if (!strncmp(param[0], "lc_osd_setting", 14)) {
		if (!strncmp(param[1], "show", 4)) {
	...
	} else if (!strncmp(param[0], "osd_iir_en", 10)) {
		if (kstrtoul(param[1], 10, &val) < 0)
			goto free_buf;
		vpp_module_lc_set_alg_param(EN_LC_OSD_IIR_EN, val);
	} else if (!strncmp(param[0], "iir_refresh", 11)) {
		if (!strncmp(param[1], "show", 4)) {
	...
	} else if (!strncmp(param[0], "scene_change_th", 15)) {
	...
	} else if (!strncmp(param[0], "detect_signal_range_en", 22)) {
		if (kstrtoul(param[1], 10, &val) < 0)
			goto free_buf;
		vpp_module_lc_set_alg_param(EN_LC_DET_RANGE_MODE, val);
	} else if (!strncmp(param[0], "detect_signal_range_threshold", 29)) {
		if (kstrtoul(param[1], 10, &val) < 0)
			goto free_buf;
	...
	} else if (!strncmp(param[0], "set_alg_param", 13)) {
		if (kstrtoul(param[1], 10, &val) < 0)
			goto free_buf;
		lc_alg_type = val;
	...
	} else if (!strncmp(param[0], "set_cfg_param", 13)) {
		if (kstrtoul(param[1], 10, &val) < 0)
			goto free_buf;
		lc_cfg_type = val;
	...
	} else if (!strncmp(param[0], "set_tune_param", 14)) {
		if (kstrtoul(param[1], 10, &val) < 0)
			goto free_buf;
		lc_tune_type = val;
	...
	} else if (!strncmp(param[0], "dump_alg_param", 14)) {
		if (kstrtoul(param[1], 10, &val) < 0)
			goto free_buf;
		lc_alg_type = val;
		if (lc_alg_type > EN_LC_ALG_PARAM_MAX)
	...
	} else if (!strncmp(param[0], "dump_cfg_param", 14)) {
		if (kstrtoul(param[1], 10, &val) < 0)
			goto free_buf;
		lc_cfg_type = val;
		if (lc_cfg_type > EN_LC_CFG_PARAM_MAX)
	...
	} else if (!strncmp(param[0], "dump_tune_param", 15)) {
		if (kstrtoul(param[1], 10, &val) < 0)
			goto free_buf;
		lc_tune_type = val;
		if (lc_tune_type > EN_LC_CURVE_TUNE_PARAM_MAX)
	...
	} else if (!strncmp(param[0], "set_lc_param", 12)) {
		if (kstrtoul(param[1], 10, &val) < 0)
			goto free_buf;
	...
	char *buf)
{
	...
	PR_DRV("0: None, 1: SDR, 2: HDR10, 3: HLG,\n");
	PR_DRV("4: HDR10+, 5: CUVA, 6: CUVA_HLG\n");
	...
	char *buf)
{
	...
	PR_DRV("0: None, 1: BT601, 2: BT709, 3: BT2020\n");
...
ssize_t vpp_debug_histogram_show(const struct class *class,
	...
	char *buf)
{
	struct vpp_hist_bin_s *pdata = NULL;
	unsigned int buf_size;
	int i = 0;

	buf_size = sizeof(struct vpp_hist_bin_s);
	...
	for (i = 0; i < VPP_HIST_BIN_COUNT; i++) {
		pr_info("[%d]0x%-8x\t", i, pdata->hist[i]);
		if ((i + 1) % 8 == 0)
			pr_info("\n");
	}
	pr_info("\n\t dump_dnlp_hist done\n");
	...
	for (i = 0; i < VPP_COLOR_HIST_BIN_COUNT; i++) {
		pr_info("[%d]0x%-8x\t", i, pdata->hue_hist[i]);
		if ((i + 1) % 8 == 0)
			pr_info("\n");
	}
	pr_info("\n\t dump_hue_hist done\n");
	...
	pr_info("\n\t dump_sat_hist begin\n");
	for (i = 0; i < VPP_COLOR_HIST_BIN_COUNT; i++) {
		pr_info("[%d]0x%-8x\t", i, pdata->sat_hist[i]);
		if ((i + 1) % 8 == 0)
			pr_info("\n");
	}
	pr_info("\n\t dump_sat_hist done\n");
	...
	if (vpp_module_meter_get_dark_hist_support()) {
		...
		for (i = 0; i < VPP_HIST_BIN_COUNT; i++) {
			pr_info("[%d]0x%-8x\t", i, pdata->dark_hist[i]);
			if ((i + 1) % 8 == 0)
				pr_info("\n");
		}
		pr_info("\n\t dump_dark_hist done\n");
...
ssize_t vpp_debug_histogram_hdr_show(const struct class *class,
	...
	char *buf)
{
	struct vpp_hdr_hist_param_s *pdata = NULL;
	unsigned int buf_size;
	int i = 0;

	buf_size = sizeof(struct vpp_hdr_hist_param_s);
	pdata = kmalloc(buf_size, GFP_KERNEL);
	if (!pdata)
		return 0;
	...
	const char *buf, size_t count)
{
	...
	struct vpp_mtrx_param_s *data = NULL;
	unsigned int buf_size;
	int i = 0;

	if (likely(_parse_param_int(buf, 17, parsed) != 17)) {
		PR_DRV("settings data wrong.\n");
		return -EINVAL;
	}
	...
	for (i = 0; i < VPP_MTRX_OFFSET_LEN; i++) {
		data->pre_offset[i] = parsed[1 + i];
		data->post_offset[i] = parsed[13 + i];
	}

	for (i = 0; i < VPP_MTRX_COEF_LEN; i++)
		data->matrix_coef[i] = parsed[4 + i];

	data->right_shift = parsed[16];

	vpp_module_matrix_rs(mode, data->right_shift);
	vpp_module_matrix_set_pre_offset(mode, &data->pre_offset[0]);
	vpp_module_matrix_set_offset(mode, &data->post_offset[0]);
	vpp_module_matrix_set_coef_3x3(mode, &data->matrix_coef[0]);
	...
	const char *buf, size_t count)
{
	...
	if (!strncmp(param[0], "dump_cm2_curve", 14)) {
		if (kstrtoul(param[1], 10, &val) < 0)
			goto free_buf;
...
ssize_t vpp_debug_blkext_show(const struct class *class,
	...
	char *buf)
{
	PR_DRV("echo value0~3 > /sys/class/aml_vpp/vpp_blkext\n");
	PR_DRV("--> value0: start, value1: slope1\n");
	PR_DRV("--> value2: mid_point, value3: slope2\n");
...
{
	int parsed[4] = {0};
	struct vpp_blkext_param_s *data = NULL;
	unsigned int buf_size;
	int i = 0;
	...
	PR_DRV("[%s] set param = 0x%x/%x/%x/%x\n",
		__func__, parsed[0], parsed[1], parsed[2], parsed[3]);

	buf_size = sizeof(struct vpp_blkext_param_s);
	data = kmalloc(buf_size, GFP_KERNEL);
	if (!data)
		return count;

	for (i = 0; i < EN_BLKEXT_MAX; i++)
		data->param[i] = parsed[i];
...
ssize_t vpp_debug_ccoring_show(const struct class *class,
	...
	char *buf)
{
	PR_DRV("echo value0~2 > /sys/class/aml_vpp/vpp_ccoring\n");
	PR_DRV("--> value0: slope, value1: th, value2: bypass_yth\n");
...
{
	int parsed[3] = {0};
	struct vpp_cc_param_s *data = NULL;
	unsigned int buf_size;
	int i = 0;
	...
	PR_DRV("[%s] set param = 0x%x/%x/%x\n",
		__func__, parsed[0], parsed[1], parsed[2]);

	buf_size = sizeof(struct vpp_cc_param_s);
	data = kmalloc(buf_size, GFP_KERNEL);
	if (!data)
		return count;
...
ssize_t vpp_debug_blue_stretch_show(const struct class *class,
	...
	char *buf)
{
	PR_DRV("echo value0~17 > /sys/class/aml_vpp/vpp_blue_stretch\n");
	PR_DRV("--> value0: en_sel, value1: cb_inc\n");
	PR_DRV("--> value2: cr_inc, value3: gain\n");
	PR_DRV("--> value4: gain_cb4cr, value5: luma_high\n");
	PR_DRV("--> value6: err_crp, value7: err_sign3\n");
	PR_DRV("--> value8: err_crp_inv, value9: err_crn\n");
	PR_DRV("--> value10: err_sign2, value11: err_crn_inv\n");
	PR_DRV("--> value12: err_cbp, value13: err_sign1\n");
	PR_DRV("--> value14: err_cbp_inv, value15: err_cbn\n");
	PR_DRV("--> value16: err_sign0, value17: err_cbn_inv\n");
...
{
	int parsed[18] = {0};
	struct vpp_bs_param_s *data = NULL;
	unsigned int buf_size;
	int i = 0;

	if (likely(_parse_param_int(buf, 18, parsed) != 18)) {
		PR_DRV("settings data wrong.\n");
		return -EINVAL;
	}
	...
	buf_size = sizeof(struct vpp_bs_param_s);
	data = kmalloc(buf_size, GFP_KERNEL);
	if (!data)
		return count;
...
ssize_t vpp_debug_lut3d_show(const struct class *class,
	...
	char *buf)
{
	PR_DRV("echo pattern enable > /sys/class/aml_vpp/vpp_lut3d\n");
	PR_DRV("echo pattern disable > /sys/class/aml_vpp/vpp_lut3d\n");
	PR_DRV("echo pattern val_r val_g val_b > /sys/class/aml_vpp/vpp_lut3d\n");
	PR_DRV("--> val_r/g/b: 0~255\n");
	PR_DRV("echo mount > /sys/class/aml_vpp/vpp_lut3d\n");
	PR_DRV("echo demount > /sys/class/aml_vpp/vpp_lut3d\n");
	...
	const char *buf, size_t count)
{
	...
	if (!vpp_module_lut3d_get_support()) {
		PR_DRV("No support 3dlut.\n");
		return count;
	}
	...
	if (!strncmp(param[0], "pattern", 7)) {
		if (!strncmp(param[1], "enable", 6)) {
			vpp_module_lut3d_pattern_en(true);
			PR_DRV("[lut3d] test pattern on.\n");
		} else if (!strncmp(param[1], "disable", 7)) {
			vpp_module_lut3d_pattern_en(false);
			PR_DRV("[lut3d] test pattern off.\n");
		} else {
			if (kstrtouint(param[1], 10, &val[0]) < 0)
				goto free_buf;

			if (kstrtouint(param[2], 10, &val[1]) < 0)
				goto free_buf;

			if (kstrtouint(param[3], 10, &val[2]) < 0)
				goto free_buf;

			vpp_module_lut3d_pattern_set(val[0], val[1], val[2]);
			PR_DRV("[lut3d] set test pattern %d/%d/%d.\n",
				val[0], val[1], val[2]);
		}
	} else if (!strncmp(param[0], "mount", 5)) {
		vpp_module_lut3d_mount();
	} else if (!strncmp(param[0], "demount", 7)) {
		vpp_module_lut3d_demount();
	}
...
ssize_t vpp_debug_data_show(const struct class *class,
	...
	char *buf)
{
	PR_DRV("echo value_type > /sys/class/aml_vpp/vpp_data\n");
	PR_DRV("--> 0: lc\n");
	...
	const char *buf, size_t count)
{
	...
	val = vpp_check_range(val,
		EN_DUMP_DATA_LC, EN_DUMP_DATA_VS_INFO);
	PR_DRV("[%s] dump type = %d\n", __func__, val);

	vpp_data_dump_info(val);
