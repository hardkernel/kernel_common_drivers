...
const struct class_attribute vpp_class_attr[] = {
	...
	__ATTR(vpp_white_balance, 0644,
		vpp_debug_white_balance_show,
		vpp_debug_white_balance_store),
	...
	__ATTR(vpp_blkext, 0644,
		vpp_debug_blkext_show,
		vpp_debug_blkext_store),
	__ATTR(vpp_ccoring, 0644,
		vpp_debug_ccoring_show,
		vpp_debug_ccoring_store),
	__ATTR(vpp_blue_stretch, 0644,
		vpp_debug_blue_stretch_show,
		vpp_debug_blue_stretch_store),
	__ATTR(vpp_lut3d, 0644,
		vpp_debug_lut3d_show,
		vpp_debug_lut3d_store),
	__ATTR(vpp_data, 0644,
		vpp_debug_data_show,
		vpp_debug_data_store),
	__ATTR_NULL,
};
...
const struct match_data_s vpp_t5m_match = {
	.chip_id = CHIP_T5M,
};
	...
	unsigned int cmd, unsigned long arg)
{
	...
	struct vpp_mtrx_info_s *mtrx_data  = NULL;
	struct vpp_module_ctrl_s module_status;
	struct vpp_dnlp_curve_param_s *dnlp_data = NULL;
	struct vpp_lc_curve_s *lc_curve_data = NULL;
	struct vpp_lc_param_s *lc_param_data = NULL;
	...
	struct vpp_hist_ave_s hist_ave;
	struct vpp_table_s overscan_table;
	struct vpp_hist_bin_s *hist_data = NULL;
	struct vpp_hdr_hist_param_s *hist_data_hdr = NULL;
	struct vpp_color_primary_s *color_pri_data = NULL;
	...
	memset(&hist_ave, 0, sizeof(struct vpp_hist_ave_s));
	...
	case VPP_IOC_SET_MATRIX_PARAM:
		...
		mtrx_data = kmalloc(buffer_size, GFP_KERNEL);
		if (!mtrx_data) {
			ret = -ENOMEM;
		} else {
			if (copy_from_user(mtrx_data, argp, buffer_size))
				ret = -EFAULT;
			else
				ret = vpp_pq_mgr_set_matrix_param(mtrx_data);
			kfree(mtrx_data);
		}
		break;
	...
	case VPP_IOC_SET_DNLP_PARAM:
		...
		dnlp_data = kmalloc(buffer_size, GFP_KERNEL);
		if (!dnlp_data) {
			ret = -ENOMEM;
		} else {
			if (copy_from_user(dnlp_data, argp, buffer_size))
				ret = -EFAULT;
			else
				ret = vpp_pq_mgr_set_dnlp_param(dnlp_data);
			kfree(dnlp_data);
		}
		break;
	case VPP_IOC_SET_LC_CURVE:
		buffer_size = sizeof(struct vpp_lc_curve_s);
		lc_curve_data = kmalloc(buffer_size, GFP_KERNEL);
		if (!lc_curve_data) {
			ret = -ENOMEM;
		} else {
			if (copy_from_user(lc_curve_data, argp, buffer_size))
				ret = -EFAULT;
			else
				ret = vpp_pq_mgr_set_lc_curve(lc_curve_data);
			kfree(lc_curve_data);
		}
		break;
	case VPP_IOC_SET_LC_PARAM:
		buffer_size = sizeof(struct vpp_lc_param_s);
		lc_param_data = kmalloc(buffer_size, GFP_KERNEL);
		if (!lc_param_data) {
			ret = -ENOMEM;
		} else {
			if (copy_from_user(lc_param_data, argp, buffer_size))
				ret = -EFAULT;
			else
				ret = vpp_pq_mgr_set_lc_param(lc_param_data);
			kfree(lc_param_data);
		}
		break;
	...
	case VPP_IOC_SET_COLOR_PRIMARY:
		...
		color_pri_data = kmalloc(buffer_size, GFP_KERNEL);
		if (!color_pri_data) {
			ret = -ENOMEM;
		} else {
			if (copy_from_user(color_pri_data, argp, buffer_size))
				ret = -EFAULT;
			else
				ret = vpp_pq_mgr_set_color_primary(color_pri_data);
			kfree(color_pri_data);
		}
		break;
	...
	case VPP_IOC_SET_3DLUT_DATA:
		if (copy_from_user(&lut3d_data, argp,
			...
			tmp_data = kmalloc(buffer_size, GFP_KERNEL);
			if (!tmp_data) {
				ret = -ENOMEM;
			} else {
				if (copy_from_user(tmp_data, argp, buffer_size)) {
					ret = -EFAULT;
				} else {
					lut3d_data.data = tmp_data;
		...
		if (copy_from_user(&hdr_lut_data, argp,
			...
			if (!tmp_data) {
				...
				if (copy_from_user(tmp_data, argp, buffer_size)) {
					...
					hdr_lut_data.lut_data = tmp_data;
	...
	case VPP_IOC_SET_CM_OFFSET_CURVE:
		if (copy_from_user(&cm_data, argp,
			...
			tmp_data_char = kmalloc(buffer_size, GFP_KERNEL);
			if (!tmp_data_char) {
				ret = -ENOMEM;
			} else {
				if (copy_from_user(tmp_data_char, argp, buffer_size))
					ret = -EFAULT;
				else
	...
	case VPP_IOC_SET_AIPQ_TABLE:
		...
		tmp_data_char[data_len] = '\0';
		if (strlen(tmp_data_char) != data_len) {
			kfree(tmp_data_char);
			break;
		}

		ret = vpp_pq_mgr_set_aipq_offset_table(tmp_data_char,
	...
	case VPP_IOC_GET_HIST_AVG:
		...
		if (copy_to_user(argp, &hist_ave,
			sizeof(struct vpp_hist_ave_s)))
			ret = -EFAULT;
		break;
	case VPP_IOC_GET_HIST_BIN:
		buffer_size = sizeof(struct vpp_hist_bin_s);
		hist_data = kmalloc(buffer_size, GFP_KERNEL);
		if (!hist_data) {
			ret = -ENOMEM;
		} else {
			vpp_pq_mgr_get_histogram(hist_data);
			if (copy_to_user(argp, hist_data, buffer_size))
				ret = -EFAULT;
			kfree(hist_data);
		}
		break;
	...
	case VPP_IOC_GET_HDR_HIST:
		buffer_size = sizeof(struct vpp_hdr_hist_param_s);
		hist_data_hdr = kmalloc(buffer_size, GFP_KERNEL);
		if (!hist_data_hdr) {
			ret = -ENOMEM;
		} else {
			vpp_pq_mgr_get_hdr_histogram(hist_data_hdr);
			if (copy_to_user(argp, hist_data_hdr, buffer_size))
				ret = -EFAULT;
			kfree(hist_data_hdr);
		}
		break;
	...
	pr_vpp(PR_IOC, "[vpp_pq_ioctl] cmd/ret = 0x%x/%d\n", cmd, ret);
...
static int vpp_dts_parse(struct vpp_dev_s *vpp_devp)
{
	...
	if (of_id) {
		PR_DRV("%s compatible\n", of_id->compatible);
		vpp_devp->m_data = (struct match_data_s *)of_id->data;
	}
...
static int vpp_drv_probe(struct platform_device *pdev)
{
	...
	vpp_devp->clsp = class_create(VPP_CLS_NAME);
	...
	if (vpp_devp->res_lc_irq) {
		if (request_irq(vpp_devp->res_lc_irq->start,
			...
			vpp_devp->lc_isr_defined = 1;
			PR_DRV("request res_lc_curve_irq successful\n");
		}
	} else {
		PR_DRV("no support res_lc_curve_irq\n");
	}
...
static void vpp_drv_remove(struct platform_device *pdev)
{
	int i;
...
{
//	PR_DRV("%s module init %s\n", __func__, VERSION_VPP_DRV);
