...
static struct _lc_csc_mtrx_coef_s mtrx_coef_tbl[] = {
	...
	/*YUV601L_RGB*/
	{{0x04a8, 0x0000, 0x0662,
	0x04a8, 0x1e70, 0x1cbf,
	0x04a8, 0x0812, 0x0000}},
	{{0x012a, 0x0000, 0x0198,
	0x012a, 0x0f9c, 0x0f30,
	0x012a, 0x0204, 0x0000}},
	/*RGB_YUV709L*/
	{{0x00bb, 0x0275, 0x003f,
	0x1f99, 0x1ea6, 0x01c2,
	0x01c2, 0x1e67, 0x1fd7}},
	/*YUV709L_RGB*/
	{{0x04a8, 0x0000, 0x072c,
	0x04a8, 0x1f26, 0x1ddd,
	0x04a8, 0x0876, 0x0000}},
	{{0x012a, 0x0000, 0x01cb,
	0x012a, 0x1fc9, 0x0f77,
	0x012a, 0x021d, 0x0000}},
	/*RGB_YUV709*/
	{{0x00da, 0x02dc, 0x004a,
	0x1f8a, 0x1e76, 0x0200,
	0x0200, 0x1e2f, 0x1fd1}},
	/*YUV709_RGB*/
	{{0x0400, 0x0000, 0x064d,
	0x0400, 0x1f41, 0x1e21,
	0x0400, 0x076d, 0x0000}},
	{{0x0100, 0x0000, 0x0193,
	0x0100, 0x1fd0, 0x1f88,
	0x0100, 0x01db, 0x0000}},
};

static struct _lc_csc_mtrx_ofst_s mtrx_ofst_tbl[] = {
	{{0x0, 0x0, 0x0, 0x0, 0x0, 0x0}},
	{{0x40, 0x200, 0x3ff, 0x0, 0x0, 0x0}},
	{{0x100, 0x800, 0xfff, 0x0, 0x0, 0x0}},
	{{0x0, 0x200, 0x3ff, 0x0, 0x0, 0x0}},
	{{0x0, 0x800, 0xfff, 0x0, 0x0, 0x0}},
	{{0x40, 0x200, 0x200, 0x0, 0x0, 0x0}},
	{{0x0, 0x0, 0x0, 0x7c0, 0x600, 0x600}},
	{{0x0, 0x0, 0x0, 0x0, 0x600, 0x600}},
};
...
static int pre_scene_change_flag;
...
static void _set_lc_curve_ymin_lmt(int *data)
{
...
static void _set_lc_curve_ypkbv_ymax_lmt(int *data)
{
	unsigned int i, tmp;
	unsigned int addr = 0;
	enum io_mode_e io_mode = EN_MODE_DIR;
...
static void _set_lc_curve_ymax_lmt(int *data)
{
	unsigned int i, j, tmp;
	unsigned int addr = 0;
	enum io_mode_e io_mode = EN_MODE_DIR;
...
static void _set_lc_curve_ypkbv_lmt(int *data)
{
	unsigned int i, tmp;
	unsigned int addr = 0;
	enum io_mode_e io_mode = EN_MODE_DIR;
...
static void _set_lc_curve_ypkbv_rat(int *data)
{
	unsigned int tmp;
	unsigned int addr = 0;
	...
	tmp = (data[0] & 0xff) << 24 | (data[1] & 0xff) << 16 |
		(data[2] & 0xff) << 8 | (data[3] & 0xff);

	WRITE_VPP_REG_BY_MODE(EN_MODE_DIR, addr, tmp);
}

static void _set_lc_curve_ypkbv_slp_lmt(int *data)
{
	unsigned int tmp;
	unsigned int addr = 0;
	...
	tmp = (data[0] & 0xff) << 8 | (data[1] & 0xff);
	WRITE_VPP_REG_BY_MODE(EN_MODE_DIR, addr, tmp);
}

static void _set_lc_curve_cntst_lmt_lh(int *data)
{
	unsigned int tmp;
	unsigned int addr = 0;
...
static void _set_lc_stts_mtrx(int *coef_data,
	int *offset_data)
{
	int i, idx;
	unsigned int addr = 0;
	...
	idx = LC_MTRX_SIZE >> 1;
	for (i = 0; i < idx; i++) {
		reg_data = (coef_data[2 * i] << 16) | coef_data[2 * i + 1];
		WRITE_VPP_REG_BY_MODE(io_mode, addr + i, reg_data);
	}

	WRITE_VPP_REG_BY_MODE(io_mode, addr + idx, coef_data[LC_MTRX_SIZE - 1]);

	reg_data = (offset_data[0] << 16) | offset_data[1];
	addr = ADDR_PARAM(lc_reg_cfg.page,
		lc_reg_cfg.reg_stts_mtrx_offset0_1);
	WRITE_VPP_REG_BY_MODE(io_mode, addr, reg_data);

	addr = ADDR_PARAM(lc_reg_cfg.page,
		lc_reg_cfg.reg_stts_mtrx_offset2);
	WRITE_VPP_REG_BY_MODE(io_mode, addr, offset_data[2]);

	reg_data = (offset_data[3] << 16) | offset_data[4];
	addr = ADDR_PARAM(lc_reg_cfg.page,
	...
	WRITE_VPP_REG_BY_MODE(io_mode, addr, offset_data[5]);
}

static void _set_lc_y2r_mtrx(int *coef_data,
	int *offset_data)
{
	int i, idx;
	unsigned int addr = 0;
...
static void _set_lc_r2y_mtrx(int *coef_data,
	int *offset_data)
{
	int i, idx;
	unsigned int addr = 0;
...
static void _set_lc_input_mux(int val,
	unsigned char start, unsigned char len)
{
	unsigned int addr = 0;
	enum io_mode_e io_mode = EN_MODE_DIR;
...
static void _set_lc_top_ctrl(int val,
	unsigned char start, unsigned char len)
{
	unsigned int addr = 0;
	enum io_mode_e io_mode = EN_MODE_DIR;
...
static void _set_lc_hv_num(int h_num, int v_num)
{
	unsigned int addr = 0;
	enum io_mode_e io_mode = EN_MODE_DIR;
...
static void _set_lc_blk_bdry(unsigned int height,
	unsigned int width)
{
	unsigned int addr = 0;
	enum io_mode_e io_mode = EN_MODE_DIR;
...
static void _set_lc_sat_lut(int *data)
{
...
static void _set_lc_map_curve(enum io_mode_e io_mode,
	int init, int *data)
{
	int h_num, v_num;
	unsigned int i, j, k;
...
static int _get_lc_curve_ymin_lmt(int *data, int len)
{
	int ret = 0;
...
static int _get_lc_curve_ypkbv_ymax_lmt(int *data, int len)
{
	int ret = 0;
	unsigned int i, tmp, tmp1, tmp2;
	unsigned int addr = 0;
...
static int _get_lc_curve_ymax_lmt(int *data, int len)
{
	int ret = 0;
	unsigned int i, j, tmp, tmp1, tmp2;
	unsigned int addr = 0;
...
static int _get_lc_curve_ypkbv_lmt(int *data, int len)
{
	int ret = 0;
	unsigned int i, tmp, tmp1, tmp2;
	unsigned int addr = 0;
...
static int _get_lc_curve_ypkbv_rat(int *data, int len)
{
	int ret = 0;
	unsigned int tmp;
	unsigned int addr = 0;
...
static int _get_lc_curve_ypkbv_slp_lmt(int *data, int len)
{
	int ret = 0;
	unsigned int tmp;
	unsigned int addr = 0;
...
static int _get_lc_curve_cntst_lmt_lh(int *data, int len)
{
	int ret = 0;
	unsigned int tmp;
	unsigned int addr = 0;
...
static int _get_lc_sat_lut(int *data, int len)
{
	int ret = 0;
	unsigned int i, tmp, tmp1, tmp2;
	unsigned int addr = 0;
...
static void _get_lc_hv_num(int *h_num, int *v_num)
{
	unsigned int addr = 0;
	int tmp = 0;
...
static void _dump_lc_reg_info(void)
{
	...
	PR_DRV("page = 0x%x\n", lc_reg_cfg.page);
	PR_DRV("reg_curve_ctrl = 0x%x\n", lc_reg_cfg.reg_curve_ctrl);
	PR_DRV("reg_curve_hv_num = 0x%x\n", lc_reg_cfg.reg_curve_hv_num);
	PR_DRV("reg_curve_lmt_rat = 0x%x\n", lc_reg_cfg.reg_curve_lmt_rat);
	PR_DRV("reg_curve_contrast_lh = 0x%x\n",
	...
	PR_DRV("page = 0x%x\n", sr1_reg_cfg.page);
	PR_DRV("reg_lc_input_mux = 0x%x\n", sr1_reg_cfg.reg_lc_input_mux);
	PR_DRV("reg_lc_top_ctrl = 0x%x\n", sr1_reg_cfg.reg_lc_top_ctrl);
	PR_DRV("reg_lc_hv_num = 0x%x\n", sr1_reg_cfg.reg_lc_hv_num);
	PR_DRV("reg_lc_sat_lut_0_1 = 0x%x\n", sr1_reg_cfg.reg_lc_sat_lut_0_1);
	PR_DRV("reg_lc_sat_lut_62 = 0x%x\n", sr1_reg_cfg.reg_lc_sat_lut_62);
	PR_DRV("reg_lc_curve_blk_hidx_0_1 = 0x%x\n",
		sr1_reg_cfg.reg_lc_curve_blk_hidx_0_1);
	PR_DRV("reg_lc_curve_blk_vidx_0_1 = 0x%x\n",
		sr1_reg_cfg.reg_lc_curve_blk_vidx_0_1);
}
...
static void _dump_lc_data_info(void)
{
	...
	if (lc_wr_curve) {
		PR_DRV("lc_wr_curve data info:\n");
		for (i = 0; i < 12 * 8; i++) {
			for (j = 0; j < 6 ; j++)
				PR_DRV("[%d]0x%08x\n",
					i * 6 + j, lc_wr_curve[i * 6 + j]);
			PR_DRV("\n");
		}
	}
}
...
static void _lc_tune_correct(int *omap, int *ihistogram, int i, int j)
{
	...
	pk_sum_2nd = (idx_pk2nd <= 1 || idx_pk2nd >= 14) ?
	...
	if (bin_pk2nd == 0) {
		...
		if ((pk_sum_2nd >= (pk_sum * 4 / 16)) && dist > 2)
			alpha = min(dist * 8, 64);
		else
			alpha = 0;
	}
	...
	alpha_sgl = 0;
	tmp_peak_val = bin_pk;/*pk_sum; bin_pk;*/
	...
	int *curve_data, int *hist_data)
{
	...
	for (i = 0; i < blk_vnum; i++) {
		for (j = 0; j < blk_hnum; j++) {
			...
			curve_data[idx_curve + 0] = temp1 & 0x3ff;/*bit0:9*/
			curve_data[idx_curve + 1] = (temp1 >> 10) & 0x3ff;/*bit10:19*/
			curve_data[idx_curve + 2] = (temp1 >> 20) & 0x3ff;/*bit20:29*/
			curve_data[idx_curve + 3] = temp2 & 0x3ff;/*bit0:9*/
			curve_data[idx_curve + 4] = (temp2 >> 10) & 0x3ff;/*bit10:19*/
			curve_data[idx_curve + 5] = (temp2 >> 20) & 0x3ff;/*bit20:29*/

			/*part2: get lc hist*/
			data32 = READ_VPP_REG_BY_MODE(io_mode, addr[1]);
			for (k = 0; k < 17; k++) {
				data32 = READ_VPP_REG_BY_MODE(io_mode, addr[2]);
				hist_data[idx_hist + k] = data32;
			}

			/*part3: add tune curve node patch--by vlsi-guopan*/
			if (lc_tune_mode == EN_LC_TUNE_CORRECT) {
				_lc_tune_correct(&curve_data[idx_curve],
						 &hist_data[idx_hist], i, j);
			} else if (lc_tune_mode == EN_LC_TUNE_LINEAR) {
				curve_data[idx_curve + 0] = 0;
				curve_data[idx_curve + 1] = 0;
				curve_data[idx_curve + 2] = 512;
				curve_data[idx_curve + 3] = 1023;
				curve_data[idx_curve + 4] = 1023;
				curve_data[idx_curve + 5] = 512;
			}
		}
	}
 ...
 */
static int _lc_signal_range_detect(unsigned short *hist)
{
	int bin_blk, bin_wht;
	int range = 0;
	...
	bin_blk = hist[0] + hist[1];
	bin_wht = hist[62] + hist[63];
	...
	enum lc_csc_type_e csc_type_out)
{
	...
	_set_lc_y2r_mtrx(&mtrx_coef_tbl[idx_coef].data[0],
	...
	_set_lc_r2y_mtrx(&mtrx_coef_tbl[idx_coef].data[0],
		&mtrx_ofst_tbl[idx_ofst].data[0]);
	...
	unsigned int height, unsigned int width)
{
	...
	_set_lc_hv_num(h_num, v_num);

	/*lc curve mapping config*/
	_set_lc_blk_bdry(height, width);
}
...
static void _lc_config(int bitdepth, unsigned short *hist,
	struct lc_vs_param_s *vs_param)
{
	...
	static unsigned int flag_csc_pre;
	unsigned int sps_w_in, sps_h_in;
	unsigned int height, width;
	unsigned int flag_csc, flag_full = 0;
	...
	if (lc_alg_cfg[EN_LC_DET_RANGE_MODE] == 2) {
		flag_full = _lc_signal_range_detect(hist);
		if (vs_param->vf_type & 0x20000000)
	...
	if (lc_vsr_mode) {
		sps_h_in = vs_param->vd_info.vd1_in_vsize;
		sps_w_in = vs_param->vd_info.vd1_in_hsize;
	} else {
		sps_h_in = vs_param->sps_h_in;
		sps_w_in = vs_param->sps_w_in;
	}

	if (!sps_h_in || !sps_w_in)
		return;

	if (vf_height_pre == vs_param->vf_height &&
		vf_width_pre == vs_param->vf_width &&
	...
	if (vf_height_pre != vs_param->vf_height) {
		pr_vpp(PR_DEBUG_LC, "[lc_config] vf_height from %d to %d\n",
			vf_height_pre, vs_param->vf_height);
		vf_height_pre = vs_param->vf_height;
	}

	if (vf_width_pre != vs_param->vf_width) {
		pr_vpp(PR_DEBUG_LC, "[lc_config] vf_width from %d to %d\n",
			vf_width_pre, vs_param->vf_width);
		vf_width_pre = vs_param->vf_width;
	}

	if (sps_w_in_pre != sps_w_in) {
		pr_vpp(PR_DEBUG_LC, "[lc_config] sps_w_in from %d to %d\n",
			sps_w_in_pre, sps_w_in);
		sps_w_in_pre = sps_w_in;
	}

	if (sps_h_in_pre != sps_h_in) {
		pr_vpp(PR_DEBUG_LC, "[lc_config] sps_h_in from %d to %d\n",
			sps_h_in_pre, sps_h_in);
		sps_h_in_pre = sps_h_in;
	}

	if (flag_full_pre != flag_full) {
		pr_vpp(PR_DEBUG_LC, "[lc_config] flag_full from %d to %d\n",
			flag_full_pre, flag_full);
		flag_full_pre = flag_full;
	}
	...
	/*"film", "bt2020"*/
	if (vs_param->vf_signal_type & (1 << 29))
		flag_csc = (vs_param->vf_signal_type >> 16) & 0xff;
	else
	...
	if (flag_csc_pre != flag_csc) {
		pr_vpp(PR_DEBUG_LC, "[lc_config] vf_signal_type: 0x%08x\n",
			vs_param->vf_signal_type);
		pr_vpp(PR_DEBUG_LC, "[lc_config] flag_csc from %d to %d\n",
			flag_csc_pre, flag_csc);
		flag_csc_pre = flag_csc;
	}
	...
	if (lc_vsr_mode) {
		height = vs_param->vd_info.vd1_dout_vsize;
		width = vs_param->vd_info.vd1_dout_hsize;
	} else {
		height = sps_h_in << vs_param->sps_h_en;
		width = sps_w_in << vs_param->sps_v_en;
	}
 ...
 */
static void _lc_osd_det_func(int *osd_flag_cnt,
	int *frm_cnt, int blk_hist_sum, int *lc_hist,
 ...
 */
static void _lc_osd_detect(int *osd_flag_cnt_above,
	int *osd_flag_cnt_below, int *frm_cnt_above,
	int *frm_cnt_below, int *lc_hist, int blk_hnum)
{
	int i, tmp;
	unsigned long blk_hist_sum = 0;

	if (!osd_flag_cnt_above ||
		!osd_flag_cnt_below ||
		!frm_cnt_above ||
		!frm_cnt_below ||
		!lc_hist)
	...
	for (i = 0; i < LC_HIST_BIN_COUNT; i++) {
		...
		blk_hist_sum += lc_hist[tmp];
	}

	/*above situation*/
	_lc_osd_det_func(osd_flag_cnt_above, frm_cnt_above,
		blk_hist_sum, lc_hist, blk_hnum,
		lc_alg_cfg[EN_LC_VNUM_START_ABOVE],
		lc_alg_cfg[EN_LC_VNUM_END_ABOVE]);

	/*below situation*/
	_lc_osd_det_func(osd_flag_cnt_below, frm_cnt_below,
		blk_hist_sum, lc_hist, blk_hnum,
 ...
 */
static int _lc_global_scene_change(int *curve_nodes_cur,
	int *curve_nodes_pre, int blk_vnum, int blk_hnum,
	int *osd_flag_cnt_above, int *osd_flag_cnt_below)
{
	static int scene_dif[LC_N];
	static int frm_dif[LC_N];
	/*store frame valid block for frm diff calculate*/
	...
	if (video_scene_change_flag_en) {
		...
		/* 2.2.1 use block APL to calculate frame diff:*/
		...
		for (i = 0; i < blk_vnum; i++) {
			for (j = 0; j < blk_hnum; j++) {
				...
				apl_cur = curve_nodes_cur[tmp];
				apl_pre = curve_nodes_pre[tmp];
				/*frame motion*/
				frm_dif[LC_N - 1] += abs(apl_cur - apl_pre);

				/*remove osd to calculate frame motion*/
		...
		/*2.2.2 motion diff too large, scene change*/
		scene_dif[LC_N - 1] =
 ...
 */
static int _lc_cal_iir_alpha(int *refresh_alpha,
	...
	int frm_cnt_above, int frm_cnt_below)
{
	int i, j, val;
	int addr_curv1;
	int osd_local_p = 0, osd_local_m = 0;
 ...
 */
static int _lc_cal_curv_iir(int *curve_nodes_cur,
	int *curve_nodes_pre, int *refresh_alpha,
	int blk_vnum, int blk_hnum, int refresh)
{
	int i, j, k;
	int node_cur;
	...
	for (i = 0; i < blk_vnum; i++) {
		for (j = 0; j < blk_hnum; j++) {
			...
			for (k = 0; k < LC_CURVE_NODES; k++) {
				...
				/*u12 calc*/
				node_cur = curve_nodes_cur[addr_curv2] << 2;
				...
				curve_nodes_cur[addr_curv2] = (int)(tmap[k]);
				/*delay for next iir*/
				curve_nodes_pre[addr_curv2] = (int)(tmap[k]);
			}
		}
	}
 ...
 */
static void _lc_curve_iir_process(int *lc_hist,
	int *lc_szcurve, int blk_vnum, int blk_hnum)
{
	/*osd detect*/
	...
	if (scene_change_flag != pre_scene_change_flag) {
		pr_vpp(PR_DEBUG_LC, "[lc_iir_process] scene_change from %d to %d\n",
			pre_scene_change_flag, scene_change_flag);
		pre_scene_change_flag = scene_change_flag;
	}
...
curve_fresh:
	for (i = 0; i < LC_CURVE_SIZE; i++)
		lc_szcurve[i] = curve_nodes_cur[i];
}
...
static void _lc_buff_init(void)
{
	...
	lc_wr_curve = kcalloc(LC_CURVE_SIZE, sizeof(int), GFP_KERNEL);
	if (!lc_wr_curve) {
		kfree(lc_hist_data);
		return;
	}
	...
	memset(lc_wr_curve, 0, LC_CURVE_SIZE * sizeof(int));
...
static void _lc_config_init(void)
{
	...
	_set_lc_sat_lut(&sat_lut_tbl[0].data[0]);
	/*lc curve linear parameters*/
	_set_lc_map_curve(EN_MODE_DIR, 1, NULL);
}
...
/*External functions*/
int vpp_module_lc_init(struct vpp_dev_s *dev)
{
	...
	lc_curve_isr_defined = dev->lc_isr_defined;
	lc_curve_isr_used = 1;
...
void vpp_module_lc_write_lut(enum lc_lut_type_e type, int *data)
{
	if (!data)
		return;
	...
	switch (type) {
	case EN_LC_SAT:
		_set_lc_sat_lut(data);
		break;
	case EN_LC_YMIN_LMT:
		_set_lc_curve_ymin_lmt(data);
		break;
	case EN_LC_YPKBV_YMAX_LMT:
		_set_lc_curve_ypkbv_ymax_lmt(data);
		break;
	case EN_LC_YMAX_LMT:
		_set_lc_curve_ymax_lmt(data);
		break;
	case EN_LC_YPKBV_LMT:
		_set_lc_curve_ypkbv_lmt(data);
		break;
	case EN_LC_YPKBV_RAT:
		_set_lc_curve_ypkbv_rat(data);
		break;
	case EN_LC_YPKBV_SLP_LMT:
		_set_lc_curve_ypkbv_slp_lmt(data);
		break;
	case EN_LC_CNTST_LMT:
		_set_lc_curve_cntst_lmt_lh(data);
		break;
	default:
		break;
	}
...
int vpp_module_lc_read_lut(enum lc_lut_type_e type, int *data, int len)
{
	int ret = 0;
	...
	switch (type) {
	case EN_LC_SAT:
		ret = _get_lc_sat_lut(data, len);
		break;
	case EN_LC_YMIN_LMT:
		ret = _get_lc_curve_ymin_lmt(data, len);
		break;
	case EN_LC_YPKBV_YMAX_LMT:
		ret = _get_lc_curve_ypkbv_ymax_lmt(data, len);
		break;
	case EN_LC_YMAX_LMT:
		ret = _get_lc_curve_ymax_lmt(data, len);
		break;
	case EN_LC_YPKBV_LMT:
		ret = _get_lc_curve_ypkbv_lmt(data, len);
		break;
	case EN_LC_YPKBV_RAT:
		ret = _get_lc_curve_ypkbv_rat(data, len);
		break;
	case EN_LC_YPKBV_SLP_LMT:
		ret = _get_lc_curve_ypkbv_slp_lmt(data, len);
		break;
	case EN_LC_CNTST_LMT:
		ret = _get_lc_curve_cntst_lmt_lh(data, len);
		break;
	default:
		break;
	}
...
void vpp_module_lc_on_vs(unsigned short *data_hist,
	struct lc_vs_param_s *vs_param)
{
	...
	memcpy(&lc_vs_param, vs_param, sizeof(struct lc_vs_param_s));

	_lc_config(lc_alg_cfg[EN_LC_BITDEPTH], data_hist, vs_param);

	_get_lc_hv_num(&blk_hnum, &blk_vnum);

	if (!lc_curve_isr_used || !lc_curve_isr_defined)
		_get_lc_region_data(blk_vnum, blk_hnum, lc_wr_curve, lc_hist_data);

	/*do time domain iir*/
	_lc_curve_iir_process(lc_hist_data, lc_wr_curve, blk_vnum, blk_hnum);

	/*set curve data*/
	_set_lc_map_curve(EN_MODE_RDMA, 0, lc_wr_curve);
}
