...
static bool pre_cm_en;
static bool cur_cm_en;
	...
	unsigned char start, unsigned char len, enum io_mode_e io_mode)
{
	...
	WRITE_VPP_REG_BY_MODE(EN_MODE_DIR, reg_addr[0], addr);
	tmp = READ_VPP_REG_BY_MODE(EN_MODE_DIR, reg_addr[1]);
...
static void _set_cm2_luma_by_index(int *data,
	int start, int end)
{
	unsigned int addr = 0;
	...
	for (i = start; i <= end; i++) {
		...
		/*curve 0*/
		val[0] &= 0xffffff00;
		val[0] |= data[i] & 0x000000ff;

		for (j = 0; j < CM2_DATA_WR_COUNT; j++) {
			addr = cm_addr_cfg.addr_cm2_enh_coef0_h00 + i * 8 + j;
			_set_cm_reg_by_addr(addr, val[j], EN_MODE_DIR);
		}
	}
}

static void _set_cm2_sat_by_index(int *data,
	int start, int end)
{
	unsigned int addr = 0;
	int val[CM2_DATA_WR_COUNT] = {0};
	...
	for (i = start; i <= end; i++) {
		...
		val[0] &= 0x000000ff;
		/*curve 0*/
		tmp = data[i + CM2_CURVE_SIZE * 0];
		val[0] |= (tmp << 8) & 0x0000ff00;
		/*curve 1*/
		tmp = data[i + CM2_CURVE_SIZE * 1];
		val[0] |= (tmp << 16) & 0x00ff0000;
		/*curve 2*/
		tmp = data[i + CM2_CURVE_SIZE * 2];
...
static void _set_cm2_sat_via_y_by_index(int *data,
	int start, int end)
{
	unsigned int addr = 0;
	int val[CM2_DATA_WR_COUNT] = {0};
	...
	for (i = start; i <= end; i++) {
		...
		tmp = data[i + CM2_CURVE_SIZE * 3];
		val[4] |= (tmp << 16) & 0x00ff0000;
		tmp = data[i + CM2_CURVE_SIZE * 4];
...
static void _set_cm2_hue_by_index(int *data,
	int start, int end)
{
	unsigned int addr = 0;
	int val[CM2_DATA_WR_COUNT] = {0};
...
static void _set_cm2_hue_via_s_by_index(int *data,
	int start, int end)
{
	unsigned int addr = 0;
	int val[CM2_DATA_WR_COUNT] = {0};
...
static void _set_cm2_hue_via_y_by_index(int *data,
	int start, int end)
{
	unsigned int addr = 0;
	int val[CM2_DATA_WR_COUNT] = {0};
...
static void _set_cm2_sat_by_l(int *data)
{
	unsigned int addr = 0;
	int val = 0;
	...
	val |= data[2] << 16;
	val |= data[3] << 24;
	_set_cm_reg_by_addr(addr, val, EN_MODE_DIR);
	...
	int *data, int start, int end)
{
	...
	for (i = start; i <= end; i++) {
		...
		case EN_CM2_CURVE_LUMA:
			/*curve 0*/
			data[i] = val[0] & 0x000000ff;
			data_len += 1;
			break;
		case EN_CM2_CURVE_SAT:
			/*curve 0*/
			data[i + CM2_CURVE_SIZE * 0] = (val[0] & 0x0000ff00) >> 8;
			/*curve 1*/
			data[i + CM2_CURVE_SIZE * 1] = (val[0] & 0x00ff0000) >> 16;
			/*curve 2*/
			data[i + CM2_CURVE_SIZE * 2] = (val[0] & 0xff000000) >> 24;
			data_len += 3;
			break;
		case EN_CM2_CURVE_SAT_Y:
			/*curve 0*/
			data[i + CM2_CURVE_SIZE * 0] = (val[3] & 0xff000000) >> 24;
			/*curve 1,2,3,4*/
			data[i + CM2_CURVE_SIZE * 1] = val[4] & 0x000000ff;
			data[i + CM2_CURVE_SIZE * 2] = (val[4] & 0x0000ff00) >> 8;
			data[i + CM2_CURVE_SIZE * 3] = (val[4] & 0x00ff0000) >> 16;
			data[i + CM2_CURVE_SIZE * 4] = (val[4] & 0xff000000) >> 24;
			data_len += 5;
			break;
		case EN_CM2_CURVE_HUE:
			/*curve 0*/
			data[i] = val[1] & 0x000000ff;
			data_len += 1;
			break;
		case EN_CM2_CURVE_HUE_S:
			/*curve 0,1*/
			data[i + CM2_CURVE_SIZE * 0] = (val[2] & 0x00ff0000) >> 16;
			data[i + CM2_CURVE_SIZE * 1] = (val[2] & 0xff000000) >> 24;
			/*curve 2,3,4*/
			data[i + CM2_CURVE_SIZE * 2] = val[3] & 0x000000ff;
			data[i + CM2_CURVE_SIZE * 3] = (val[3] & 0x0000ff00) >> 8;
			data[i + CM2_CURVE_SIZE * 4] = (val[3] & 0x00ff0000) >> 16;
			data_len += 5;
			break;
		case EN_CM2_CURVE_HUE_Y:
			/*curve 0,1,2*/
			data[i + CM2_CURVE_SIZE * 0] = (val[1] & 0x0000ff00) >> 8;
			data[i + CM2_CURVE_SIZE * 1] = (val[1] & 0x00ff0000) >> 16;
			data[i + CM2_CURVE_SIZE * 2] = (val[1] & 0xff000000) >> 24;
			/*curve 3,4*/
			data[i + CM2_CURVE_SIZE * 3] = val[2] & 0x000000ff;
			data[i + CM2_CURVE_SIZE * 4] = (val[2] & 0x0000ff00) >> 8;
			data_len += 5;
			break;
		}
	}
...
static void _dump_cm_reg_info(void)
{
	...
	PR_DRV("page = 0x%x\n", chroma_reg_cfg.page);
	PR_DRV("reg_misc = 0x%x\n", chroma_reg_cfg.reg_misc);
	PR_DRV("reg_addr_port = 0x%x\n", chroma_reg_cfg.reg_addr_port);
	PR_DRV("reg_data_port = 0x%x\n", chroma_reg_cfg.reg_data_port);

	PR_DRV("cm_addr_cfg info:\n");
	PR_DRV("addr_sat_byyb_node0 = 0x%x\n",
		cm_addr_cfg.addr_sat_byyb_node0);
	PR_DRV("addr_sat_byyb_node1 = 0x%x\n",
		cm_addr_cfg.addr_sat_byyb_node1);
	PR_DRV("addr_sat_byyb_node2 = 0x%x\n",
		cm_addr_cfg.addr_sat_byyb_node2);
	PR_DRV("addr_sat_src_node = 0x%x\n",
		cm_addr_cfg.addr_sat_src_node);
	PR_DRV("addr_cm_enh_sft_mode = 0x%x\n",
		cm_addr_cfg.addr_cm_enh_sft_mode);
	PR_DRV("addr_frm_size = 0x%x\n",
		cm_addr_cfg.addr_frm_size);
	PR_DRV("addr_filter_cfg = 0x%x\n",
		cm_addr_cfg.addr_filter_cfg);
	PR_DRV("addr_cm_global_gain = 0x%x\n",
		cm_addr_cfg.addr_cm_global_gain);
	PR_DRV("addr_cm_enh_ctl = 0x%x\n",
		cm_addr_cfg.addr_cm_enh_ctl);
	PR_DRV("addr_roi_x_scope = 0x%x\n",
		cm_addr_cfg.addr_roi_x_scope);
	PR_DRV("addr_roi_y_scope = 0x%x\n",
		cm_addr_cfg.addr_roi_y_scope);
	PR_DRV("addr_ifo_mode = 0x%x\n",
		cm_addr_cfg.addr_ifo_mode);
	PR_DRV("addr_luma_adj_lmt = 0x%x\n",
		cm_addr_cfg.addr_luma_adj_lmt);
	PR_DRV("addr_sat_adj_lmt = 0x%x\n",
		cm_addr_cfg.addr_sat_adj_lmt);
	PR_DRV("addr_hue_adj_lmt = 0x%x\n",
		cm_addr_cfg.addr_hue_adj_lmt);
	PR_DRV("addr_hue_cfg = 0x%x\n",
		cm_addr_cfg.addr_hue_cfg);
	PR_DRV("addr_luma_adj1 = 0x%x\n",
		cm_addr_cfg.addr_luma_adj1);
	PR_DRV("addr_sta_cfg = 0x%x\n",
		cm_addr_cfg.addr_sta_cfg);
	PR_DRV("addr_sta_sat_hist0 = 0x%x\n",
		cm_addr_cfg.addr_sta_sat_hist0);
	PR_DRV("addr_sta_sat_hist1 = 0x%x\n",
		cm_addr_cfg.addr_sta_sat_hist1);
	PR_DRV("addr_cm2_enh_coef0_h00 = 0x%x\n",
		cm_addr_cfg.addr_cm2_enh_coef0_h00);

	if (support_cm_final_gain) {
		PR_DRV("addr_cm_final_gain_luma = 0x%x\n",
			cm_addr_cfg.addr_cm_final_gain_luma);
		PR_DRV("addr_cm_final_gain_sat = 0x%x\n",
			cm_addr_cfg.addr_cm_final_gain_sat);
		PR_DRV("addr_cm_final_gain_hue = 0x%x\n",
...
int vpp_module_cm_init(struct vpp_dev_s *dev)
{
	...
	_set_cm_reg_by_addr(cm_addr_cfg.addr_cm_enh_ctl, 0x1d, EN_MODE_DIR);
...
int vpp_module_cm_en(bool enable, enum io_mode_e io_mode)
{
	...
	if (enable) {
		...
		val = 24;
		_set_cm_bit_by_addr(addr, val, start, len, EN_MODE_DIR);
...
/*input array size is 32 for data*/
void vpp_module_cm_set_cm2_luma(int *data)
{
	...
	for (i = 0; i < CM2_CURVE_SIZE; i++) {
		tmp = data[i] + cur_cm2_offset_luma[i];
...
/*input array size is 32*3 as curve 0/1/2 for data*/
void vpp_module_cm_set_cm2_sat(int *data)
{
	...
	for (i = 0; i < CM2_CURVE_SIZE * 3; i++) {
		tmp = data[i] + cur_cm2_offset_sat[i] + cm_ai_pq_offset.sat[i];
		cur_cm2_sat[i] = vpp_check_range(tmp, (-128), 127);
		cm_ai_pq_base.sat[i] = data[i];
	}

	_set_cm2_sat_by_index(&cur_cm2_sat[0], 0, 31);
}

/*input array size is 9 for data*/
void vpp_module_cm_set_cm2_sat_by_l(int *data)
{
	...
	for (i = 0; i < 9; i++)
		cur_cm2_sat_l[i] = data[i] & 0x000000ff;

	_set_cm2_sat_by_l(&cur_cm2_sat_l[0]);
}

/*input array size is 32*5 as curve 0/1/2/3/4 for data*/
...
void vpp_module_cm_set_cm2_hue(int *data)
{
	...
	for (i = 0; i < CM2_CURVE_SIZE; i++) {
		tmp = data[i] + cur_cm2_offset_hue[i];
...
void vpp_module_cm_set_cm2_hue_by_hs(int *data)
{
	...
	for (i = 0; i < CM2_CURVE_SIZE * 5; i++) {
		tmp = data[i] + cur_cm2_offset_hue_hs[i];
...
void vpp_module_cm_set_demo_mode(bool enable, bool left_side)
{
	...
	if (enable) {
		...
		_set_cm_bit_by_addr(cm_addr_cfg.addr_roi_x_scope,
			...
			cm_bit_cfg.bit_roi_x_end.len, EN_MODE_DIR);

		_set_cm_bit_by_addr(cm_addr_cfg.addr_roi_y_scope,
			height,
			cm_bit_cfg.bit_roi_y_end.start,
			cm_bit_cfg.bit_roi_y_end.len, EN_MODE_DIR);
...
void vpp_module_cm_set_cfg_param(struct cm_cfg_param_s *param)
{
	...
	val = (param->frm_width & 0x00001fff) |
		((param->frm_height & 0x00001fff) << 16);
	...
	bool enable)
{
	...
	case EN_SUB_MD_LUMA_ADJ:
		...
		_set_cm_bit_by_addr(addr, enable, start, len, EN_MODE_DIR);
		break;
...
void vpp_module_cm_set_cm2_offset_hue_by_hs(char *data)
{
	...
	for (i = 0; i < CM2_CURVE_SIZE * 5; i++)
		cur_cm2_offset_hue_hs[i] = data[i];
}
...
int vpp_module_cm_get_cm2_luma(int *data)
{
	...
	return _get_cm2_curve_by_index(EN_CM2_CURVE_LUMA, data, 0, 31);
}
...
int vpp_module_cm_get_cm2_sat(int *data)
{
	...
	return _get_cm2_curve_by_index(EN_CM2_CURVE_SAT, data, 0, 31);
}
...
int vpp_module_cm_get_cm2_sat_by_l(int *data)
{
	...
	data[5] = (val & 0x0000ff00) >> 8;
	data[6] = (val & 0x00ff0000) >> 16;
	data[7] = (val & 0xff000000) >> 24;

	addr = cm_addr_cfg.addr_sat_byyb_node2;
	val = _get_cm_reg_by_addr(addr);
	data[8] = val & 0x000000ff;
...
int vpp_module_cm_get_cm2_sat_by_hl(int *data)
{
	...
	return _get_cm2_curve_by_index(EN_CM2_CURVE_SAT_Y, data, 0, 31);
}
...
int vpp_module_cm_get_cm2_hue(int *data)
{
	...
	return _get_cm2_curve_by_index(EN_CM2_CURVE_HUE, data, 0, 31);
}
...
int vpp_module_cm_get_cm2_hue_by_hs(int *data)
{
	...
	return _get_cm2_curve_by_index(EN_CM2_CURVE_HUE_S, data, 0, 31);
}
...
int vpp_module_cm_get_cm2_hue_by_hl(int *data)
{
	...
	return _get_cm2_curve_by_index(EN_CM2_CURVE_HUE_Y, data, 0, 31);
}
...
//			pre_cm_en, cur_cm_en);
		...
		_set_cm_bit_by_addr(addr, cur_cm_en,
...
void vpp_module_cm_get_ai_pq_base(struct cm_ai_pq_param_s *param)
{
	...
	for (i = 0; i < CM2_CURVE_SIZE * 3; i++)
		param->sat[i] = cm_ai_pq_base.sat[i];
}
...
void vpp_module_cm_set_ai_pq_offset(struct cm_ai_pq_param_s *param)
{
	...
	for (i = 0; i < CM2_CURVE_SIZE * 3; i++)
		if (cm_ai_pq_offset.sat[i] != param->sat[i]) {
			cm_ai_pq_offset.sat[i] = param->sat[i];
