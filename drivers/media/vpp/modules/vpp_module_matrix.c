...
struct _matrix_bit_cfg_s {
	...
	struct _bit_s bit_mtrx_clip_mode;
...
static int _set_matrix_coef(enum io_mode_e io_mode,
	enum matrix_mode_e mode, enum _matrix_size_e size, int *data)
{
	unsigned int addr = 0;
	unsigned int val = 0;
...
static void _dump_matrix_reg_info(void)
{
	...
	for (i = 0; i < EN_MTRX_MODE_MAX; i++) {
		...
		PR_DRV("page = 0x%x\n", mtrx_reg_cfg[i].page);
		PR_DRV("reg_mtrx_coef00_01 = 0x%x\n",
			mtrx_reg_cfg[i].reg_mtrx_coef00_01);
		PR_DRV("reg_mtrx_coef22 = 0x%x\n",
			mtrx_reg_cfg[i].reg_mtrx_coef22);
		PR_DRV("reg_mtrx_coef13_14 = 0x%x\n",
			mtrx_reg_cfg[i].reg_mtrx_coef13_14);
		PR_DRV("reg_mtrx_clip = 0x%x\n",
			mtrx_reg_cfg[i].reg_mtrx_clip);
		PR_DRV("reg_mtrx_offset0_1 = 0x%x\n",
			mtrx_reg_cfg[i].reg_mtrx_offset0_1);
		PR_DRV("reg_mtrx_pre_offset2 = 0x%x\n",
			mtrx_reg_cfg[i].reg_mtrx_pre_offset2);
		PR_DRV("reg_mtrx_en_ctrl = 0x%x\n",
...
static void _dump_matrix_data_info(void)
{
	...
	for (i = 0; i < EN_MTRX_MODE_MAX; i++) {
		for (j = 0; j < MTRX_OFFSET_CNT; j++)
			PR_DRV("0x%08x\t", cur_mtrx_data[i].pre_offset[j]);
		PR_DRV("\n");
		for (j = 0; j < 15; j++)
			PR_DRV("0x%08x\t", cur_mtrx_data[i].coef[j]);
		PR_DRV("\n");
		for (j = 0; j < MTRX_OFFSET_CNT; j++)
			PR_DRV("0x%08x\t", cur_mtrx_data[i].offset[j]);
	}
}

/*External functions*/
int vpp_module_matrix_init(struct vpp_dev_s *dev)
{
	int i = 0;
...
int vpp_module_matrix_clip_mode(enum matrix_mode_e mode, int val)
{
	return _set_matrix_clip(mode, val,
		mtrx_bit_cfg.bit_mtrx_clip_mode.start,
		mtrx_bit_cfg.bit_mtrx_clip_mode.len);
}
	...
	int *data)
{
	...
	memcpy(&cur_mtrx_data[mode].offset[0], data,
	...
	int *data)
{
	...
	memcpy(&cur_mtrx_data[mode].pre_offset[0], data,
		sizeof(int) * MTRX_OFFSET_CNT);
	cur_mtrx_data[mode].changed = true;
...
int vpp_module_matrix_set_coef_3x3(enum matrix_mode_e mode, int *data)
{
	if (mode == EN_MTRX_MODE_MAX || !data)
		return 0;

	memcpy(&cur_mtrx_data[mode].coef[0], data, sizeof(int) * 9);
	cur_mtrx_data[mode].changed = true;

	return 0;
}

int vpp_module_matrix_set_coef_3x5(enum matrix_mode_e mode, int *data)
{
	if (mode == EN_MTRX_MODE_MAX || !data)
		return 0;

	memcpy(&cur_mtrx_data[mode].coef[0], data, sizeof(int) * 15);
	cur_mtrx_data[mode].changed = true;
