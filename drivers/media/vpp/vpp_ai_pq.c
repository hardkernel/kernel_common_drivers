...
static int _ai_pq_skin_tone_scene_process(int offset, bool enable)
{
	static bool first_frame = true;
	static int reg_val;
	static int offset_val;
...
static int _ai_pq_peaking_scene_process(int offset, bool enable)
{
	...
	int tmp;
	int i;
	struct sr_final_gain_param_s param;
	...
	for (i = 0; i < 4; i++)
		base_val[i] = param.final_gains[i];
	...
	for (i = 0; i < 4; i++) {
		...
		param.final_gains[i] = offset_val[i];
	}
...
int vpp_ai_pq_init(void)
{
	...
	for (index = EN_BLUE_SCENE; index <= EN_SCENE_MAX; index++) {
		...
		case EN_SKIN_TONE_SCENE:
			detect_scenes[index].enable = true;
			detect_scenes[index].func =
				_ai_pq_skin_tone_scene_process;
			break;
