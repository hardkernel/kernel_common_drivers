...
static bool adc_b2_180M_enable = true;
module_param(adc_b2_180M_enable, bool, 0644);
MODULE_PARM_DESC(adc_b2_180M_enable, "");

static bool adc_s2_mux_en;
module_param(adc_s2_mux_en, bool, 0644);
MODULE_PARM_DESC(adc_s2_mux_en, "");

static bool adc_pll_switch = true;
module_param(adc_pll_switch, bool, 0644);
MODULE_PARM_DESC(adc_pll_switch, "");
...
static int adc_32m_pll_config(struct tvin_adc_dev *devp)
{
	...
	if (devp->plat_data->chip_id == ADC_CHIP_TXHD2) {
		...
		adc_wr_hiu(pll_addr->adc_pll_cntl_1, 0xc3b02004);
		   ...
		   devp->plat_data->chip_id == ADC_CHIP_T6X) {
		...
		devp->plat_data->chip_id == ADC_CHIP_T6W) {
		adc_wr_hiu(pll_addr->adc_pll_cntl_0, 0x000100c0);
		adc_wr_hiu(pll_addr->adc_pll_cntl_1, 0x0230132f);
...
static int adc_dadc_atv_cntl_config(struct tvin_adc_dev *devp)
{
	...
	} else if (devp->plat_data->chip_id == ADC_CHIP_T6X) {
		adc_wr_hiu(adc_addr->vdac_cntl_0, 0x00418882);
		adc_wr_hiu(adc_addr->s2_dadc_cntl_2, 0x00000580);
		adc_wr_hiu(adc_addr->dadc_cntl_4, 0x00000000);
		adc_wr_hiu(adc_addr->dadc_cntl, 0x06263104);
	} else
#endif
	if (devp->plat_data->chip_id >= ADC_CHIP_T5) {
		adc_wr_hiu_bits(adc_addr->vdac_cntl_0, 1, 11, 1);
	...
	if (devp->plat_data->chip_id <= ADC_CHIP_T3X) {
		...
		adc_wr_hiu(adc_addr->dadc_cntl_2, 0x0000c0c8);
		adc_wr_hiu(adc_addr->dadc_cntl_3, 0x00341b83);
	}
#endif
	return 0;
}
...
static int adc_dadc_av_cntl_config(struct tvin_adc_dev *devp)
{
	...
	if (devp->plat_data->chip_id <= ADC_CHIP_T3X) {
		...
		adc_wr_hiu(adc_addr->dadc_cntl_2, 0x0000c048);
		adc_wr_hiu(adc_addr->dadc_cntl_3, 0x00341b83);
	}
#endif
	return 0;
...
static int adc_dadc_dvbs_cntl_config(struct tvin_adc_dev *devp)
{
	...
	if (devp->plat_data->chip_id <= ADC_CHIP_T3X) {
		...
		adc_wr_hiu(adc_addr->s2_dadc_cntl, 0x41239107);
		adc_wr_hiu(adc_addr->s2_dadc_cntl_2, 0x0000580);
#endif
	}
	return 0;
}
...
static int adc_dadc_dvbt_cntl_config(struct tvin_adc_dev *devp)
{
	...
	if (devp->plat_data->chip_id <= ADC_CHIP_T3X) {
		...
		adc_wr_hiu(adc_addr->dadc_cntl_2, 0x0000c0c8);
		adc_wr_hiu(adc_addr->dadc_cntl_3, 0x00341b93);
	}
#endif
	return 0;
}
...
static void adc_av_filter_config(struct tvin_adc_dev *devp)
{
	...
	if (devp->plat_data->chip_id == ADC_CHIP_TXHD2) {
		...
		adc_wr_afe(AFE_VAFE_CTRL2, 0x04250d87);
	} else if (devp->plat_data->chip_id == ADC_CHIP_T6X) {
		adc_wr_afe(AFE_VAFE_CTRL0, 0x008b0790);
		adc_wr_afe(AFE_VAFE_CTRL1, 0x0000010e);
		adc_wr_afe(AFE_VAFE_CTRL2, 0x0830fd8b);
		...
		adc_wr_afe(AFE_VAFE_CTRL2, 0x0010ef83);
	}
#endif
}
...
static void adc_atv_filter_config(struct tvin_adc_dev *devp)
{
	...
	if (devp->plat_data->chip_id == ADC_CHIP_TXHD2) {
		...
		adc_wr_afe(AFE_VAFE_CTRL2, 0x1fc09f80);
	} else if (devp->plat_data->chip_id == ADC_CHIP_T6D ||
		devp->plat_data->chip_id == ADC_CHIP_T6W) {
		...
		adc_wr_afe(AFE_VAFE_CTRL0, 0x8F0730);
		adc_wr_afe(AFE_VAFE_CTRL1, 0xe);
		adc_wr_afe(AFE_VAFE_CTRL2, 0xA0CFD98);
	} else if (cpu_after_eq(MESON_CPU_MAJOR_ID_TL1)) {
		adc_wr_afe(AFE_VAFE_CTRL0, 0x000d0710);
		adc_wr_afe(AFE_VAFE_CTRL1, 0x3000);
		adc_wr_afe(AFE_VAFE_CTRL2, 0x1fe09e01);
	}
#ifndef CONFIG_AMLOGIC_ZAPPER_CUT
	else if (get_cpu_type() == MESON_CPU_MAJOR_ID_TXL ||
		get_cpu_type() == MESON_CPU_MAJOR_ID_TXLX) {
...
//t6w isdbt 180M
static int adc_dpll_isdbt_config(struct tvin_adc_dev *devp,
				struct dfe_adcpll_para *p_dtv_para)
{
	struct adc_pll_reg_addr *pll_addr;
	...
	if (devp->plat_data->chip_id == ADC_CHIP_T6W && adc_b2_180M_enable &&
		...
		adc_wr_hiu(pll_addr->adc_pll_cntl_0, 0x000100b4);
		adc_wr_hiu(pll_addr->adc_pll_cntl_1, 0x0230132f);
		adc_wr_hiu(pll_addr->adc_pll_cntl_2, 0x07061d0c);
		adc_wr_hiu(pll_addr->adc_pll_cntl_0, 0x100100b4);
		usleep_range(20, 25);
		adc_wr_hiu(pll_addr->adc_pll_cntl_0, 0x500100b4);
		usleep_range(20, 25);
...
static int adc_dpll_dvbc_config(struct tvin_adc_dev *devp,
				struct dfe_adcpll_para *p_dtv_para)
{
	struct adc_pll_reg_addr *pll_addr;
	...
	if (devp->plat_data->chip_id == ADC_CHIP_T6X) {
		/* adc_dpll_intclk 480M */
		if (adc_pll_switch) {
			adc_wr_hiu(pll_addr->adc_pll_cntl_0, 0x11fe4140);
			adc_wr_hiu(pll_addr->adc_pll_cntl_1, 0x021a8605);
			adc_wr_hiu(pll_addr->adc_pll_cntl_2, 0x00000080);
			adc_wr_hiu(pll_addr->adc_pll_cntl_1, 0x021ac605);
			usleep_range(20, 25);
			adc_wr_hiu(pll_addr->adc_pll_cntl_0, 0x31fe4140);
			usleep_range(20, 25);
			adc_wr_hiu(pll_addr->adc_pll_cntl_1, 0x021a4605);
		} else {
			adc_wr_hiu(pll_addr->adc_pll_cntl_0, 0x11ee410e);
...
void adc_t2_mux_s2(void)
{
	...
	adc_addr = &devp->plat_data->adc_addr;
	adc_wr_hiu(adc_addr->vdac_cntl_0, 0x00418882);
	adc_wr_hiu(adc_addr->vdac_cntl_1, 0x8);
	adc_wr_afe(AFE_VAFE_CTRL0, 0x00870730);
	adc_wr_afe(AFE_VAFE_CTRL1, 0xe);
	adc_wr_afe(AFE_VAFE_CTRL2, 0xa0cfd98);
	adc_wr_hiu(adc_addr->dadc_cntl, 0x06263105);
	adc_wr_hiu(adc_addr->dadc_cntl_2, 0xc048);
	adc_wr_hiu(adc_addr->dadc_cntl_3, 0x02341b80);
	adc_wr_hiu(adc_addr->dadc_cntl_4, 0x80);
	adc_dpll_dvbt_config(devp);
	adc_wr_hiu(adc_addr->s2_dadc_cntl, 0x41209111);
	adc_wr_hiu(adc_addr->s2_dadc_cntl_2, 0x560);
}
EXPORT_SYMBOL(adc_t2_mux_s2);

void adc_dvbc_mux_s2(void)
{
	struct tvin_adc_dev *devp = adc_devp;
	struct adc_reg_addr *adc_addr;
	struct adc_pll_reg_addr *pll_addr;
	...
	adc_wr_hiu(adc_addr->s2_dadc_cntl, 0x41209105);
	adc_wr_hiu(adc_addr->s2_dadc_cntl_2, 0x560);
}
EXPORT_SYMBOL(adc_dvbc_mux_s2);

void adc_atv_mux_s2(void)
{
	struct tvin_adc_dev *devp = adc_devp;
	struct adc_reg_addr *adc_addr;
	struct adc_pll_reg_addr *pll_addr;
						...
						struct dfe_adcpll_para *p_dtv_para)
{
	...
	case SYS_ISDBT:
		if (adc_s2_mux_en)
			...
			adc_dpll_isdbt_config(devp, p_dtv_para);
		break;
	...
	case SYS_DVBC_ANNEX_C:
		if (adc_s2_mux_en)
			...
			adc_dpll_dvbc_config(devp, p_dtv_para);
		break;
	default:
		if (adc_s2_mux_en)
			adc_dvbc_mux_s2();
...
static bool adc_check_lock_status(enum adc_chip_ver chip_id,
	struct adc_pll_reg_addr *pll_addr)
{
	...
	for (i = 0; i < 10; ++i) {
		...
		case ADC_CHIP_T3X:
		case ADC_CHIP_T6D:
			addr = ANACTRL_ADC_PLL_STS;
			break;
		case ADC_CHIP_T3:
		case ADC_CHIP_T6W:
		case ADC_CHIP_T6X:
			addr = pll_addr->adc_pll_cntl_7;
			break;
		case ADC_CHIP_S4:
		case ADC_CHIP_S4D:
			addr = pll_addr->adc_pll_cntl_0 + 0x10;
			break;
		default:
...
int adc_set_pll_cntl(bool on, enum adc_sel module_sel, void *p_para)
{
	...
	case ADC_ATV_DEMOD:
		...
		if (devp->pll_flg & ADC_ATV_DEMOD) {
			if (devp->print_en & ADC_DBG_EN)
				pr_info("%s:ATV DEMOD PLL exist!:%d\n",
	...
	case ADC_DTV_DEMOD:
		...
		if (devp->pll_flg & ADC_DTV_DEMOD) {
			if (devp->print_en & ADC_DBG_EN)
				pr_info("%s:DTV DEMOD PLL exist!:%d\n",
					__func__, devp->pll_flg);
			mutex_unlock(&devp->pll_mutex);
			break;
		}
	...
	case ADC_DTV_DEMODPLL:
		...
		if (devp->pll_flg & ADC_DTV_DEMODPLL) {
			if (devp->print_en & ADC_DBG_EN)
				pr_info("%s:DTV DEMODPLL exist!:%d\n",
					__func__, devp->pll_flg);
			mutex_unlock(&devp->pll_mutex);
			break;
		}
...
void adc_config_cvbsout(u8 sel)
{
	...
	adc_wr_hiu(adc_addr->vdac_cntl_0, cntl0_val);
...
static const struct adc_platform_data_s adc_data_t6w = {
	ADC_ADDR_T6W,
	ADC_PLL_ADDR_T6W,
	.chip_id = ADC_CHIP_T6W,
	.is_tv_chip = true,
};

static const struct adc_platform_data_s adc_data_t6x = {
	ADC_ADDR_T6X,
	ADC_PLL_ADDR_T6X,
...
static const struct of_device_id adc_dt_match[] = {
	...
	{
		.compatible = "amlogic, adc-t6w",
		.data = &adc_data_t6w,
	},
	{
		.compatible = "amlogic, adc-t6x",
		.data = &adc_data_t6x,
	},
#endif
	{
