...
u8 *vdin_vmap(ulong addr, u32 size)
{
	...
	if (!vaddr) {
		pr_debug("the phy(%lx) vmap fail, size: %d\n",
...
{
	if (devp->debug.vdin_dbg_en & VDIN_DBG_CNTL_FLUSH)
 ...
 */
static void vdin0_wr_mif_reset(struct vdin_dev_s *devp)
{
	if (devp->vsync_reset_mask & 0x08) {
 ...
 */
static void vdin1_wr_mif_reset(struct vdin_dev_s *devp)
{
	if (devp->vsync_reset_mask & 0x08) {
...
void vdin_get_format_convert(struct vdin_dev_s *devp)
{
	...
	if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_CFMT)
			...
			if (devp->prop.dest_cfmt == TVIN_NV21) {
				...
				if (manual_md &&
				    (devp->prop.dest_cfmt == TVIN_RGB444 ||
				     devp->prop.dest_cfmt == TVIN_RGBA8888))
					format_convert =
	...
	if (vdin_is_convert_to_rgb(format_convert) && devp->prop.dest_cfmt == TVIN_RGBA8888)
		devp->local_var.is_rgba_en = true;
	else
		devp->local_var.is_rgba_en = false;
		       ...
		       unsigned int *r, unsigned int *g, unsigned int *b)
{
	...
	*b = rd_bits(offset, VDIN_MATRIX_PROBE_COLOR,
				  COMPONENT2_PROBE_COLOR_BIT,
				  COMPONENT0_PROBE_COLOR_WID);
	*g = rd_bits(offset, VDIN_MATRIX_PROBE_COLOR,
				  COMPONENT1_PROBE_COLOR_BIT,
				  COMPONENT1_PROBE_COLOR_WID);
	*r = rd_bits(offset, VDIN_MATRIX_PROBE_COLOR,
...
#ifndef CONFIG_AMLOGIC_ZAPPER_CUT
	if (is_meson_s5_cpu()) {
		vdin_prob_get_yuv_s5(offset, y, u, v);
		return;
	}
#endif

	*v = ((rd_bits(offset, VDIN_MATRIX_PROBE_COLOR,
			      COMPONENT2_PROBE_COLOR_BIT,
			      COMPONENT2_PROBE_COLOR_WID) << 8) >> 10);
	*u = ((rd_bits(offset, VDIN_MATRIX_PROBE_COLOR,
			      COMPONENT1_PROBE_COLOR_BIT,
			      COMPONENT1_PROBE_COLOR_WID) << 8) >> 10);
	*y = ((rd_bits(offset, VDIN_MATRIX_PROBE_COLOR,
			      COMPONENT0_PROBE_COLOR_BIT,
 ...
 *a.devp->debug.rgb_info_x	b.devp->debug.rgb_info_y
 *debug usage:
		      ...
		      unsigned int x, unsigned int y, struct vdin_dev_s *devp)
{
	...
	/* set position */
	devp->debug.rgb_info_x = x;
	if (devp->fmt_info_p->scan_mode == TVIN_SCAN_MODE_INTERLACED)
		devp->debug.rgb_info_y = y / 2;
	else
		devp->debug.rgb_info_y = y;
	/* #if defined(VDIN_V1) */
	wr_bits(offset, VDIN_MATRIX_PROBE_POS, devp->debug.rgb_info_y,
		PROBE_POX_Y_BIT, PROBE_POX_Y_WID);
	wr_bits(offset, VDIN_MATRIX_PROBE_POS, devp->debug.rgb_info_x,
		  ...
		  enum bt_path_e bt_path)
{
	...
	if (vdin_dv_is_need_tunnel(devp) && devp->dtdata->hw_ver != VDIN_HW_T6X) {
		vdin_data_bus_0 = VDIN_MAP_BPB;
		vdin_data_bus_1 = VDIN_MAP_Y_G;
	...
	vdin_set_scl_mode(devp, devp->dv_hw5.hw5_ctl & BIT0);

	if (devp->dtdata->hw_ver == VDIN_HW_T6X &&
		devp->hw_core == VDIN_HW_CORE_NORMAL) {
		/* vdin_preproc */
		wr(0, VPU_VDIN_HDMI0_CTRL0_T6X, /* VPU_VDIN_HDMI0_CTRL0 */
			(1 << 0) | /* reg_hdmi_en */
			(0 << 2) | /* reg_in_ppc,hdmi rx */
			(1 << 4) | /* reg_out_ppc,2ppc */
			(2 << 6));/* reg_dsc_ppc,444 */
		wr(0, VPU_VDIN_HDMI0_CTRL1_T6X, /*VPU_VDIN_HDMI0_CTRL1*/
			(0 << 0) |  /* xxx */
			(100 << 8));/* reg_rdwin_auto */
		wr(0, VPU_VDIN_HDMI1_CTRL1_T6X, /*VPU_VDIN_HDMI1_CTRL1*/
			(0 << 0) |  /* xxx */
			(100 << 8));/* reg_rdwin_auto */
		if (devp->h_skip_en)
			wr_bits(0, VPU_VDIN_HDMI0_CTRL1_T6X, 1, 4, 2); /* reg_hskip_mode */
		if (devp->v_skip_en)
			wr_bits(0, VPU_VDIN_HDMI0_CTRL1_T6X, 1, 7, 1); /* reg_vskip_en */
		if (devp->v_active >= 4320) {
			wr_bits(0, VPU_VDIN_HDMI0_CTRL1_T6X, 0, 30, 1);
		} else {
			wr_bits(0, VPU_VDIN_HDMI0_CTRL1_T6X, devp->pre_prop.up_sample_en, 30, 1);
			wr_bits(0, VPU_VDIN_HDMI0_CTRL1_T6X, devp->pre_prop.up_sample_en, 29, 1);
		}
	}
}
...
void vdin_set_decimation(struct vdin_dev_s *devp)
{
	...
	wr(offset, VDIN_WIDTHM1I_WIDTHM1O, ((devp->h_active - 1) << 16) | (devp->h_active  - 1));
...
void vdin_change_matrix1(u32 offset, u32 matrix_csc)
{
	...
	if (matrix_csc == VDIN_MATRIX_NULL)	{
		...
		wr(offset, VDIN_MATRIX_COEF20_21, matrix_tbl->coef20_21);
		wr(offset, VDIN_MATRIX_COEF22, matrix_tbl->coef22);
		if (devp && devp->dtdata->hw_ver == VDIN_HW_T6X) {
	...
	enum vdin_format_convert_e format_convert)
{
	...
	/*struct vdin_matrix_lup_s *matrix_tbl;*/
	struct tvin_format_s *fmt_info = devp->fmt_info_p;
	unsigned int vdin_hdr_flag = devp->prop.vdin_hdr_flag | devp->dv.dv_flag;
	...
	case VDIN_FORMAT_CONVERT_YUV_NV21:
		if ((fmt_info->scan_mode == TVIN_SCAN_MODE_PROGRESSIVE &&
			...
			else if (devp->color_range_mode == 0 &&
				devp->prop.color_fmt_range == TVIN_YUV_FULL)
				matrix_csc = VDIN_MATRIX_YUV709F_YUV709;
		} else {
			if (devp->color_range_mode == 1) {
				if (devp->prop.color_fmt_range == TVIN_YUV_FULL)
					matrix_csc =
						VDIN_MATRIX_YUV601F_YUV709F;
				else
	...
	if (matrix_sel == VDIN_SEL_MATRIX0)
		...
		vdin_change_matrix1(devp->addr_offset, matrix_csc);
	else if (matrix_sel == VDIN_SEL_MATRIX_HDR)
		vdin_change_matrix_hdr(devp->addr_offset, matrix_csc);
	else
		matrix_csc = VDIN_MATRIX_NULL;
...
void vdin_set_matrix(struct vdin_dev_s *devp)
{
	...
	if (devp->debug.rgb_info_enable == 0) {
		/* matrix1 disable */
		wr_bits(offset, VDIN_MATRIX_CTRL, 0,
			VDIN_MATRIX1_EN_BIT, VDIN_MATRIX1_EN_WID);
		      ...
		      enum vdin_format_convert_e csc)
{
	...
	if (devp->debug.vdin_dbg_en)
		pr_info("matrix_id=%d, convert_id=%d, csc_id=%d", id, csc, devp->csc_idx);
}
				      ...
				      unsigned int ve)
{
	...
	if (hs < he && vs < ve) {
		...
		if (devp->dtdata->hw_ver == VDIN_HW_T6X) {
			/* win_hs */
			wr_bits(devp->addr_offset, VDIN_HIST_H_START_END_T6X, hs,
				HIST_HSTART_BIT, HIST_HSTART_WID);
			/* win_he */
			wr_bits(devp->addr_offset, VDIN_HIST_H_START_END_T6X, he,
				HIST_HEND_BIT, HIST_HEND_WID);
			/* win_vs */
			wr_bits(devp->addr_offset, VDIN_HIST_V_START_END_T6X, vs,
				HIST_VSTART_BIT, HIST_VSTART_WID);
			/* win_ve */
			wr_bits(devp->addr_offset, VDIN_HIST_V_START_END_T6X, ve,
				    ...
				    unsigned int source_bitdepth)
{
	...
	if (devp->dtdata->hw_ver == VDIN_HW_T6X)
		offset = offset + VDIN0_WRMIF_REG_OFFSET_T6X;
	...
	if (devp->dtdata->hw_ver == VDIN_HW_T6W || devp->dtdata->hw_ver == VDIN_HW_T6X) {
		if (write_fmt == MIF_FMT_NV12_21) {
			wr_bits(offset, VDIN0_WRMIF_CHRM_X, (h - 1) >> 1, 16, 13);
			/* vlsi suggest */
			wr_bits(offset, VDIN0_WRMIF_CHRM_Y, (v - 1) >> 1, 16, 13);
			wr_bits(offset, VDIN0_WRMIF_CTRL1, 1, 25, 1);
		} else {
			wr_bits(offset, VDIN0_WRMIF_CHRM_X, (h - 1), 16, 13);
			/* vlsi suggest */
			wr_bits(offset, VDIN0_WRMIF_CHRM_Y, (v - 1), 16, 13);
		}
		wr_bits(offset, VDIN0_WRMIF_LUMA_X, (h - 1), 16, 13);
		/* win_ve */
		wr_bits(offset, VDIN0_WRMIF_LUMA_Y, (v - 1), 16, 13);
	...
	if (write_fmt == MIF_FMT_NV12_21) {
		if (devp->dtdata->hw_ver == VDIN_HW_T6W || devp->dtdata->hw_ver == VDIN_HW_T6X) {
			/* swap_cbcr */
			wr_bits(offset, VDIN0_WRMIF_CTRL1,
				devp->parm.h_reverse ^ !swap_cbcr,
				24, 1);
			...
			/* swap_cbcr */
			wr_bits(offset, VDIN_WRMIF_CTRL1,
				devp->parm.h_reverse ^ !swap_cbcr,
				24, 1);
			/* output even lines's cbcr */
		...
		/* output all cbcr */
		if (devp->dtdata->hw_ver == VDIN_HW_T6W || devp->dtdata->hw_ver == VDIN_HW_T6X)
			wr_bits(offset, VDIN0_WRMIF_CTRL1, 3,
				22, VCONV_MODE_WID);
		else if (devp->dtdata->hw_ver == VDIN_HW_T6D)
		...
		if (devp->dtdata->hw_ver == VDIN_HW_T6W || devp->dtdata->hw_ver == VDIN_HW_T6X)
			/* swap_cbcr */
			wr_bits(offset, VDIN0_WRMIF_CTRL1, 0, 24, SWAP_CBCR_WID);
		else if (devp->dtdata->hw_ver == VDIN_HW_T6D)
			/* swap_cbcr */
	...
	if (devp->dtdata->hw_ver == VDIN_HW_T6W || devp->dtdata->hw_ver == VDIN_HW_T6X)
		wr_bits(offset, VDIN0_WRMIF_CTRL1, t6d_wrmif_fmt, 26, 2);
	else if (devp->dtdata->hw_ver == VDIN_HW_T6D)
	...
	if (devp->set_canvas_manual == 1 || devp->cfg_dma_buf ||
		...
		if (devp->dtdata->hw_ver == VDIN_HW_T6W || devp->dtdata->hw_ver == VDIN_HW_T6X) {
			...
			/*little endian*/
			wr_bits(offset, VDIN0_WRMIF_LUMA_CTRL0, 1,
		...
		if (devp->dtdata->hw_ver == VDIN_HW_T6W || devp->dtdata->hw_ver == VDIN_HW_T6X) {
			...
			wr_bits(offset, VDIN0_WRMIF_LUMA_CTRL0, 0,
				T6D_WR_ENDIAN_BIT, T6D_WR_ENDIAN_WID);
			wr_bits(offset, VDIN0_WRMIF_CHRM_CTRL0, 1,
				T6D_SWAP64_BIT, T6D_WR_ENDIAN_WID);
			wr_bits(offset, VDIN0_WRMIF_CHRM_CTRL0, 0,
...
{
	bool reg_pix_separate = 0;
	unsigned int wrmif_ctrl1_reg = 0;
	enum vdin_mif_fmt write_fmt = MIF_FMT_YUV422;
	unsigned int swap_cbcr = 0;
	...
	if (devp->dtdata->hw_ver == VDIN_HW_T6D)
		wrmif_ctrl1_reg = VDIN_WRMIF_CTRL1;
	else if (devp->dtdata->hw_ver == VDIN_HW_T6W || devp->dtdata->hw_ver == VDIN_HW_T6X)
		wrmif_ctrl1_reg = VDIN0_WRMIF_CTRL1;
	...
	if (rdma_enable) {
		if (devp->dtdata->hw_ver == VDIN_HW_T6W || devp->dtdata->hw_ver == VDIN_HW_T6X) {
			...
			rdma_write_reg(devp->rdma_handle,
				VDIN_WRMIF_LUMA_CTRL1 + devp->addr_offset, stride_luma);
			rdma_write_reg(devp->rdma_handle,
				VDIN_WRMIF_CHRM_CTRL1 + devp->addr_offset, stride_chroma);
			rdma_write_reg_bits(devp->rdma_handle,
			...
			rdma_write_reg(devp->rdma_handle,
				VDIN_WR_STRIDE_LUMA + devp->addr_offset, stride_luma);
			rdma_write_reg(devp->rdma_handle,
				VDIN_WR_STRIDE_CHROMA + devp->addr_offset, stride_chroma);
			rdma_write_reg_bits(devp->rdma_handle,
...
#endif
		if (devp->dtdata->hw_ver == VDIN_HW_T6W || devp->dtdata->hw_ver == VDIN_HW_T6X) {
			wr(offset, VDIN0_WRMIF_LUMA_CTRL1, stride_luma);
			wr(offset, VDIN0_WRMIF_CHRM_CTRL1, stride_chroma);
			wr_bits(offset, VDIN0_WRMIF_CTRL1, t6d_wrmif_fmt,
				26, 2);
			wr_bits(offset, VDIN0_WRMIF_CTRL1, hconv_mode,
				20, HCONV_MODE_WID);
			wr_bits(offset, VDIN0_WRMIF_CTRL1, vconv_mode,
				22, VCONV_MODE_WID);
			wr_bits(offset, VDIN0_WRMIF_CTRL1, !swap_cbcr,
				24, SWAP_CBCR_WID);
			wr_bits(offset, VDIN0_WRMIF_CTRL1, reg_pix_separate, 25, 1);
		} else if (devp->dtdata->hw_ver == VDIN_HW_T6D) {
			wr(offset, VDIN_WRMIF_LUMA_CTRL1, stride_luma);
			wr(offset, VDIN_WRMIF_CHRM_CTRL1, stride_chroma);
			...
			wr_bits(offset, VDIN_WRMIF_CTRL1, !swap_cbcr,
				24, SWAP_CBCR_WID);
			wr_bits(offset, VDIN_WRMIF_CTRL1, reg_pix_separate, 25, 1);
		} else {
			wr(offset, VDIN_WR_STRIDE_LUMA, stride_luma);
			wr(offset, VDIN_WR_STRIDE_CHROMA, stride_chroma);
...
#ifndef CONFIG_AMLOGIC_ZAPPER_CUT
	if (is_meson_s5_cpu() || is_meson_t3x_cpu() || is_meson_t6w_cpu() || is_meson_t6x_cpu())
		return;/*t6w t6x todo*/
#endif
...
void vdin_set_mif_on_off(struct vdin_dev_s *devp, unsigned int rdma_enable)
{
	...
	if (rdma_enable) {
		if (devp->dtdata->hw_ver == VDIN_HW_T6W || devp->dtdata->hw_ver == VDIN_HW_T6X) {
			rdma_write_reg_bits(devp->rdma_handle, VDIN0_WRMIF_FRM_EN_CTRL + offset,
...
unsigned int vdin_get_meas_v_stamp_t6x(struct vdin_dev_s *devp)
{
	u64 low_cnt, high_cnt;
	u64 stamp = 0;

	low_cnt  = rd(0, VDIN_MEAS_VS_COUNT_LO);
	high_cnt = rd(0, VDIN_MEAS_VS_COUNT_HI) & 0x1;

	if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_CYCLE)
		pr_info("low_cnt = %#x,high_cnt = %#x\n",
			(u32)low_cnt, (u32)high_cnt);
	stamp = ((high_cnt << 32) | low_cnt) / 2;
...
unsigned int vdin_get_meas_v_stamp(struct vdin_dev_s *devp)
{
	...
	else if (is_meson_t6x_cpu()) /* Calculate based on 50M */
		return vdin_get_meas_v_stamp_t6x(devp);
	else
#endif
			...
			struct vf_entry *vfe)
{
	...
	if (rdma_enable) {
		if (devp->dtdata->hw_ver == VDIN_HW_T6W || devp->dtdata->hw_ver == VDIN_HW_T6X) {
			rdma_write_reg(devp->rdma_handle,
				VDIN0_WRMIF_LUMA_BADDR + offset,
				phy_addr_luma >> 4);
			rdma_write_reg(devp->rdma_handle,
				       VDIN0_WRMIF_LUMA_CTRL1 + offset,
				       stride_luma);
		...
		if (vfe->vf.plane_num == 2) {
			if (devp->dtdata->hw_ver == VDIN_HW_T6W ||
				...
				rdma_write_reg(devp->rdma_handle,
					VDIN0_WRMIF_CHRM_BADDR + offset,
					phy_addr_chroma >> 4);
				rdma_write_reg(devp->rdma_handle,
					       VDIN0_WRMIF_CHRM_CTRL1 + offset,
					       stride_chroma);
			} else if (devp->dtdata->hw_ver == VDIN_HW_T6D) {
				rdma_write_reg(devp->rdma_handle,
		...
		if (devp->dtdata->hw_ver == VDIN_HW_T6W || devp->dtdata->hw_ver == VDIN_HW_T6X)
			rdma_write_reg_bits(devp->rdma_handle,
				VDIN0_WRMIF_FRM_EN_CTRL + offset, !pause_en, 15, 1);
	} else {
		if (devp->debug.vdin_dbg_en)
			pr_info("%s,phy_addr_luma:0x%lx,stride_luma:%d\n",
				__func__, phy_addr_luma, stride_luma);
		if (devp->dtdata->hw_ver == VDIN_HW_T6W || devp->dtdata->hw_ver == VDIN_HW_T6X) {
			wr(offset, VDIN0_WRMIF_LUMA_BADDR, phy_addr_luma >> 4);
			wr(offset, VDIN0_WRMIF_LUMA_CTRL1, stride_luma);
		} else if (devp->dtdata->hw_ver == VDIN_HW_T6D) {
		...
		if (vfe->vf.plane_num == 2) {
			if (devp->dtdata->hw_ver == VDIN_HW_T6W ||
			    devp->dtdata->hw_ver == VDIN_HW_T6X) {
				wr(offset, VDIN0_WRMIF_CHRM_BADDR,
					phy_addr_chroma >> 4);
				wr(offset, VDIN0_WRMIF_CHRM_CTRL1, stride_chroma);
				wr_bits(offset, VDIN0_WRMIF_CTRL1, 1, 25, 1);
			} else if (devp->dtdata->hw_ver == VDIN_HW_T6D) {
			...
			struct vf_entry *vfe)
{
	...
	if (is_meson_s5_cpu()) {
		...
		return;
	} else if (is_meson_t6d_cpu() || is_meson_t6w_cpu() || is_meson_t6x_cpu()) {
		return;
	}
#endif
			       ...
			       struct vdin_dev_s *devp)
{
	 ...
	 */
	if (devp->dtdata->hw_ver == VDIN_HW_T6X) {
		vf->prop.hist.chroma_sum = rd(offset, 0x1299);
		vf->prop.hist.pixel_sum  = rd_bits(offset, 0x1298, 0, 24);
		vf->prop.hist.luma_sum   = rd(offset, VDIN_HIST_SPL_VAL_T6X);
		vf->prop.hist.height     = rd_bits(offset, VDIN_HIST_V_START_END_T6X,
						   HIST_VEND_BIT, HIST_VEND_WID) -
			rd_bits(offset, VDIN_HIST_V_START_END_T6X,
				HIST_VSTART_BIT, HIST_VSTART_WID) + 1;
		vf->prop.hist.width      = rd_bits(offset, VDIN_HIST_H_START_END_T6X,
						   HIST_HEND_BIT, HIST_HEND_WID) -
					rd_bits(offset, VDIN_HIST_H_START_END_T6X,
						HIST_HSTART_BIT, HIST_HSTART_WID) + 1;
		vf->prop.hist.luma_max   = rd_bits(offset, 0x1296,
					   HIST_MAX_BIT, HIST_MAX_WID);
		vf->prop.hist.luma_min   = rd_bits(offset, 0x1296,
	...
	if ((devp->debug.vdin_ctl_dbg & CTL_DEBUG_LUMA_MAX) &&
	...
	if (devp->prop.qms_plus_flag)
		vf->type_ext |= VIDTYPE_EXT_VDIN_QMS_PLUS;
}
...
#ifndef CONFIG_AMLOGIC_ZAPPER_CUT
	if (is_meson_s5_cpu() || is_meson_t3x_cpu() || is_meson_t6x_cpu())
		return;
...
void vdin_set_dv_tunnel(struct vdin_dev_s *devp)
{
	...
	if (vdin_dv_is_need_tunnel(devp)) {
		...
		/* t6x_rx_tunnel VPU_VDIN_HDMI0_TUNNEL 0x274f*/
		if (devp->dtdata->hw_ver == VDIN_HW_T6X)
			wr(0, VPU_VDIN_HDMI0_TUNNEL_T6X, 0x80304512);
		else	/*hdmi rx call back, 422 tunnel to 444*/
			sm_ops->hdmi_dv_config(true, devp->frontend, devp->port_type);
		pr_info("dv rx tunnel mode\n");
	} else {
		if (devp->dtdata->hw_ver == VDIN_HW_T6X)
			wr(0, VPU_VDIN_HDMI0_TUNNEL_T6X, 0x0);
		else	/*hdmi rx call back, 422 tunnel to 444*/
...
void vdin_set_default_regmap(struct vdin_dev_s *devp)
{
	...
	if (cpu_after_eq(MESON_CPU_MAJOR_ID_TM2)) {
		...
		if (devp->index == 0) {
			...
			wr_bits(0, VDIN_TOP_DOUBLE_CTRL, WR_SEL_VDIN0_NOR,
				MIF0_OUT_SEL_BIT, VDIN_REORDER_SEL_WID);
			/* enable frm end interrupt */
			if (devp->dtdata->hw_ver == VDIN_HW_T6W ||
				devp->dtdata->hw_ver == VDIN_HW_T6X) {
				/* bit0-vfce,bit1-wrmif0,bit2-wrmif1,bit3-wrmif2 */
				wr_bits(0, VDIN_TOP_DOUBLE_CTRL, 0xc,
					VDIN0_INT_MASK_BIT, VDIN_INT_MASK_WID);
				/* vdin0 frm_end int enable */
				wr_bits(0, VDIN_TOP_SIZE, 0x1, 25, 1);
				//wr_bits(0, VDIN_TOP_DOUBLE_CTRL, 0xd,
				//	VDIN1_INT_MASK_BIT, VDIN_INT_MASK_WID);
				/* vdin1 frm_end int enable */
				//wr_bits(0, VDIN_TOP_DOUBLE_CTRL, 0x1, 26, 1);
			}
			if (devp->dtdata->hw_ver != VDIN_HW_T6W &&
			    devp->dtdata->hw_ver != VDIN_HW_T6X)
	...
	wr_bits(offset, VDIN_INTF_WIDTHM1, (devp->h_active_org - 1),
		VDIN_INTF_WIDTHM1_BIT, VDIN_INTF_WIDTHM1_WID);
	...
	if (devp->dtdata->hw_ver == VDIN_HW_T6W || devp->dtdata->hw_ver == VDIN_HW_T6X) {
		...
		wr(offset, VDIN_IN_SIZE, devp->h_active_org | (devp->v_active_org << 16));
	} else if (devp->dtdata->hw_ver == VDIN_HW_T6D) {
	...
	if (devp->dtdata->hw_ver == VDIN_HW_TXHD2) {
		...
		if (devp->hw_core == VDIN_HW_CORE_LITE) {
			...
			if (devp->dtdata->hw_ver == VDIN_HW_T6X) {
				wr(0, VDIN_HIST_H_START_END_T6X, devp->h_active_org - 1);
				wr(0, VDIN_HIST_V_START_END_T6X, devp->v_active_org - 1);
			} else {
...
static void filter_unstable_vsync(struct vdin_dev_s *devp)
{
	...
	if (!cpu_after_eq(MESON_CPU_MAJOR_ID_TM2) || is_meson_s6_cpu() ||
		is_meson_t6d_cpu() || is_meson_t6w_cpu() || is_meson_t6x_cpu())
		return;
#endif
...
void vdin_dlg_update_hist_hv(unsigned int h, unsigned int v, unsigned int fps)
{
	...
	if (devp->vdin_function_sel & VDIN_MUX_VDIN0_HIST ||
		devp->dtdata->hw_ver == VDIN_HW_TXHD2 || devp->dtdata->hw_ver >= VDIN_HW_T6D)
...
void vdin_get_hist_val(struct vdin_dev_s *devp, struct vdin_hist_s *vdin1_hist_temp)
{
	...
	if (devp->dtdata->hw_ver == VDIN_HW_T6X) {
		vdin1_hist_temp->width = (rd(offset, VDIN_HIST_H_START_END_T6X) & 0x1fff) + 1;
		vdin1_hist_temp->height = (rd(offset, VDIN_HIST_V_START_END_T6X) & 0x1fff) + 1;
		vdin1_hist_temp->sum = rd(offset, VDIN_HIST_SPL_VAL_T6X);
	} else {
...
void vdin_hist_init(struct vdin_dev_s *devp)
{
	...
	if (devp->dtdata->hw_ver == VDIN_HW_TXHD2) {
		...
		if (devp->vdin_function_sel & VDIN_MUX_VDIN0_HIST)
			...
			wr_bits(0, VDIN_TOP_MISC, 0, 12, 1);
		wr_bits(offset, VDIN_LFIFO_CTRL, 1, 28, 1);//discard data enable
		wr(offset, VDIN_ASFIFO_CTRL1, 0xa0e4); //vlsi
	} else {
		wr_bits(offset, VDIN_WR_CTRL2, 1,
			DISCARD_BEF_LINE_FIFO_BIT, DISCARD_BEF_LINE_FIFO_WID);
		wr(offset, VDIN_ASFIFO_CTRL1, 0x80e4); //vlsi
	...
	if (devp->dtdata->hw_ver == VDIN_HW_T6X &&
		...
		wr(offset, VDIN_INTF_WIDTHM1, (vinfo->width / 2) - 1);
	else
	...
	if (devp->dtdata->hw_ver == VDIN_HW_T6X) {
		/* win_hs */
		wr_bits(hist_offset, VDIN_HIST_H_START_END_T6X, 0,
			HIST_HSTART_BIT, HIST_HSTART_WID); //1231
		/* win_he */
		if (vinfo->width * vinfo->height * vinfo->std_duration >
			VDIN_LITE_CORE_MAX_PIXEL_CLOCK)
			wr_bits(hist_offset, VDIN_HIST_H_START_END_T6X,
				(vinfo->width / 2) - 1,
				HIST_HEND_BIT, HIST_HEND_WID);
		else
			wr_bits(hist_offset, VDIN_HIST_H_START_END_T6X,
				...
				HIST_HEND_BIT, HIST_HEND_WID);
		/* win_vs */
		wr_bits(hist_offset, VDIN_HIST_V_START_END_T6X, 0,
			HIST_VSTART_BIT, HIST_VSTART_WID); //1232
		/* win_ve */
		wr_bits(hist_offset, VDIN_HIST_V_START_END_T6X, vinfo->height - 1,
			HIST_VEND_BIT, HIST_VEND_WID);
	} else {
...
void vdin_ioctl_get_hist(struct vdin_dev_s *devp, struct vdin_hist_s *vdin1_hist_temp)
{
	...
	if (devp->debug.vdin_dbg_en & DBG_VDIN1_HIST) {
...
inline int vdin_vsync_reset_mif(struct vdin_dev_s *devp)
{
	int i;
	int start_line;
	...
	if (!devp->enable_reset || vdin_reset_flag || start_line > 0)
		return 0;
	...
	if (devp->index == 0) {
		...
		if (R_VCBUS_BIT(VPU_ARB_DBG_STAT_L1C2, VDIN_DET_IDLE_BIT,
			...
			vdin0_wr_mif_reset(devp);
		} else {
			for (i = 0; i < devp->dts_config.vdin_det_idle_wait; i++) {
			...
			if (i >= devp->dts_config.vdin_det_idle_wait && devp->debug.vdin_ctl_dbg)
	...
	return devp->vsync_reset_mask & 0x08;
}
	...
	if (devp->dtdata->hw_ver == VDIN_HW_T6X) {
		...
		if (dst_w != src_w)
			wr(offset, VDIN_SC_MISC_CTRL,
			   ...
			   (1 << 25) | /* HSCL_RPT_P0_NUM_BIT */
			   (4 << 21) | /* hsc_bank_length */
			   (0 << INIT_PIX_IN_PTR_BIT) |
			   (1 << HSCL_EN_BIT) | /* hsc_en */
			   (0 << SHORT_LN_OUT_EN_BIT) | /* short_line_o_en */
			   (0 << HSCL_NEAREST_EN_BIT) | /* nearest_en */
			   (0 << PHASE0_ALWAYS_EN_BIT) /* phase0_always_en */
			   );
		wr(offset, VDIN_HSC_MISC_CTRL,
			(7 << 20) | /* hsc_nor_rs_bits */
			(4 << 16) | /* hsc_ini_rcv_num */
			((dst_w  - 1) << 0));
		wr(offset, VDIN_HSC_REGION34_STARTP,
			(dst_w << 16) | /* hsc_ini_rcv_num */
			(dst_w << 0));
	} else {
		/* hsc_p0_num */
...
static void vdin_set_h_aa_ds(struct vdin_dev_s *devp)
{
	...
	unsigned int aa_ds_x_rate = 0, aa_ds_x_ratio = 0, wbuf_mode = 0;

	if (devp->dtdata->hw_ver == VDIN_HW_T6X)
		offset = offset + VDIN0_AA_DS_REG_OFFSET_T6X;

	if (dst_w)
		aa_ds_x_rate = src_w / dst_w;

	/* check legality, valid is 1,2,4,8 */
	switch (aa_ds_x_rate) {
	case 1:
		aa_ds_x_ratio = 0;
		break;
	...
	if (aa_ds_x_rate == 1 || aa_ds_x_rate == 2) {
		// for 7tap, max_hsize=960
		wr_bits(offset, VDIN0_AA_DS_CTRL, wbuf_mode, 21, 1);
	...
	/* aa_ds_x in_hsize */
	wr_bits(devp->addr_offset, VDIN_VSHRK_SIZE_M1, src_w - 1, 0, 16);
	/* aa_ds_x enable */
	wr_bits(devp->addr_offset, VDIN_LFIFO_CTRL, aa_ds_x_ratio, 19, 2);
	pr_info("vdin%d %s done! rate/ratio = %d/%d\n",
		devp->index, __func__, aa_ds_x_rate, aa_ds_x_ratio);
}
...
static void vdin_set_h_shrink(struct vdin_dev_s *devp)
{
	...
	/* Only t6w/t6x vdin0 has aa downsample */
	if ((devp->dtdata->hw_ver == VDIN_HW_T6W || devp->dtdata->hw_ver == VDIN_HW_T6X) &&
		devp->hw_core == VDIN_HW_CORE_NORMAL) {
		vdin_set_h_aa_ds(devp);
		return;
	}
...
/* t6w newly added */
static void vdin_set_v_aa_ds(struct vdin_dev_s *devp)
{
	unsigned int offset = devp->addr_offset;
	unsigned int src_h = devp->v_active;
	unsigned int dst_h = devp->v_shrink_out;
	unsigned int dst_w = devp->h_shrink_out;
	unsigned int aa_ds_y_rate = 0, aa_ds_y_ratio = 0;

	if (devp->dtdata->hw_ver == VDIN_HW_T6X)
		offset = offset + VDIN0_AA_DS_REG_OFFSET_T6X;

	if (dst_h)
		aa_ds_y_rate = src_h / dst_h;
	...
	} else if (aa_ds_y_rate == 1 || aa_ds_y_rate == 2) {
		// for 7tap, max_hsize=960
	...
	/* aa_ds_x in_vsize */
	wr_bits(devp->addr_offset, VDIN_VSHRK_SIZE_M1, src_h - 1, 16, 16);
	/* aa_ds_x out_vsize */
	wr_bits(devp->addr_offset, VDIN_SCIN_HEIGHTM1, dst_h - 1, 16, 13);
	/* aa_ds_y enable */
	wr_bits(devp->addr_offset, VDIN_LFIFO_CTRL, aa_ds_y_ratio, 21, 2);
	pr_info("vdin%d %s done! rate/ratio = %d/%d\n",
		devp->index, __func__, aa_ds_y_rate, aa_ds_y_ratio);
}
...
void vdin_set_bitdepth(struct vdin_dev_s *devp, unsigned int rdma_enable)
{
	...
	unsigned int convert_fmt, reg_bit_depth = 0;
	...
	if (devp->dtdata->hw_ver == VDIN_HW_T3X) {
	#ifndef CONFIG_AMLOGIC_ZAPPER_CUT
		vdin_set_bitdepth_t3x(devp, rdma_enable);
	#endif
	} else if (devp->dtdata->hw_ver == VDIN_HW_S5) {
	#ifndef CONFIG_AMLOGIC_ZAPPER_CUT
		vdin_set_bitdepth_s5(devp, rdma_enable);
	#endif
		...
		if (rdma_enable) {
			rdma_write_reg_bits(devp->rdma_handle,
				VDIN0_WRMIF_CTRL1 + offset,
				reg_bit_depth, T6D_WRMIF_10BIT_MODE_BIT, T6D_WRMIF_10BIT_MODE_WID);
		} else {
			wr_bits(offset, VDIN0_WRMIF_CTRL1, reg_bit_depth,
		...
		if (rdma_enable) {
			...
			wr_bits(offset, VDIN_WRMIF_CTRL1, reg_bit_depth,
				T6D_WRMIF_10BIT_MODE_BIT, T6D_WRMIF_10BIT_MODE_WID);
		}
	} else {
		if (rdma_enable) {
			...
			wr_bits(offset, VDIN_WR_CTRL2, reg_bit_depth,
				VDIN_WR_10BIT_MODE_BIT, VDIN_WR_10BIT_MODE_WID);
		}
	}

	if (devp->debug.vdin_dbg_en)
		pr_info("%s bitdepth:%d %d,unique_drm:%d,cfg:0x%x prop.dep:%x\n", __func__,
...
void vdin_set_to_vpp_parm(struct vdin_dev_s *devp)
{
	...
	if (IS_TVAFE_ATV_SRC(devp->parm.port))
		devp->vdin2vpp_info.tuner_id = devp->tuner_id;
	else
		devp->vdin2vpp_info.tuner_id = -1;
...
void vdin_set_cm2(unsigned int offset, unsigned int w,
		  unsigned int h, unsigned int *cm2, bool cm_enable)
{
	unsigned int i = 0, j = 0, start_addr = 0x100;

#ifndef CONFIG_AMLOGIC_ZAPPER_CUT
	if (is_meson_s5_cpu()) {
		vdin_set_cm2_s5(offset, w, h, cm2, cm_enable);
		return;
	} else if (is_meson_t3x_cpu()) {
		vdin_set_cm2_t3x(offset, w, h, cm2, cm_enable);
		return;
	}
#endif
...
{
	if (devp->debug.dv_dbg_log & DV_DEBUG_RAW_DATA)
...
{
	if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_HDR_RAW_DATA)
...
{
	if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_HDR_EMP_DATA)
...
void vdin_dolby_buffer_update(struct vdin_dev_s *devp, unsigned int index)
{
	...
	if (index >= devp->canvas_max_num) {
		if (devp->debug.dv_dbg_log & DV_DEBUG_CANVAS_NUM)
			pr_info("%s er: %d %d\n", __func__,
		...
		if (meta_size > DV_META_SINGLE_PKT_SIZE || multimeta_flag) {
			...
			for (j = 0; j < (128 * rev_cnt); j += (128 * rpt_cnt)) {
				...
				if (rpt_cnt == 0)
					break;
				if (((cnt % devp->debug.dv_dbg_log_du) == 0) &&
				    (devp->debug.dv_dbg_log & DV_DEBUG_HW_META_DATA))
	...
	if (crc != crc_r ||
		...
		if (((cnt % devp->debug.dv_dbg_log_du) == 0) &&
			(devp->debug.dv_dbg_log & DV_DEBUG_OK_META_DATA))
...
void vdin_dolby_mdata_write_en(unsigned int offset, unsigned int en)
{
	...
	if (is_meson_s5_cpu()) {
		...
		vdin_dolby_mdata_write_en_t6x(offset, en);
		return;
	}
#endif
				   ...
				   unsigned int on_off)
{
	...
	if (devp->dtdata->hw_ver == VDIN_HW_T6W ||
		...
		if (on_off) {
			if (devp->double_wr) {
				/* reg_detunnel_en */
				wr_bits(offset, VDIN_DSC_CTRL, 1, 3, 1);
				/* reg_dither_en=0 */
				wr_bits(offset, VDIN_DSC_CTRL, 0, 1, 1);
				/* /rpt pix */
				wr_bits(offset, VDIN_CFMT_CTRL, 1, 8, 1);
				/* rpt_p0_en */
...
int vdin_event_cb(int type, void *data, void *op_arg)
{
	...
	if (!data) {
		if (devp->debug.vdin_ctl_dbg & CTL_DEBUG_EVENT_DATA)
	...
	if (type & VFRAME_EVENT_RECEIVER_GET_AUX_DATA) {
		...
		if (!req->vf) {
			req->aux_size = 0;
			req->low_latency = 0;
			if (devp->debug.vdin_ctl_dbg & CTL_DEBUG_EVENT_DISP_MODE)
		...
		if (devp->debug.dv_dbg_log & DV_DEBUG_EVENT_META_DATA) {
		...
		if (index_disp >= VFRAME_DISP_MAX_NUM) {
			if (devp->debug.vdin_ctl_dbg & CTL_DEBUG_EVENT_INDEX)
				pr_info("%s:req->vf->index_disp(%d) is overflow!\n",
					__func__, index_disp);
			return -1;
		}
		disp_mode = p->disp_mode[index_disp];
		if (devp->debug.force_disp_mode)
			req->disp_mode = devp->debug.force_disp_mode - 1;
		else if (devp->frame_drop_num)
			req->disp_mode = VFRAME_DISP_MODE_SKIP;
		else if (devp->game_mode & VDIN_GAME_MODE_1_2)
		...
		if (devp->debug.vdin_ctl_dbg & CTL_DEBUG_EVENT_DISP_MODE)
			pr_info("%s(type 0x%x vf %d)disp_mode %d %d,req_mode:%d;[%d]=%d,[%d]=%d\n",
				__func__, type, index_disp,
				req->disp_mode, disp_mode, req->req_mode,
		...
		if (devp->debug.vdin_ctl_dbg & CTL_DEBUG_EVENT_NO_COMP)
		       ...
		       struct vframe_s *vf)
{
	...
	if (devp->prop.cuva_info.cuva_on)
		vf->signal_type |= (1 << 31);
	else
		vf->signal_type &= ~(1 << 31);
...
{
	if (devp->pre_prop.qms_plus_flag != devp->prop.qms_plus_flag)
		return true;
	else
		return false;
}
...
int vdin_package_done_check_state(struct vdin_dev_s *devp)
{
	...
	if (devp->debug.vdin_isr_monitor) {
		pr_info("vdin%d,dv[%d %d],hdr[%d,%d],allm[%d %d],vrr[%d %d %d %d],[%d %d],%d\n",
			...
			devp->vrr_data.vdin_vrr_en_flag, devp->prop.vtem_data.vrr_en,
...
void vdin_vs_proc_monitor(struct vdin_dev_s *devp)
{
	if (IS_HDMI_SRC(devp->parm.port) ||
		...
		if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_FLAG)
			pr_info("dv:%d, LL:%d hdr st:%d eotf:%d fg:%d allm:%d sty:0x%x spd:0x%x 0x%x\n",
				...
				devp->prop.spd_data.data[0],
				devp->prop.spd_data.data[5]);
		if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_EMP)
...
u32 vdin_get_curr_field_type(struct vdin_dev_s *devp)
{
	...
	if (devp->secure_en)
		...
		type &= ~VIDTYPE_SEC_MD;
#endif
	devp->curr_field_type_dw = type;
	if (devp->is_422_12bit_enabled) {
		type &= ~(VIDTYPE_VIU_NV21 | VIDTYPE_VIU_422 | VIDTYPE_VIU_444 | VIDTYPE_RGB_444);
		type |= VIDTYPE_VIU_422;
	}

	if (devp->debug.dbg_force_type >= 0x80000000)
		type = 0;
	else if (devp->debug.dbg_force_type)
		type = devp->debug.dbg_force_type;
				     ...
				     struct vframe_s *vf)
{
	...
	case VDIN_COLOR_DEEPS_12BIT:
		vf->bitdepth = BITDEPTH_Y12 | BITDEPTH_U12 | BITDEPTH_V12;
		break;
	...
	case VDIN_COLOR_DEEPS_12BIT:
		vf->bitdepth_dw = BITDEPTH_Y12 | BITDEPTH_U12 | BITDEPTH_V12;
		break;
	case VDIN_COLOR_DEEPS_10BIT:
...
void vdin_set_vf_reinit_flag(struct vdin_dev_s *devp)
{
	...
	for (i = 0; i < p->size; ++i) {
		...
		if (master) {
			master->flag |= VF_FLAG_NEED_UPDATE;
		} else {
...
void vdin_vf_reinit(struct vdin_dev_s *devp, struct vf_entry *vfe)
{
	...
	vfe->vf.canvas0_config[0].width = devp->canvas_w;
	/* TBC */
}
...
void vdin_bist_t6x(struct vdin_dev_s *devp, unsigned int pat)
{
	unsigned int offset = devp->addr_offset;
	unsigned int de_start = 0x4;
	unsigned int v_blank = 0xff;
	...
	if (pat) {
		if (devp->dtdata->hw_ver == VDIN_HW_T6X) {
			wr_bits(offset, VDIN_ASFIFO_CTRL3, 0xe4,
				VDI6_ASFIFO_CTRL_BIT, VDI_ASFIFO_CTRL_WID);
			wr_bits(offset, VDIN_COM_CTRL0, VDIN_MUX_VIU1_WB0,
				VDIN_SEL_BIT, VDIN_SEL_WID);
			wr(offset, VDIN_WIDTHM1I_WIDTHM1O,
			((devp->h_active_org - 1) << 16) | (devp->h_active_org - 1));
		}
		wr(offset, VDIN_SCIN_HEIGHTM1,
			((devp->v_active_org - 1) << 16) | (devp->v_active_org - 1));
		/* output_width_m1 */
		wr_bits(offset, VDIN_INTF_WIDTHM1, ((devp->v_active_org - 1) << 16) |
							(devp->h_active_org - 1),
			VDIN_INTF_WIDTHM1_BIT, VDIN_INTF_WIDTHM1_WID);
		...
		wr_bits(offset, VDIN_ASFIFO_CTRL0, 1, 31, 1);
...
void vdin_set_ip_pattern(struct vdin_dev_s *devp, unsigned int pat)
{
	...
	/* ip pattern gen */
	wr(0, VDIN0_IP_PAT_CTRL, 0x24);
	wr(0, VDIN0_IP_PAT_FG_X_RMP_RGB, 0xff0000);
	wr(0, VDIN0_IP_PAT_RECT_LOCATION, 0x160016);
	if (devp->dtdata->hw_ver == VDIN_HW_T6W) {
		wr(0, VDIN0_IP_PAT_BG_Y_RMP_RGB, 0xff00);
		wr(0, VDIN0_IP_PAT_RECT_SIZE, 0x1900190);
		wr(0, VDIN0_IP_PAT_RECT_CYCLE, 0x145);
	} else if (devp->dtdata->hw_ver == VDIN_HW_T6X) {
		wr(0, VDIN0_IP_PAT_BG_Y_RMP_RGB, 0xff);
		wr(0, VDIN0_IP_PAT_RECT_SIZE, 0x800080);
		wr(0, VDIN0_IP_PAT_RECT_CYCLE, 0x45);
	}
	wr(0, VDIN0_IP_PAT_RECT_MV, 0xb0000);
	wr(0, VDIN0_IP_PAT_RECT_FLG, 0x14);
	...
	if (pat) {
		if (devp->dtdata->hw_ver == VDIN_HW_T6W) {
			...
			wr(offset, VDIN_ASFIFO_CTRL3, 0x3a000000);
			wr_bits(offset, VDIN_COM_CTRL0, VDIN_MUX_HDMI, VDIN_SEL_BIT, VDIN_SEL_WID);
			wr(offset, VDIN_WIDTHM1I_WIDTHM1O,
			((devp->h_active_org - 1) << 16) | (devp->h_active_org - 1));
		}
		wr(offset, VDIN_SCIN_HEIGHTM1,
		...
		wr_bits(offset, VDIN_ASFIFO_CTRL0, 1,
			VDI6_BIST_EN_BIT, VDI6_BIST_EN_WID);
		/* enable IP pattern gen */
		wr_bits(offset, VDIN0_IP_PAT_CTRL, 1, 8, 1);
	} else {
		wr_bits(offset, VDIN_COM_CTRL0, VDIN_MUX_HDMI, VDIN_SEL_BIT, VDIN_SEL_WID);
		wr_bits(offset, VDIN_ASFIFO_CTRL0, 0,
			VDI6_BIST_EN_BIT, VDI6_BIST_EN_WID);
		/* disable IP pattern gen */
		wr_bits(offset, VDIN0_IP_PAT_CTRL, 0, 8, 1);
	}
}
...
enum vdin_vrr_mode_e get_cur_vrr_status(struct vdin_dev_s *devp)
{
	...
	if (!IS_HDMI_SRC(devp->parm.port)) {
		ret = VDIN_VRR_OFF;
	} else if (devp->prop.vtem_data.vrr_en) {
		ret = VDIN_VRR_BASIC;
	...
	if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_VRR_DATA)
		pr_info("%s port:0x%x vrr:%d freesync:%d game:%d allm:%d instead:%d small:%d\n",
			...
			vrr_instead_vlock(), vrr_small_window());
...
{
	if (on_off)
		wr_bits(devp->addr_offset, VDIN_LFIFO_CTRL, 0x1, 16, 1);
	else
		wr_bits(devp->addr_offset, VDIN_LFIFO_CTRL, 0x0, 16, 1);
}
