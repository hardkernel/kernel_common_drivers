...
static unsigned int g_disp_count;/* Multiple display */
...
unsigned int vdin_game_mode_dly = 3;
module_param(vdin_game_mode_dly, uint, 0664);
MODULE_PARM_DESC(vdin_game_mode_dly, "vdin_game_mode_dly");
...
void tvin_update_vdin_prop(u8 port_type, u8 pkt_type)
{
	...
	ret = vdin_package_done_check_state(devp);
	if (ret) {
		if (devp->game_mode & VDIN_GAME_MODE_1_2) {
			if ((ret & BIT1) && !devp->mute_cnt) {
				if (devp->debug.vdin_isr_monitor)
					pr_info("%s vdin%d,irq_cnt:%d,game:%d\n", __func__,
						devp->index, devp->irq_cnt, devp->game_mode);
				set_video_mute_vcbus(VDIN_MUTE_SET, true);
	...
	if (devp->debug.vdin_isr_monitor & DBG_RX_UPDATE_VDIN_PROP && update_wr_vf)
		pr_info("%s: index:%d disp:%d game:%d\n",
			__func__, update_wr_vf->index,
			update_wr_vf->index_disp, devp->game_mode);
	else if (devp->debug.vdin_isr_monitor & DBG_RX_UPDATE_VDIN_PROP)
...
static inline void vdin_game_mode_dynamic_check(struct vdin_dev_s *devp)
{
	...
	if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_GAME)
		pr_info("%s vrr_mode:%d,game cur(%#x)in fps:%d out fps:%d cycle:%#x\n",
			__func__, devp->vrr_data.vrr_mode, devp->game_mode,
...
void vdin_game_mode_reconfig(struct vdin_dev_s *devp)
{
	...
	if (!vdin_is_444_input(devp->prop.color_format) ||
	   (!game_mode && vdin_is_convert_to_422(devp->format_convert)) ||
	   (game_mode && vdin_is_convert_to_444(devp->format_convert))) {
		if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_GAME)
			pr_info("vdin%d,game:%d %d,color_format:%d\n", devp->index,
				game_mode, devp->game_mode, devp->prop.color_format);
		return;
	}

	if (devp->game_mode_chg == VDIN_GAME_MODE_CHK) {
		devp->flags |= VDIN_FLAG_DYN_RECONFIG;
		devp->game_mode_dly = devp->put_frame_cnt + vdin_game_mode_dly;
	}
	if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_GAME)
		pr_info("vdin%d,game:%d,cvt:%d,flags:%#x\n",
			devp->index, devp->game_mode, devp->format_convert, devp->flags);
}
...
void vdin_dyn_fmt(struct vdin_dev_s *devp)
{
	...
	if (!vdin_is_444_input(devp->prop.color_format)) {
		if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_GAME)
			pr_info("vdin%d,game:%d,color_format:%d\n",
				devp->index, devp->game_mode, devp->prop.color_format);
		return;
	}
	...
	unsigned int old_mode, unsigned int new_mode)
{
	...
	if (devp->flags & VDIN_FLAG_DEC_STARTED) {
		...
		game_mode = new_mode;
		vdin_game_mode_reconfig(devp);
		spin_unlock_irqrestore(&devp->isr_lock, flags);
	}
}

static bool vdin_is_vlock_locked(struct vdin_dev_s *devp)
{
	if (((vlock_get_phlock_flag() && vlock_get_vlock_flag()) || frame_lock_vrr_lock_status()) &&
		!devp->force_unlock)
		return true;
	else
		return false;
...
static void vdin_game_mode_transfer(struct vdin_dev_s *devp)
{
	...
	if (cpu_after_eq(MESON_CPU_MAJOR_ID_TL1)) {
		if (devp->game_mode & VDIN_GAME_MODE_SWITCH_EN) {
			...
			if (vdin_is_vlock_locked(devp))
				phase_lock_flag++;
			else
				phase_lock_flag = 0;
			if ((phase_lock_flag >= devp->dts_config.game_mode_phlock_switch_frames &&
				!frame_lock_type_vrr_lock()) || (frame_lock_type_vrr_lock() &&
				phase_lock_flag >= devp->dts_config.game_mode_switch_frames)) {
		...
		if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_GAME)
			pr_info("1-lock_cnt:%d, mode:%x in fps:%d out fps:%d.%d %d %d\n",
				phase_lock_flag, devp->game_mode,
				devp->vdin_std_duration, devp->vinfo_std_duration,
				vlock_get_phlock_flag(), vlock_get_vlock_flag(),
				frame_lock_vrr_lock_status());
	} else {
		if (devp->frame_cnt >= devp->dts_config.game_mode_switch_frames &&
		    (devp->game_mode & VDIN_GAME_MODE_SWITCH_EN)) {
			if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_GAME) {
	...
	if (devp->prop.vtem_data.qms_en)
		devp->game_mode = (VDIN_GAME_MODE_0 | VDIN_GAME_MODE_SWITCH_EN);
	if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_GAME)
		pr_info("%s game mode cur:0x%x\n", __func__, devp->game_mode);
}
...
static void vdin_handle_game_mode_chg(struct vdin_dev_s *devp)
{
	...
	/* game mode 1/2 changed, send vf strategy needs to be changed,recycle unused vf */
	if ((game_mode_pre & VDIN_GAME_MODE_1_2) != (devp->game_mode & VDIN_GAME_MODE_1_2)) {
		if (devp->game_mode & VDIN_GAME_MODE_2) {
	...
	devp->game_mode_chg = VDIN_GAME_MODE_UN_CHG;
	if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_GAME)
		pr_info("%s: game_mode,cfg=0x%x;pre:%#x,cur:%#x,force mode:0x%x,fps:%d,vout:%d\n",
			__func__, game_mode, game_mode_pre, devp->game_mode,
			vdin_force_game_mode, devp->vdin_std_duration,
			devp->vinfo_std_duration);
}
...
static void vdin_vf_init(struct vdin_dev_s *devp)
{
	...
	for (i = 0; i < p->size; ++i) {
		...
		if (devp->dtdata->hw_ver >= VDIN_HW_T7 &&
		    i < devp->canvas_max_num && devp->baddr_en) {
			/* use phy address */
...
#endif
	if (devp->double_wr && (devp->afbce_flag & VDIN_AFBCE_EN_422_12BIT) &&
		((vdin_is_dolby_signal_in(devp) &&
		!(vdin_dv_is_hw2(devp) || vdin_dv_is_hw5(devp)) &&
		(devp->format_convert == VDIN_FORMAT_CONVERT_YUV_YUV444 ||
		devp->format_convert == VDIN_FORMAT_CONVERT_RGB_RGB)) ||
		devp->debug.force_afbce_422_12bit == 2)) {
		devp->is_422_12bit_enabled = true;
		if (devp->debug.vdin_dbg_en)
			pr_info("vdin%d,422_12bit_enabled\n", devp->index);
	} else {
		devp->is_422_12bit_enabled = false;
	}
...
/* For DV HW5 on T3X/T6W only */
static void vdin_dv_hw5_init(struct vdin_dev_s *devp)
{
	if ((!is_meson_t3x_cpu() && !is_meson_t6w_cpu() && !is_meson_t6x_cpu()) ||
		devp->work_mode == VDIN_WORK_MD_V4L)
		return;
	...
	if ((devp->dtdata->hw_ver == VDIN_HW_T6W || devp->dtdata->hw_ver == VDIN_HW_T6X) &&
		vdin_dv_is_source_led(devp) && devp->dbg_force_one_buffer != 3)
		devp->dv_hw5.is_auto_mute_needed = true;
}
...
int vdin_crop_parm_adjust(struct vdin_dev_s *devp, struct vdin_parm_s  *para)
{
	...
	unsigned short	dest_h_active;
	unsigned short	dest_v_active;

	dest_h_active = rounddown(para->dest_h_active, devp->canvas_align);
	if (para->dest_h_active != dest_h_active)
		para->dest_h_active = dest_h_active;

	dest_v_active = rounddown(para->dest_v_active, 2);
	if (para->dest_v_active != dest_v_active)
		para->dest_v_active = dest_v_active;
	...
	if (devp->prop.loopback_crop_en) {
		...
		if (para->h_active > devp->dest_h_active)
			devp->prop.scaling4w = devp->dest_h_active;
		else
			devp->prop.scaling4w = para->h_active;

		if (para->v_active > devp->dest_v_active)
			devp->prop.scaling4h = devp->dest_v_active;
		else
			devp->prop.scaling4h = para->v_active;
	}
...
static void vdin_scale_and_cutwin_handle(struct vdin_dev_s *devp)
{
	...
	if (devp->dv_hw5.hw5_ctl) {
		...
		//if scaler in vdin_pp_top,aka scl_mode
	...
	if (devp->debug.vdin_dbg_en)
		pr_info("vdin%d %s %#x,%dx%d active:%ux%u.scaling:%dx%d\n",
			devp->index, __func__, vdin0_max_w_h,
...
{
	unsigned int vdin_pc_mode_pre = devp->vdin_pc_mode;
	unsigned int vdin_pc_mode_now;

	if ((devp->vdin_function_sel & VDIN_AUTO_PC_MODE) &&
	    vdin_is_auto_pc_mode(devp) && !vdin_pc_mode)
		vdin_pc_mode_now = 1;
	else
		vdin_pc_mode_now = vdin_pc_mode;

	devp->vdin_pc_mode = vdin_pc_mode_now;

	if ((devp->flags & VDIN_FLAG_DEC_STARTED) &&
		vdin_pc_mode_pre != vdin_pc_mode_now) {
		/* set flag for changing pc mode dynamically in ISR */
		devp->flags |= VDIN_FLAG_DYN_RECONFIG;
	} //todo
}
...
int vdin_start_dec(struct vdin_dev_s *devp)
{
	...
	else if (devp->dts_config.canvas_config_mode == 2)
...
int vdin_loopback_parm_adjust(struct vdin_dev_s *devp, struct vdin_parm_s  *para)
{
	...
	vinfo_pixel_clock = (unsigned long)vinfo->width
				* vinfo->height * vinfo->std_duration;
	...
	if ((devp->dtdata->hw_ver == VDIN_HW_T3X && devp->vinfo_over_pixel_clk) ||
		...
		if (para->dest_v_active > para->dest_h_active &&
...
int start_tvin_service(int no, struct vdin_parm_s  *para)
{
	...
	if (devp->parm.port != TVIN_PORT_VIU1 || devp->dts_config.viu_hw_irq != 0) {
...
int start_tvin_capture_ex(int dev_num, enum port_vpp_e port, struct vdin_parm_s  *para)
{
	...
	else if (port == PORT_VPP0_OSD_VIDEO_NO_PQ)
...
static int vdin_func(int no, struct vdin_arg_s *arg)
{
	...
	case VDIN_CMD_SET_CM2:
		vdin_set_cm2(devp->addr_offset, devp->h_active,
			     devp->v_active, parm->cm2, devp->dts_config.cm_enable);
		break;
	case VDIN_CMD_ISR:
		vdin_rdma_isr(devp);
		break;
...
int vdin_vs_duration_check(struct vdin_dev_s *devp)
{
	...
	if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_VS)
		pr_info("isr:diff_time:%d, cycle:%d, fps:%d %d\n",
	...
	if (diff_time > 0x186a0) {//jitter 2ms
		temp = diff_time;
	...
	if (devp->frame_cnt < devp->vdin_drop_num || devp->vdin_irq_flag ||
		...
		if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_VF)
			pr_info("vdin%d cnt:%d vf(%px):%d sg_type:%#x %#x flag:%#x %#x;vrr:%d\n",
				...
				devp->vfp->last_last_vfe->vf.vf_vrr_param.frc_get_vrr);
		if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_VF)
...
irqreturn_t vdin_write_done_isr(int irq, void *dev_id)
{
	...
	if (devp->dtdata->hw_ver == VDIN_HW_T6W || devp->dtdata->hw_ver == VDIN_HW_T6X)
		devp->afbce_mmu_num = rd(devp->addr_offset, VFCE_MIF_PLANE0_MMU_STA);

	if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_WRITE_DONE)
		pr_info("vdin%d,[%d %d]wr_done_irq_cnt:%d,MMU:%d\n",
			devp->index, devp->irq_cnt, devp->frame_cnt,
			devp->stats.wr_done_irq_cnt, devp->afbce_mmu_num);
...
static inline void vdin_dynamic_switch_vrr(struct vdin_dev_s *devp)
{
	...
	if (!devp->vrr_data.frame_lock_vrr_en) {
		...
		if (devp->vrr_on_add_cnt > devp->dts_config.vdin_vrr_switch_cnt) {
			vdin_frame_lock_check(devp, 1);
			devp->vrr_on_add_cnt = 0;
			devp->vrr_off_add_cnt = 0;
		}
		...
		if (devp->vrr_off_add_cnt > devp->dts_config.vdin_vrr_switch_cnt) {
			vdin_frame_lock_check(devp, 1);
			devp->vrr_on_add_cnt = 0;
			devp->vrr_off_add_cnt = 0;
		}
...
void vdin_update_vf_flag(struct vdin_dev_s *devp, struct vf_entry *vfe)
{
	...
	if (devp->vdin_pc_mode)
		vfe->vf.flag |= VFRAME_FLAG_PC_MODE;
	else
		vfe->vf.flag &= ~VFRAME_FLAG_PC_MODE;
...
{
	vfe->vf.type = devp->curr_field_type;
	vfe->vf.type_dw = devp->curr_field_type_dw;
	...
	vfe->vf.vf_vrr_param.qms_en = devp->prop.vtem_data.qms_en;
	...
	if (devp->is_vfce_en && vdin_is_afbce_enabled(devp))
...
static bool vdin_dyn_output_fmt(struct vdin_dev_s *devp)
{
	bool ret = false;

	if (devp->csc_cfg && !(devp->flags & VDIN_FLAG_DYN_RECONFIG)) {
		devp->flags |= VDIN_FLAG_DYN_RECONFIG;
		if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_CROP)
			pr_info("vdin%d,the csc_cfg has changed\n", devp->index);
	}

	if ((devp->flags & VDIN_FLAG_DYN_RECONFIG) && !devp->vdin_irq_flag) {
		if ((devp->game_mode & VDIN_GAME_MODE_2) && !devp->force_unlock) {
			devp->force_unlock = true;
			devp->game_mode_dly = devp->put_frame_cnt + vdin_game_mode_dly;
		}
		if (devp->put_frame_cnt >= devp->game_mode_dly) {
			vdin_get_format_convert(devp);
			if (devp->csc_cfg) {
				devp->csc_cfg = 0;
				vdin_set_matrix(devp);
			}
			vdin_set_bitdepth(devp, devp->flags & VDIN_FLAG_RDMA_ENABLE);
		...
		#endif
			vdin_set_vf_reinit_flag(devp);
			devp->flags &= ~VDIN_FLAG_DYN_RECONFIG;
			devp->force_unlock = false;
			vdin_drop_frame_info(devp, "dyn dfmt chg");
			ret = true;
		}
	}
...
//		goto irq_handled;
	...
	if (devp->afbce_mode == 1) {
		...
		devp->vdin_reset_flag = vdin_vsync_reset_mif(devp);
	}
	if ((devp->flags & VDIN_FLAG_DEC_STOP_ISR) &&
	    (!(devp->debug.vdin_isr_flag & VDIN_BYPASS_STOP_CHECK))) {
		vdin_hw_close(devp);
		devp->flags &= ~VDIN_FLAG_DEC_STOP_ISR;
	...
	curr_wr_vf->type_dw = devp->curr_field_type_dw;
	...
	if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_VF)
		pr_info("vdin%d,[%d %d]last_frame_idx:%d,cur_frame_idx:%d\n",
			devp->index, devp->irq_cnt, devp->frame_cnt,
...
irq_handled:
	...
	if (vf_drop_cnt < devp->vdin_drop_cnt)
	...
	if (devp->debug.vdin_isr_monitor & VDIN_ISR_MONITOR_BUFFER)
...
{
	if (devp->debug.ignore_vdin_ioctl == 0xff ||
	    devp->debug.ignore_vdin_ioctl == _IOC_NR(cmd))
		return -EBUSY;
...
static long vdin_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	void *table_buf;
	void *body_buf;
	void __user *user_head_ptr = NULL;
	void __user *user_table_ptr = NULL;
	void __user *user_body_ptr = NULL;
	...
	if (vdin_cmd_check(devp, cmd)) {
		pr_warn("vdin%d,ignore cmd:%#x %#x\n",
			devp->index, cmd, _IOC_NR(cmd));
		return 0;
	}

	if (devp->debug.vdin_dbg_en & VDIN_DBG_CNTL_IOCTL)
		pr_info("vdin%d cmd:0x%x %#x come in\n",
			devp->index, cmd, _IOC_NR(cmd));
	...
	case TVIN_IOC_G_MISC_INFO: {
		struct tvin_misc_info_s misc_info;

		memset(&misc_info, 0, sizeof(struct tvin_misc_info_s));
		mutex_lock(&devp->fe_lock);
		/* if port is not opened, ignore this command */
		if (!(devp->flags & VDIN_FLAG_DEC_OPENED)) {
			ret = -EPERM;
		...
		memcpy(&misc_info, &devp->misc_info, sizeof(struct tvin_misc_info_s));

		if (copy_to_user(argp, &misc_info, sizeof(struct tvin_misc_info_s)))
			ret = -EFAULT;

		if (devp->debug.vdin_dbg_en)
			pr_info("%s afd_info:%#x\n", __func__, misc_info.afd_info);
		mutex_unlock(&devp->fe_lock);
		break;
	}
	...
	case TVIN_IOC_S_TUNER_ID:
		if (copy_from_user(&devp->tuner_id, argp, sizeof(int))) {
			ret = -EFAULT;
			break;
		}
		if (devp->debug.vdin_dbg_en)
			pr_info("TVIN_IOC_S_TUNER_ID(%d) done\n\n", devp->tuner_id);
		break;
	...
	case TVIN_IOC_G_QMS_STATUS:
		...
		vdin_qms_status.qms_plus_en = devp->prop.qms_plus_flag;
		...
		pr_info("vdin%d,TVIN_IOC_G_QMS_STATUS qms_en:%d,qms_fr:%d, qms+:%d\n\n",
	...
	case TVIN_IOC_S_PC_MODE:
		...
		if (devp->debug.vdin_dbg_en)
			pr_info("TVIN_IOC_S_PC_MODE(%d->%d) flags:%#x chg:%d\n",
				vdin_pc_mode, tmp, devp->flags,
				devp->debug.bypass_pc_mode);
	...
	case TVIN_IOC_DUMP_BUF:
		if (!(devp->flags & VDIN_FLAG_DEC_STARTED))
			return -EFAULT;
		if (copy_from_user(&dump_arg, (void __user *)arg, sizeof(dump_arg))) {
			mutex_unlock(&devp->fe_lock);
			return -EFAULT;
		}

		user_buf = (void __user *)(uintptr_t)dump_arg.user_buf;
		if (!access_ok(user_buf, dump_arg.written_bytes)) {
			mutex_unlock(&devp->fe_lock);
			return -EFAULT;
		}
		if (vdin_is_convert_to_nv21(devp->format_convert))
			kernel_buf = vmalloc(devp->canvas_h * devp->canvas_w * 3 / 2);
		else
			kernel_buf = vmalloc(devp->canvas_h * devp->canvas_w);
		if (!kernel_buf) {
			mutex_unlock(&devp->fe_lock);
			return -ENOMEM;
		}
		...
		dump_arg.written_bytes = written_bytes;
	...
	case TVIN_IOC_DUMP_AFBC_BUF:
		if (!(devp->flags & VDIN_FLAG_DEC_STARTED))
			return -EFAULT;
		mutex_lock(&devp->fe_lock);
		if (copy_from_user(&dump_afbc_arg, (void __user *)arg, sizeof(dump_afbc_arg))) {
			ret = -EFAULT;
			goto exit_unlock;
		}
		user_head_ptr = (void __user *)(uintptr_t)dump_afbc_arg.user_head_buf;
		user_table_ptr = (void __user *)(uintptr_t)dump_afbc_arg.user_table_buf;
		user_body_ptr = (void __user *)(uintptr_t)dump_afbc_arg.user_body_buf;
		if (!access_ok(user_head_ptr, devp->afbce_info->frame_head_size) ||
			!access_ok(user_table_ptr, devp->afbce_info->frame_table_size) ||
			!access_ok(user_body_ptr, devp->afbce_info->frame_body_size * 2)) {
			ret = -EFAULT;
			goto exit_unlock;
		}
		head_buf = vmalloc(devp->afbce_info->frame_head_size);
		table_buf = vmalloc(devp->afbce_info->frame_table_size);
		body_buf = vmalloc(devp->afbce_info->frame_body_size * 2);
		if (!head_buf || !table_buf || !body_buf) {
			pr_err("%s: Buffer allocation failed\n", __func__);
			ret = -ENOMEM;
			goto exit_free;
		}
		dump_afbc_arg.user_head_buf_size = devp->afbce_info->frame_head_size;
		dump_afbc_arg.user_table_buf_size = devp->afbce_info->frame_table_size;
		dump_afbc_arg.user_body_buf_size = devp->afbce_info->frame_body_size;
		vdin_dump_one_afbce_mem_user(head_buf, table_buf, body_buf, devp,
			dump_afbc_arg.buf_num);
		if (copy_to_user(user_head_ptr, head_buf,
			dump_afbc_arg.user_head_buf_size) ||
			copy_to_user(user_table_ptr, table_buf,
			dump_afbc_arg.user_table_buf_size) ||
			copy_to_user(user_body_ptr, body_buf,
			dump_afbc_arg.user_body_buf_size)) {
			pr_err("%s: Copy to user failed\n", __func__);
			ret = -EFAULT;
			goto exit_free;
		}
		if (copy_to_user((void __user *)arg, &dump_afbc_arg, sizeof(dump_afbc_arg))) {
			pr_err("%s: Final copy failed\n", __func__);
			ret = -EFAULT;
			goto exit_free;
		}
exit_free:
...
static bool vdin_set_sig_property(struct tvin_frontend_s *fe)
{
	...
	if (!(devp->flags & VDIN_FLAG_ISR_EN) && devp->dts_config.vdin_get_prop_in_vs_en)
		return 1;
					...
					struct vdin_dev_s *devp)
{
	...
	devp->wr_done_irq = of_irq_get_byname(pdev->dev.of_node, "write_done_int");
	if (devp->wr_done_irq <= 0) {
		/* get irq from resource */
		res = platform_get_resource(pdev, IORESOURCE_IRQ, 1);
		if (res)
			devp->wr_done_irq = res->start;
	}
	...
	pr_info("%s %s=%d,%s=%d,%s=%d,%s=%d\n", __func__,
		...
		devp->vpu_crash_irq_name, devp->vpu_crash_irq,
		devp->wr_done_irq_name, devp->wr_done_irq,
		devp->vdin2_meta_wr_done_irq_name, devp->vdin2_meta_wr_done_irq);
...
static const struct match_data_s vdin_dt_t6w = {
	.name = "vdin-t6w",
	.hw_ver = VDIN_HW_T6W,
	.vdin0_en = 1,                  .vdin1_en = 1,
	.de_tunnel_tunnel = 1, /*0,1*/  .ipt444_to_422_12bit = 0, /*0,1*/
	.vdin0_line_buff_size = 4096,   .vdin1_line_buff_size = 4096,
					.vdin1_set_hdr = false,
};

static const struct match_data_s vdin_dt_t6x = {
	.name = "vdin-t6x",
	.hw_ver = VDIN_HW_T6X,
	.vdin0_en = 1,                  .vdin1_en = 1,
	.de_tunnel_tunnel = 1, /*0,1*/  .ipt444_to_422_12bit = 0, /*0,1*/
...
static const struct of_device_id vdin_dt_match[] = {
	...
	{
		.compatible = "amlogic, vdin-t6w",
		.data = &vdin_dt_t6w,
	},
	{
		.compatible = "amlogic, vdin-t6x",
		.data = &vdin_dt_t6x,
	},
#endif
...
#endif
		break;
	case VIDEO_HDMI_CH_COUNT_CHANGED:
		if (!para) {
			pr_warn("%s VIDEO_HDMI_CH_COUNT_CHANGED,para == NULL!\n", __func__);
			break;
		}
		g_disp_count = *(unsigned int *)para;
		if (devp->debug.vdin_dbg_en)
			pr_info("%s VIDEO_HDMI_CH_COUNT_CHANGED,disp_count = %d\n",
	...
	struct platform_device *pdev)
{
	...
	//t6w default
	devp->cr_lossy_param.cr_lossy_target_byte = 32;
	devp->cr_lossy_param.mmu_en = 1;

	/* txhd2 keystone,0:vppout;1:postblend */
	devp->dts_config.keystone_sel = 0;
	devp->set_canvas_manual = 0;
	if (devp->dtdata->hw_ver < VDIN_HW_T7 || devp->dtdata->hw_ver == VDIN_HW_S4 ||
		devp->dtdata->hw_ver == VDIN_HW_S4D || devp->dtdata->hw_ver == VDIN_HW_TXHD2)
		devp->baddr_en = false;
	else
		devp->baddr_en = true;
	...
	devp->dts_config.max_buf_num = VDIN_CANVAS_MAX_CNT;
	devp->dts_config.min_buf_num = VDIN_CANVAS_MIN_CNT;
	devp->dts_config.cm_enable = true;
	devp->dts_config.vdin_det_idle_wait = 100;
	devp->dts_config.vdin_get_prop_in_vs_en = true;
	devp->dts_config.vdin_get_prop_in_sm_en = true;
	devp->dts_config.canvas_config_mode = 2;
	devp->dts_config.viu_hw_irq = 1;
	devp->dts_config.game_mode_switch_frames = 10;
	devp->dts_config.game_mode_phlock_switch_frames = 60;
	devp->dts_config.vdin_vrr_switch_cnt = 3;
	devp->dts_config.sct_cache_size = 192;
	devp->dts_config.vdin_re_config = (RE_CONFIG_DV_EN | RE_CONFIG_HDR_EN);
	devp->dts_config.vdin_re_cfg_drop_cnt = 8;
	devp->dts_config.back_nosig_max_cnt = BACK_NOSIG_MAX_CNT;
	devp->dts_config.atv_unstable_in_cnt = 45;
	devp->dts_config.atv_unstable_out_cnt = 50;
	devp->dts_config.hdmi_unstable_out_cnt = 1;
	devp->dts_config.hdmi_stable_out_cnt = 1;
	devp->dts_config.atv_stable_out_cnt = 100;
	devp->dts_config.atv_stable_fmt_check_cnt = 10;
	devp->dts_config.atv_prestable_out_cnt = 50;
	devp->dts_config.other_stable_out_cnt = EXIT_STABLE_MAX_CNT;
	devp->dts_config.other_unstable_out_cnt = BACK_STABLE_MAX_CNT;
	devp->dts_config.other_unstable_in_cnt = UNSTABLE_MAX_CNT;
	devp->dts_config.nosig_in_cnt = NOSIG_MAX_CNT;
	devp->dts_config.nosig2_unstable_cnt = EXIT_NOSIG_MAX_CNT;
	devp->dts_config.vdin_dv_chg_cnt = 1;
...
static int vdin_drv_probe(struct platform_device *pdev)
{
	...
	if (devp->dts_config.canvas_config_mode == 0 || devp->dts_config.canvas_config_mode == 1)
