...
static unsigned int vdin_qms_plus_chg_cnt = 2;
module_param(vdin_qms_plus_chg_cnt, uint, 0664);
MODULE_PARM_DESC(vdin_qms_plus_chg_cnt, "vdin_qms_chg_cnt");
...
static enum tvin_sg_chg_flg vdin_hdmirx_fmt_chg_detect(struct vdin_dev_s *devp)
{
	...
	if (sm_ops->get_sig_property) {
		...
		if (vdin_is_qms_plus_state_chg(devp)) {
			...
			if (devp->vrr_data.qms_plus_chg_cnt >= vdin_qms_chg_cnt) {
