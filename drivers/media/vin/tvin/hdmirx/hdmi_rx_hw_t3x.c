...
//	usleep_range(10, 20);
	...
	usleep_range(10, 20);
	hdmirx_wr_amlphy_t3x(T3X_RG_RX20PLL_1, 0x014812a6, port);
	usleep_range(10, 20);
	hdmirx_wr_amlphy_t3x(T3X_RG_RX20PLL_0, 0x0530a001, port);
	usleep_range(10, 20);
	hdmirx_wr_amlphy_t3x(T3X_RG_RX20PLL_0, 0x0530a003, port);
	usleep_range(10, 20);
	hdmirx_wr_amlphy_t3x(T3X_RG_RX20PLL_1, 0x414012a6, port);
	usleep_range(10, 20);
...
void t3x_720p_pll_cfg_20(u8 port)
{
	...
	usleep_range(10, 20);
	hdmirx_wr_amlphy_t3x(T3X_RG_RX20PLL_1, 0x614012a6, port);
	usleep_range(10, 20);
...
void t3x_4k60_pll_cfg_20(u32 clkrate, u8 port)
{
	u8 div_4 = 0;
	...
	if (!clkrate)
		div_4 = 1;
	hdmirx_wr_amlphy_t3x(T3X_RG_RX20PLL_0, (0x05302800 | (div_4 << 5)), port);
	usleep_range(10, 20);
	hdmirx_wr_amlphy_t3x(T3X_RG_RX20PLL_1, 0x01481236, port);
	usleep_range(10, 20);
	hdmirx_wr_amlphy_t3x(T3X_RG_RX20PLL_0, (0x05302801 | (div_4 << 5)), port);
	usleep_range(10, 20);
	hdmirx_wr_amlphy_t3x(T3X_RG_RX20PLL_0, (0x45302803 | (div_4 << 5)), port);
	usleep_range(10, 20);
	hdmirx_wr_amlphy_t3x(T3X_RG_RX20PLL_1, 0x01401236, port);
	usleep_range(10, 20);
	hdmirx_wr_amlphy_t3x(T3X_RG_RX20PLL_0, (0x05302807 | (div_4 << 5)), port);
	usleep_range(10, 20);
	hdmirx_wr_amlphy_t3x(T3X_RG_RX20PLL_0, (0x45302807 | (div_4 << 5)), port);
	usleep_range(10, 20);
	rx[port].phy.aud_div = 0;
	rx[port].phy.aud_div_1 = 0x8;
}

void aml_pll_bw_cfg_t3x_20(u8 port, u32 cable_clk)
{
	...
	case PLL_BW_3:
		...
		break;
	case PLL_BW_4:
		t3x_4k60_pll_cfg_20(clk_rate, port);
		break;
	}
...
void aml_phy_cfg_t3x_20(u8 port, u32 cable_clk)
{
	u32 idx = 0;
	u32 clk_rate = 0;
	u32 data32;
...
/* set cdr_lock_gated_en to 0 for exbist debug */
static const u32 phy_dcha_t3x_21[][4] = {
	...
	{	 /* FRL 3G */
		0x0370ffff, 0x05ff1a05, 0x11030010, 0x07f06555,
	},
	{	 /* FRL 6G3L */
		0x2320ffff, 0x05ff1a05, 0x11001010, 0x07f06555,
	},
	{	 /* FRL 6G4L */
		0x2320ffff, 0x05ff1a05, 0x11001010, 0x07f06555,
	},
	{	 /* FRL 8G */
		0x2340ffff, 0x05ff1a05, 0x11102000, 0x07f06555,
	},
	{	 /* FRL 10G */
		0x7330bbbb, 0x05ff1a05, 0x11102000, 0x07f06555,
	},
	{	 /* FRL 12G */
		0x7330bbbb, 0x05ff1a05, 0x11102000, 0x07f06555,
	},
...
void aml_phy_get_trim_val_t3x_21(u8 port)
{
	...
	rx_info.aml_phy_21.rterm_val = t3x_rlevel[rx_info.aml_phy_21.rterm_dts_lvl];
}
...
void rx_21_frl_phy_cfg(u8 port, u32 cable_clk)
{
	u32 data32 = 0;
	u32 idx = 0;
	u32 clk_rate = 0;
...
void t3x_720p_pll_cfg_21(u8 port)
{
	...
	usleep_range(20, 30);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PLL_CTRL1, 0x21401286, port);
	usleep_range(20, 30);
...
void t3x_4k60_pll_cfg_21(u32 clkrate, u8 port)
{
	u8 div_4 = 0;

	if (!clkrate)
		div_4 = 1;
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PLL_CTRL0, (0x05302800 | (div_4 << 5)), port);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PLL_CTRL1, 0x01481236, port);
	usleep_range(20, 30);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PLL_CTRL0, (0x05302801 | (div_4 << 5)), port);
	usleep_range(20, 30);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PLL_CTRL0, 0x05302803 | (div_4 << 5), port);
	usleep_range(20, 30);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PLL_CTRL1, 0x01401236, port);
	usleep_range(20, 30);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PLL_CTRL0, (0x05302807 | (div_4 << 5)), port);
	usleep_range(20, 30);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PLL_CTRL0, (0x45302807 | (div_4 << 5)), port);
	usleep_range(10, 20);
	rx[port].phy.aud_div = 0;
	rx[port].phy.aud_div_1 = 0x8;
}

void aml_pll_bw_cfg_t3x_21(int f_rate, u8 port, u32 cable_clk)
{
	u32 data32;
	int pll_rst_cnt = 0;
	u32 idx = 0;
	...
	if (f_rate == FRL_OFF) {
		do {
			...
			if (idx == PLL_BW_4)
				t3x_4k60_pll_cfg_21(clk_rate, port);
			if (log_level & PHY_LOG)
				rx_pr("PLL0=0x%x\n", hdmirx_rd_amlphy_t3x(T3X_RG_RX20PLL_0, port));
			if (pll_rst_cnt++ > pll_rst_max) {
				if (log_level & PHY_LOG)
...
void rx_21_eq_cfg(int f_rate, u8 port)
{
	...
	if (cdr_bw & 0x10) {
		hdmirx_wr_bits_amlphy_t3x(T3X_HDMIRX21PHY_DCHD_CDR, MSK(4, 0), cdr_bw & 0xf, port);
	} else {
		if (rx[port].phy.phy_bw == PHY_BW_3)
			hdmirx_wr_bits_amlphy_t3x(T3X_HDMIRX21PHY_DCHD_CDR, MSK(4, 0), 7, port);
		else
			hdmirx_wr_bits_amlphy_t3x(T3X_HDMIRX21PHY_DCHD_CDR, MSK(4, 0), 4, port);
	}
...
void hdmirx_fpll_recovery_t3x(u8 port)
{
	if (port == E_PORT2) {
		...
		wr_reg_clk_ctl(T3X_CLKCTRL_HDMI_PLL0_CTRL3, 0x0b09a001);
...
void aml_phy_init_t3x_20(u8 port)
{
	...
	if (rx[port].state == FSM_WAIT_CLK_STABLE && !rx[port].cableclk_stb_flg) {
		aml_phy_cfg_t3x_20(port, cable_clk);
		return;
	}
	aml_phy_cfg_t3x_20(port, cable_clk);
	usleep_range(10, 20);
	aml_pll_bw_cfg_t3x_20(port, cable_clk);
	usleep_range(10, 20);
...
void aml_phy_init_t3x_21(u8 port)
{
	...
	rx_21_frl_phy_cfg(port, cable_clk);
	aml_pll_bw_cfg_t3x_21(rx[port].var.frl_rate, port, cable_clk);
...
void get_val_21_t3x(char *temp, unsigned int val, int len)
{
	if ((val >> (len - 1)) == 0)
		sprintf(temp, "+%d", val & (~(1 << (len - 1))));
	else
...
void comb_val_21_t3x(char *type, unsigned int val_0, unsigned int val_1,
		 unsigned int val_2, unsigned int val_3, int len)
{
	char out[32], v0_buf[16], v1_buf[16], v2_buf[16], v3_buf[16];
	int pos = 0;

	get_val_21_t3x(v0_buf, val_0, len);
	get_val_21_t3x(v1_buf, val_1, len);
	get_val_21_t3x(v2_buf, val_2, len);
	get_val_21_t3x(v3_buf, val_3, len);
	pos += snprintf(out + pos, 32 - pos, "%s[", type);
	pos += snprintf(out + pos, 32 - pos, " %s,", v0_buf);
	pos += snprintf(out + pos, 32 - pos, " %s,", v1_buf);
	pos += snprintf(out + pos, 32 - pos, " %s,", v2_buf);
...
void dump_aml_phy_sts_t3x_21(u8 port)
{
	...
	comb_val_21_t3x("	 dfe_tap0", dfe0_tap0, dfe1_tap0, dfe2_tap0, dfe3_tap0, 7);
	comb_val_21_t3x("	 dfe_tap1", dfe0_tap1, dfe1_tap1, dfe2_tap1, dfe3_tap1, 6);
	comb_val_21_t3x("	 dfe_tap2", dfe0_tap2, dfe1_tap2, dfe2_tap2, dfe3_tap2, 5);
	comb_val_21_t3x("	 dfe_tap3", dfe0_tap3, dfe1_tap3, dfe2_tap3, dfe3_tap3, 4);
	comb_val_21_t3x("	 dfe_tap4", dfe0_tap4, dfe1_tap4, dfe2_tap4, dfe3_tap4, 4);
	comb_val_21_t3x("	 dfe_tap5", dfe0_tap5, dfe1_tap5, dfe2_tap5, dfe3_tap5, 4);
	comb_val_21_t3x("	 dfe_tap6", dfe0_tap6, dfe1_tap6, dfe2_tap6, dfe3_tap6, 4);
	comb_val_21_t3x("	 dfe_tap7", dfe0_tap7, dfe1_tap7, dfe2_tap7, dfe3_tap7, 4);
	comb_val_21_t3x("	 dfe_tap8", dfe0_tap8, dfe1_tap8, dfe2_tap8, dfe3_tap8, 4);

	comb_val_21_t3x("slicer_ofst0", sli0_ofst0, sli1_ofst0, sli2_ofst0, sli3_ofst0, 5);
	comb_val_21_t3x("slicer_ofst1", sli0_ofst1, sli1_ofst1, sli2_ofst1, sli3_ofst1, 5);
	comb_val_21_t3x("slicer_ofst2", sli0_ofst2, sli1_ofst2, sli2_ofst2, sli3_ofst2, 5);
	comb_val_21_t3x("slicer_ofst3", sli0_ofst3, sli1_ofst3, sli2_ofst3, sli3_ofst3, 5);
	comb_val_21_t3x("slicer_ofst4", sli0_ofst4, sli1_ofst4, sli2_ofst4, sli3_ofst4, 5);
	comb_val_21_t3x("slicer_ofst5", sli0_ofst5, sli1_ofst5, sli2_ofst5, sli3_ofst5, 5);
	...
	comb_val_21_t3x("cdr_int", cdr0_int, cdr1_int, cdr2_int, cdr3_int, 7);
}
...
void aml_phy_exbist_t6x_21_tmds(u8 port, u8 ch)
{
	...
	rx_pr("port21 tmds exbist\n");
	//phy cfg
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_MISC0, 0x01ff7070, port);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_MISC1, 0x30f7000f, port);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_MISC2, 0x00001a00, port);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHA_AFE, 0x23208888, port);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHA_DFE, 0x001f0205, port);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHA_PI, 0x00002018, port);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHA_CTRL, 0x03f06555, port);

	//pll cfg
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PLL_CTRL0, 0x05302800, port);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PLL_CTRL1, 0x01445384, port);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PLL_CTRL0, 0x05302801, port);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PLL_CTRL0, 0x05302803, port);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PLL_CTRL0, 0x05302807, port);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PLL_CTRL0, 0x45302807, port);
	...
	//phy cdr&eq&dfe adp
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_MISC0, 0x01ff707f, port);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHD_CDR, 0x00001482, port);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHD_EQ, 0x3001104f, port);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHD_CDR, 0x00007482, port);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHD_EQ, 0x3001304f, port);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHD_EQ, 0x3001334f, port);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHD_EQ, 0x3003334f, port);
	...
	hdmirx_wr_top(TOP_CHAN_SWITCH_1, 0x1000007, port);
	data32 |= (1 << 0);//[0]enable
	hdmirx_wr_top(TOP_CHAN_SWITCH_0, 0x15000001, port);
...
void aml_phy_exbist_t6x_21_frl(u8 port, u8 ch)
{
	...
	ch_id = ch & 0xf;
	if ((ch >> 4) & 0xf)
		frl_rate_id = (ch >> 4) & 0xf;
	rx_pr("port21 frl exbist:frlrate=%d,ch%d\n", frl_rate_id, ch_id);
	if (0) {
		//phy cfg
		hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_MISC0, 0x01fcf0f0, port);
		hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_MISC1, 0x30f7000f, port);
		hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_MISC2, 0x00001a00, port);
		hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHA_AFE, 0x7330bbbb, port);
		hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHA_DFE, 0x001f0205, port);
		hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHA_PI, 0x01102008, port);
		hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHA_CTRL, 0x03f06555, port);
		...
		//phy cdr&eq&dfe adp
		hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_MISC0, 0x01fff0ff, port);
		hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHD_CDR, 0x00001482, port);
		hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHD_EQ, 0x3001104f, port);
		hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHD_CDR, 0x00007082, port);
		hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHD_EQ, 0x3001304f, port);
		hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHD_EQ, 0x3001334f, port);
		hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHD_EQ, 0x3003334f, port);
	} else {
		rx_21_frl_phy_cfg(port, rx[port].clk.cable_clk);
		aml_pll_bw_cfg_t3x_21(frl_rate_id, port, rx[port].clk.cable_clk);
		rx_21_frl_pll_cfg(frl_rate_id, port);
		rx_21_eq_cfg(frl_rate_id, port);
	}
	usleep_range(1000, 1100);
	//sw reset
	hdmirx_wr_top(TOP_SW_RESET, 0xfffffe7f, port);
...
void rx_set_aud_output_t3x(u32 param, u8 port)
{
	if (param == 2) {
		...
		if (rx_is_pip_on() && port == rx_info.sub_port)
			hdmirx_wr_bits_top_common(TOP_ACR_CNTL2_T3X, _BIT(5), 0x1);
		else if (port == rx_info.main_port)
			hdmirx_wr_bits_top_common_1(TOP_ACR_CNTL2_T3X, _BIT(5), 0x1);
	} else {
		hdmirx_wr_cor(RX_TDM_CTRL1_AUD_IVCRX, 0x00, port);
		hdmirx_wr_cor(RX_TDM_CTRL2_AUD_IVCRX, 0x10, port);
...
void hdcp_init_t3x(u8 port)
{
	...
	hdmirx_wr_cor(CP2PAX_RX_CTRL_0_HDCP2X_IVCRX, 0xe3, port);
...
void rx_aud_pll_ctl_t3x(bool en, u8 port)
{
	...
	if (en) {
		...
		if (port == rx_info.main_port)
			wr_reg_clk_ctl(T3X_CLKCTRL_AUD21_PLL_CTRL0, 0);
		else if (port == rx_info.sub_port)
