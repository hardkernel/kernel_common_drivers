...
static void t6w_dmc_port_config(struct ddr_bandwidth *db, int channel, int port)
{
	unsigned int val;
...
static void t6w_dmc_range_config(struct ddr_bandwidth *db, int channel,
...
static unsigned long t6w_get_dmc_freq_quick(struct ddr_bandwidth *db)
{
...
static void t6w_dmc_bandwidth_enable(struct ddr_bandwidth *db)
{
	unsigned int val;
...
static void t6w_dmc_bandwidth_init(struct ddr_bandwidth *db)
{
	unsigned int i;
	...
	t6w_dmc_bandwidth_enable(db);

	for (i = 0; i < db->channels; i++) {
		if (!db->port[i])
			t6w_dmc_port_config(db, i, -1);
		t6w_dmc_range_config(db, i, 0, 0xffffffff);
		db->range[i].start = 0;
		db->range[i].end   = 0xffffffff;
	}
}

static int t6w_handle_irq(struct ddr_bandwidth *db, struct ddr_grant *dg)
{
...
static int t6w_dump_reg(struct ddr_bandwidth *db, char *buf)
{
	int s = 0, i;
...
struct ddr_bandwidth_ops t6w_ddr_bw_ops = {
	.init             = t6w_dmc_bandwidth_init,
	.config_port      = t6w_dmc_port_config,
	.config_range     = t6w_dmc_range_config,
	.get_freq         = t6w_get_dmc_freq_quick,
	.handle_irq       = t6w_handle_irq,
	.bandwidth_enable = t6w_dmc_bandwidth_enable,
	.outstanding_init = outstanding_init,
	.outstanding      = outstanding_handle,
#if DDR_BANDWIDTH_DEBUG
	.dump_reg         = t6w_dump_reg,
#endif
};
