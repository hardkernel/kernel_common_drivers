...
static int check_violation(struct dmc_monitor *mon, void *data)
{
	...
	if (!ret) {
		mon_comm->port.number = (mon_comm->status >> 10) & 0xf;
...
static int dmc_sec_check(char *output)
{
	...
	unsigned long dmc_vio_status, dmc_vio_reg[8];
	int count = 0, i;
	char offset = 20;
	...
	if (dmc_vio_status & 0x1) {
		...
		if (dmc_vio_reg[5] & (0x7 << offset)) {
			tmp.addr = dmc_vio_reg[4];
			tmp.port.number = (dmc_vio_reg[5] >> 10) & 0xf;
			tmp.sub.number = (dmc_vio_reg[5] >> 6) & 0xf;
		}
		if (dmc_vio_reg[7] & (0x7 << offset)) {
			tmp.addr = dmc_vio_reg[6];
			tmp.port.number = (dmc_vio_reg[7] >> 10) & 0xf;
			tmp.sub.number = (dmc_vio_reg[7] >> 6) & 0xf;
		}
	...
	count += dmc_sec_save_reg(output + count, 0, dmc_vio_status, dmc_vio_reg, 8);
