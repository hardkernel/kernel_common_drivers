...
static int check_violation(struct dmc_monitor *mon, void *data)
{
	...
	if (!ret) {
		mon_comm->port.number = (mon_comm->status >> 9) & 0x1f;
		mon_comm->sub.number = mon_comm->status & 0x7;
...
static void s1a_dmc_vio_to_port(void *data, unsigned long *vio_bit)
{
	struct dmc_mon_comm *mon_comm = (struct dmc_mon_comm *)data;
...
static int dmc_sec_check(char *output)
{
	...
	if (dmc_vio_status & 0x1) {
		...
		tmp.port.number = (dmc_vio_reg[3] >> 9) & 0x1f;
		tmp.sub.number = dmc_vio_reg[3] & 0x7;
		if (tmp.sub.number == 0x2)
			tmp.sub.number = (dmc_vio_reg[3] >> 7) & 0x3;
		count += dmc_sec_save_info(output + count, 0, &tmp);
	}
	if (dmc_vio_status & 0x2) {
		tmp.rw = 'w';
		tmp.addr =  dmc_vio_reg[0];
		tmp.port.number = (dmc_vio_reg[1] >> 9) & 0x1f;
		tmp.sub.number = dmc_vio_reg[1] & 0x7;
		if (tmp.sub.number == 0x2)
			tmp.sub.number = (dmc_vio_reg[1] >> 7) & 0x3;
		count += dmc_sec_save_info(output + count, 0, &tmp);
	}
