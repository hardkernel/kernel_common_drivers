...
static int dmc_sec_check(char *output)
{
	...
	if (dmc_vio_status & 0x1) {
		...
		rw = 'r';
		addr =  dmc_vio_reg[2];
		port = (dmc_vio_reg[3] >> 9) & 0x1f;
		subport = dmc_vio_reg[3] & 0x7;
		if (subport == 0x2)
			subport = (dmc_vio_reg[3] >> 7) & 0x3;
	...
	if (dmc_vio_status & 0x2) {
		...
		rw = 'w';
		addr =  dmc_vio_reg[0];
		port = (dmc_vio_reg[1] >> 9) & 0x1f;
		subport = dmc_vio_reg[1] & 0x7;
		if (subport == 0x2)
			subport = (dmc_vio_reg[1] >> 7) & 0x3;
		count += sprintf(output + count,
