...
static const char *default_compressor = CONFIG_AMLOGIC_ZRAM_DEF_COMP;
...
#ifdef CONFIG_AMLOGIC_ZRAM_TRACK_ENTRY_ACTIME
		...
		struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (!sysfs_streq(buf, "all")) {
		...
		if (IS_ENABLED(CONFIG_AMLOGIC_ZRAM_TRACK_ENTRY_ACTIME) && !kstrtoull(buf, 0, &age_sec))
...
#ifdef CONFIG_AMLOGIC_ZRAM_WRITEBACK
...
#ifdef CONFIG_AMLOGIC_ZRAM_MEMORY_TRACKING
...
#ifdef CONFIG_AMLOGIC_ZRAM_MULTI_COMP
