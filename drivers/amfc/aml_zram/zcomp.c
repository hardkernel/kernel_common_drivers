...
#if IS_ENABLED(CONFIG_AMLOGIC_ZRAM_BACKEND_LZO)
	&backend_lzorle,
	&backend_lzo,
#endif
#if IS_ENABLED(CONFIG_AMLOGIC_ZRAM_BACKEND_LZ4)
	&backend_lz4,
#endif
#if IS_ENABLED(CONFIG_AMLOGIC_ZRAM_BACKEND_AMFC)
	&backend_amfc,
#endif
	NULL
};
...
static int zcomp_strm_init(struct zcomp *comp, struct zcomp_strm *zstrm)
{
	 ...
	 */
	zstrm->buffer = kzalloc(2 * PAGE_SIZE, GFP_KERNEL);
	if (!zstrm->buffer) {
