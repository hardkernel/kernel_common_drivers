			      ...
			      struct f2fs_filename *fname)
{
	...
	if (IS_CASEFOLDED(dir) &&
		...
		len = f_utf8_casefold(sb->s_encoding, fname->usr_fname,
				    buf, F2FS_NAME_LEN);
		if (len <= 0) {
			...
			int lookup, struct f2fs_filename *fname)
{
	...
	err = f_fscrypt_setup_filename(dir, iname, lookup, &crypt_name);
	if (err)
		return err;
			...
			struct f2fs_filename *fname)
{
	...
	err = f_fscrypt_prepare_lookup(dir, dentry, &crypt_name);
	if (err)
		return err;
				   ...
				   const u8 *de_name, u32 de_name_len)
{
	...
	if (fname->cf_name.name)
		return f_generic_ci_match(dir, fname->usr_fname,
					&fname->cf_name,
					de_name, de_name_len);
...
#endif
	return f_fscrypt_match_name(&f, de_name, de_name_len);
}
...
{
	return f2fs_find_entry(dir, dotdot_name_t, p);
}
		...
		struct page *page, struct inode *inode)
{
	...
	de->file_type = f_fs_umode_to_ftype(inode->i_mode);
	set_page_dirty(page);

	inode_set_mtime_to_ts(dir, f_inode_set_ctime_current(dir));
			...
			const struct f2fs_filename *fname, struct page *dpage)
{
	...
	if (is_inode_flag_set(inode, FI_NEW_INODE)) {
		...
		if (IS_ENCRYPTED(inode)) {
			err = f_fscrypt_set_context(inode, page);
			if (err)
				goto put_error;
		}
	} else {
			...
			unsigned int bit_pos)
{
	...
	de->file_type = f_fs_umode_to_ftype(mode);
	for (i = 0; i < slots; i++) {
			...
			unsigned int start_pos, struct fscrypt_str *fstr)
{
	...
	while (bit_pos < d->max) {
		...
		if (IS_ENCRYPTED(d->inode)) {
			...
			err = f_fscrypt_fname_disk_to_usr(d->inode,
...
static int f2fs_readdir(struct file *file, struct dir_context *ctx)
{
	...
	if (IS_ENCRYPTED(inode)) {
		err = f_fscrypt_prepare_readdir(inode);
		if (err)
			goto out;

		err = f_fscrypt_fname_alloc_buffer(F2FS_NAME_LEN, &fstr);
		if (err < 0)
			goto out;
	}
	...
	for (; n < npages; ctx->pos = n * NR_DENTRY_IN_BLOCK) {
		...
		if (npages - n > 1 && !ra_has_index(ra, n))
			f_page_cache_sync_readahead(inode->i_mapping, ra, file, n,
...
out_free:
	f_fscrypt_fname_free_buffer(&fstr);
out:
