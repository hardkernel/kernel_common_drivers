...
static int gc_thread_func(void *data)
{
	...
	do {
		...
		if (!f_sb_start_write_trylock(sbi->sb)) {
			stat_other_skip_bggc_count(sbi);
			continue;
		}
			...
			int type, struct victim_sel_policy *p)
{
	...
	if (f2fs_need_rand_seg(sbi))
		p->offset = f_get_random_u32_below(MAIN_SECS(sbi) *
						SEGS_PER_SEC(sbi));
				...
				int gc_type, unsigned int segno, int off)
{
	...
	if (f_clear_page_dirty_for_io(fio.encrypted_page))
		dec_page_count(fio.sbi, F2FS_DIRTY_META);

	f_set_page_writeback(fio.encrypted_page);
...
out_drop_write:
	...
	err = f_freeze_super(sbi->sb, FREEZE_HOLDER_USERSPACE);
	if (err)
		return err;

	if (f2fs_readonly(sbi->sb)) {
		err = f_thaw_super(sbi->sb, FREEZE_HOLDER_USERSPACE);
		if (err)
			return err;
		return -EROFS;
	}
...
out_err:
	...
	f2fs_up_write(&sbi->gc_lock);
	f_thaw_super(sbi->sb, FREEZE_HOLDER_USERSPACE);
	return err;
}
