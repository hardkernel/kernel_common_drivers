...
static void clear_node_page_dirty(struct page *page)
{
	if (PageDirty(page)) {
		...
		f_clear_page_dirty_for_io(page);
...
{
	return f_radix_tree_gang_lookup(&nm_i->nat_root, (void **)ep, start, nr);
}
...
{
	return f_radix_tree_gang_lookup(&nm_i->nat_set_root, (void **)ep,
							start, nr);
}
...
static int truncate_node(struct dnode_of_data *dn)
{
	...
	f_invalidate_mapping_pages(NODE_MAPPING(sbi),
			index, index);
...
static void flush_inline_data(struct f2fs_sb_info *sbi, nid_t ino)
{
	...
	if (!f_clear_page_dirty_for_io(page))
		goto page_out;
...
static struct folio *last_fsync_dnode(struct f2fs_sb_info *sbi, nid_t ino)
{
	...
	while ((nr_folios = f_filemap_get_folios_tag(NODE_MAPPING(sbi), &index,
...
int f2fs_move_node_page(struct page *node_page, int gc_type)
{
	...
	if (gc_type == FG_GC) {
		...
		if (!f_clear_page_dirty_for_io(node_page)) {
			err = -EAGAIN;
			goto out_page;
		}
...
continue_unlock:
	...
	if (!ret && atomic && !marked) {
		...
		f_folio_mark_dirty(last_folio);
		f_folio_unlock(last_folio);
		goto retry;
	}
out:
	if (nwritten)
...
static bool flush_dirty_inode(struct folio *folio)
{
	...
	inode = f_find_inode_nowait(sbi->sb, ino, f2fs_match_ino, NULL);
	if (!inode)
		return false;
...
next_step:
	...
	while (!done && (nr_folios = f_filemap_get_folios_tag(NODE_MAPPING(sbi),
						...
						unsigned int seq_id)
{
	...
	return f_filemap_check_errors(NODE_MAPPING(sbi));
}
				...
				nid_t nid, bool build, bool update)
{
	...
	f_radix_tree_preload(GFP_NOFS | __GFP_NOFAIL);
			...
			unsigned int segno, struct f2fs_summary_block *sum)
{
	...
	for (i = 0; i < last_offset; i += nrpages, addr += nrpages) {
		...
		f_invalidate_mapping_pages(META_MAPPING(sbi), addr,
							addr + nrpages);
	}
	return 0;
}
