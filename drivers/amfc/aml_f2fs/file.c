...
static vm_fault_t f2fs_vm_page_mkwrite(struct vm_fault *vmf)
{
	...
	f_sb_start_pagefault(inode->i_sb);
...
static struct vm_operations_struct f2fs_file_vm_ops = {
...
static inline enum cp_reason_type need_do_checkpoint(struct inode *inode)
{
	...
	if (!S_ISREG(inode->i_mode))
		cp_reason = CP_NON_REGULAR;
		 ...
		 !(F2FS_I(inode)->i_compress_flag & COMPRESS_SKIP_WRITE_CP))
						...
						int datasync, bool atomic)
{
	...
	ret = f_file_write_and_wait_range(file, start, end);
		...
		struct dnode_of_data *dn, pgoff_t index, int whence)
{
	...
	case SEEK_DATA:
		if (__is_valid_data_blkaddr(blkaddr))
			return true;
		if (blkaddr == NEW_ADDR &&
		    f_xa_get_mark(&mapping->i_pages, index, PAGECACHE_TAG_DIRTY))
			return true;
...
found:
	...
	return f_vfs_setpos(file, data_ofs, maxbytes);
fail:
	inode_unlock_shared(inode);
	return -ENXIO;
}
...
static loff_t f2fs_llseek(struct file *file, loff_t offset, int whence)
{
	...
	case SEEK_CUR:
	case SEEK_END:
		return f_generic_file_llseek_size(file, offset, whence,
...
static int f2fs_file_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	f_file_accessed(file);
	f2fs_file_vm_ops.map_pages = f_filemap_map_pages,
...
{
	int err = f_fscrypt_file_open(inode, filp);
	...
	err = f_fsverity_file_open(inode, filp);
	if (err)
		return err;
	...
	err = f_dquot_file_open(inode, filp);
	if (err)
		return err;
								...
								bool cache_only)
{
	...
	if (cache_only) {
		folio = f_filemap_lock_folio(mapping, index);
		if (IS_ERR(folio))
		       return 0;
...
static bool f2fs_force_buffered_io(struct inode *inode, int rw)
{
	...
	if (!f_fscrypt_dio_supported(inode))
		return true;
			   ...
			   struct inode *inode, const struct iattr *attr)
{
	...
	f_i_uid_update(idmap, attr, inode);
	f_i_gid_update(idmap, attr, inode);
	...
	if (ia_valid & ATTR_MODE) {
		...
		if (!f_in_group_or_capable(idmap, inode, f_i_gid_into_vfsgid(idmap, inode)))
			mode &= ~S_ISGID;
		 ...
		 struct iattr *attr)
{
	...
	err = f_fscrypt_prepare_setattr(dentry, attr);
	if (err)
		return err;

	err = f_fsverity_prepare_setattr(dentry, attr);
	if (err)
		return err;

	if (f_is_quota_modification(idmap, inode, attr)) {
		err = f2fs_dquot_initialize(inode);
		if (err)
			return err;
	}
	if (f_i_uid_needs_update(idmap, attr, inode) ||
	    f_i_gid_needs_update(idmap, attr, inode)) {
		f2fs_lock_op(F2FS_I_SB(inode));
		err = f_dquot_transfer(idmap, inode, attr);
		if (err) {
...
static int f2fs_punch_hole(struct inode *inode, loff_t offset, loff_t len)
{
	...
	if (pg_start == pg_end) {
		...
		if (pg_start < pg_end) {
			...
			f_truncate_pagecache_range(inode, blk_start, blk_end - 1);
								...
								int mode)
{
	...
	if (pg_start == pg_end) {
		...
		for (index = pg_start; index < pg_end;) {
			...
			f_truncate_pagecache_range(inode,
...
static int f2fs_setflags_common(struct inode *inode, u32 iflags, u32 mask)
{
	...
	f_inode_set_ctime_current(inode);
...
static int f2fs_ioc_start_atomic_write(struct file *filp, bool truncate)
{
	...
	if (!f_inode_owner_or_capable(idmap, inode))
		return -EACCES;
	...
	ret = f_mnt_want_write_file(filp);
	if (ret)
		return ret;
	...
	if (fi->cow_inode == NULL) {
		...
		f_invalidate_mapping_pages(fi->cow_inode->i_mapping, 0, -1);
						...
						bool readonly, bool need_lock)
{
	...
	case F2FS_GOING_DOWN_FULLSYNC:
		ret = f_bdev_freeze(sb->s_bdev);
		if (ret)
			goto out;
		f2fs_stop_checkpoint(sbi, false, STOP_CP_REASON_SHUTDOWN);
		f_bdev_thaw(sb->s_bdev);
		break;
...
static int f2fs_ioc_set_encryption_policy(struct file *filp, unsigned long arg)
{
	...
	ret = f_fscrypt_ioctl_set_policy(filp, (const void __user *)arg);
	f2fs_update_time(F2FS_I_SB(inode), REQ_TIME);
	return ret;
}
...
static int f2fs_ioc_get_encryption_policy(struct file *filp, unsigned long arg)
{
	if (!f2fs_sb_has_encrypt(F2FS_I_SB(file_inode(filp))))
		return -EOPNOTSUPP;
	return f_fscrypt_ioctl_get_policy(filp, (void __user *)arg);
}
...
static int f2fs_ioc_get_encryption_pwsalt(struct file *filp, unsigned long arg)
{
	...
	err = f_mnt_want_write_file(filp);
	if (err)
		return err;
	...
	f_generate_random_uuid(sbi->raw_super->encrypt_pw_salt);
					     ...
					     unsigned long arg)
{
	...
	return f_fscrypt_ioctl_get_policy_ex(filp, (void __user *)arg);
}
...
static int f2fs_ioc_add_encryption_key(struct file *filp, unsigned long arg)
{
	...
	return f_fscrypt_ioctl_add_key(filp, (void __user *)arg);
}
...
static int f2fs_ioc_remove_encryption_key(struct file *filp, unsigned long arg)
{
	...
	return f_fscrypt_ioctl_remove_key(filp, (void __user *)arg);
}
						    ...
						    unsigned long arg)
{
	...
	return f_fscrypt_ioctl_remove_key_all_users(filp, (void __user *)arg);
}
					      ...
					      unsigned long arg)
{
	...
	return f_fscrypt_ioctl_get_key_status(filp, (void __user *)arg);
}
...
static int f2fs_ioc_get_encryption_nonce(struct file *filp, unsigned long arg)
{
	...
	return f_fscrypt_ioctl_get_nonce(filp, (void __user *)arg);
}
...
out_src:
	...
	inode_set_mtime_to_ts(src, f_inode_set_ctime_current(src));
	f2fs_mark_inode_dirty_sync(src, false);
	if (src != dst) {
		inode_set_mtime_to_ts(dst, f_inode_set_ctime_current(dst));
...
int f2fs_transfer_project_quota(struct inode *inode, kprojid_t kprojid)
{
	...
	transfer_to[PRJQUOTA] = f_dqget(sb, make_kqid_projid(kprojid));
	if (IS_ERR(transfer_to[PRJQUOTA]))
		return PTR_ERR(transfer_to[PRJQUOTA]);

	err = f___dquot_transfer(inode, transfer_to);
	if (err)
		set_sbi_flag(sbi, SBI_QUOTA_NEED_REPAIR);
	f_dqput(transfer_to[PRJQUOTA]);
	return err;
}
...
int f2fs_fileattr_get(struct dentry *dentry, struct fileattr *fa)
{
	...
	f_fileattr_fill_flags(fa, fsflags & F2FS_GETTABLE_FS_FL);
...
static int f2fs_ioc_enable_verity(struct file *filp, unsigned long arg)
{
	...
	return f_fsverity_ioctl_enable(filp, (const void __user *)arg);
}
...
static int f2fs_ioc_measure_verity(struct file *filp, unsigned long arg)
{
	...
	return f_fsverity_ioctl_measure(filp, (void __user *)arg);
}
...
static int f2fs_ioc_read_verity_metadata(struct file *filp, unsigned long arg)
{
	...
	return f_fsverity_ioctl_read_metadata(filp, (const void __user *)arg);
}
...
static int f2fs_ioc_setfslabel(struct file *filp, unsigned long arg)
{
	...
	vbuf = f_strndup_user((const char __user *)arg, FSLABEL_MAX);
	if (IS_ERR(vbuf))
		return PTR_ERR(vbuf);
	...
	f_utf8s_to_utf16s(vbuf, strlen(vbuf), UTF16_LITTLE_ENDIAN,
		...
		pgoff_t off, block_t block, block_t len, u32 flags)
{
	...
	if (flags & F2FS_TRIM_FILE_DISCARD) {
		if (bdev_max_secure_erase_sectors(bdev))
			ret = f_blkdev_issue_secure_erase(bdev, sector, nr_sects,
					GFP_NOFS);
		else
	...
	if (!ret && (flags & F2FS_TRIM_FILE_ZEROOUT)) {
		if (IS_ENCRYPTED(inode))
			ret = f_fscrypt_zeroout_range(inode, off, block, len);
		else
			ret = f_blkdev_issue_zeroout(bdev, sector, nr_sects,
					GFP_NOFS, 0);
	}
...
static int f2fs_sec_trim_file(struct file *filp, unsigned long arg)
{
	...
	f_truncate_inode_pages_range(mapping, range.start,
			to_end ? -1 : end_addr - 1);
...
static int redirty_blocks(struct inode *inode, pgoff_t page_idx, int len)
{
	...
	f_page_cache_ra_unbounded(&ractl, len, 0);
...
static ssize_t f2fs_dio_read_iter(struct kiocb *iocb, struct iov_iter *to)
{
	...
	dio = f___iomap_dio_rw(iocb, to, &f2fs_iomap_ops,
				     ...
				     size_t len, unsigned int flags)
{
	...
	ret = f_filemap_splice_read(in, ppos, pipe, len, flags);
	if (ret > 0)
		f2fs_update_iostat(F2FS_I_SB(inode), inode,
				   APP_BUFFERED_READ_IO, ret);
					...
					struct iov_iter *from)
{
	...
	ret = f_generic_perform_write(iocb, from);
				      ...
				      loff_t start_pos, loff_t end_pos)
{
	...
	if (ret < 0)
		return;
	f_invalidate_mapping_pages(mapping,
				   ...
				   bool *may_need_sync)
{
	...
	dio = f___iomap_dio_rw(iocb, from, &f2fs_iomap_ops,
		...
		int advice)
{
	...
	if (advice == POSIX_FADV_SEQUENTIAL) {
		...
		bdi = f_inode_to_bdi(mapping->host);
	...
	err = f_generic_fadvise(filp, offset, len, advice);
	if (err)
		return err;
