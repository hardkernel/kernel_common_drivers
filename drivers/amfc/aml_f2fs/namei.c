						...
						const char *name)
{
	...
	err = f_fscrypt_prepare_new_inode(dir, inode, &encrypt);
	if (err)
		goto fail_drop;
		       ...
		       struct dentry *dentry, umode_t mode, bool excl)
{
	...
	f_d_instantiate_new(dentry, inode);
		...
		struct dentry *dentry)
{
	...
	err = f_fscrypt_prepare_link(old_dentry, dir, dentry);
	if (err)
		return err;
...
{
	struct page *page;
	unsigned long ino = f2fs_inode_by_name(d_inode(child), dotdot_name_t, &page);
		...
		unsigned int flags)
{
	...
	if (IS_ENCRYPTED(dir) &&
	    ...
	    !f_fscrypt_has_permitted_context(dir, inode)) {
			...
			struct dentry *dentry, const char *symname)
{
	...
	err = f_fscrypt_prepare_symlink(dir, symname, len, dir->i_sb->s_blocksize,
				      &disk_link);
	if (err)
		return err;
	...
	err = f_fscrypt_encrypt_symlink(inode, symname, len, &disk_link);
	if (err)
		goto err_out;

	err = f_page_symlink(inode, disk_link.name, disk_link.len);
			...
			struct file *file, umode_t mode)
{
	...
	return f_finish_open_simple(file, err);
}
			...
			struct dentry *new_dentry, unsigned int flags)
{
	...
	if (new_inode) {
		...
		f_inode_set_ctime_current(new_inode);
	...
	f_inode_set_ctime_current(old_inode);
			     ...
			     struct inode *new_dir, struct dentry *new_dentry)
{
	...
	f_inode_set_ctime_current(old_dir);
	if (old_nlink) {
	...
	f_inode_set_ctime_current(new_dir);
	if (new_nlink) {
			...
			unsigned int flags)
{
	...
	err = f_fscrypt_prepare_rename(old_dir, old_dentry, new_dir, new_dentry,
				     flags);
	if (err)
		return err;
					   ...
					   struct delayed_call *done)
{
	...
	target = f_fscrypt_get_symlink(inode, page_address(page),
					  ...
					  unsigned int query_flags)
{
	...
	return f_fscrypt_symlink_getattr(path, stat);
}
