...
#ifdef CONFIG_AMLOGIC_F2FS_UNFAIR_RWSEM
...
struct f2fs_sb_info {
	 ...
	 */
#ifdef CONFIG_AMLOGIC_F2FS_STAT_FS
...
#ifdef CONFIG_AMLOGIC_F2FS_IOSTAT
					...
					struct inode *inode, bool cap)
{
	...
	if (!gid_eq(F2FS_OPTION(sbi).s_resgid, GLOBAL_ROOT_GID) &&
					f_in_group_p(F2FS_OPTION(sbi).s_resgid))
		return true;
				 ...
				 struct inode *inode, blkcnt_t *count, bool partial)
{
	...
	ret = f_dquot_reserve_block(inode, *count);
	if (ret)
		return ret;
	...
	if (unlikely(release)) {
		...
		f_dquot_release_reservation_block(inode, release);
	}
					...
					struct inode *inode, bool is_inode)
{
	...
	if (is_inode) {
		if (inode) {
			err = f_dquot_alloc_inode(inode);
			if (err)
				return err;
		}
	} else {
		err = f_dquot_reserve_block(inode, 1);
		if (err)
			return err;
	}
...
enospc:
	if (is_inode) {
		if (inode)
			f_dquot_free_inode(inode);
	} else {
		f_dquot_release_reservation_block(inode, 1);
	}
	return -ENOSPC;
}
...
{
	return f_percpu_counter_sum_positive(&sbi->total_valid_inode_count);
}
		...
		pgoff_t index, bool for_write)
{
	...
	if (IS_ENABLED(CONFIG_AMLOGIC_F2FS_FAULT_INJECTION)) {
		fgf_t fgf_flags;
		...
		if (!for_write)
			...
			fgf_flags = FGP_LOCK;
		folio = f___filemap_get_folio(mapping, index, fgf_flags, 0);
		if (!IS_ERR(folio))
			return folio;
	...
	if (!for_write)
		return f_filemap_grab_folio(mapping, index);

	flags = memalloc_nofs_save();
	folio = f___filemap_get_folio(mapping, index, FGP_WRITEBEGIN,
					...
					block_t diff, bool add, bool claim)
{
	...
	if (add) {
		if (claim)
			f_dquot_claim_block(inode, diff);
		else
			f_dquot_alloc_block_nofail(inode, diff);
	} else {
		f_dquot_free_block(inode, diff);
	}
...
extern struct file_operations f2fs_file_operations;
extern const struct inode_operations f2fs_file_inode_operations;
extern struct address_space_operations f2fs_dblock_aops;
extern struct address_space_operations f2fs_node_aops;
extern struct address_space_operations f2fs_meta_aops;
					...
					block_t blkaddr, unsigned int cnt)
{
	...
	f_truncate_inode_pages_range(META_MAPPING(sbi),
