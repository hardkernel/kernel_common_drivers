			   ...
			   loff_t pos)
{
	...
	while (count) {
		...
		f_memcpy_to_folio(folio, offset_in_folio(folio, pos), buf, n);
...
cleanup:
	...
	f_truncate_inode_pages(inode->i_mapping, inode->i_size);
					       ...
					       unsigned long num_ra_pages)
{
	...
	folio = f___filemap_get_folio(inode->i_mapping, index, FGP_ACCESSED, 0);
	if (IS_ERR(folio) || !folio_test_uptodate(folio)) {
		...
		if (!IS_ERR(folio))
			...
			f_page_cache_ra_unbounded(&ractl, num_ra_pages, 0);
		folio = f_read_mapping_folio(inode->i_mapping, index, NULL);
		if (IS_ERR(folio))
