		...
		struct writeback_control *wbc, bool redirty, int unlock)
{
	...
	for (i = 0; i < cc->cluster_size; i++) {
		if (!cc->rpages[i])
			continue;
		if (redirty)
			f_redirty_page_for_writepage(wbc, cc->rpages[i]);
...
static int lz4_compress_pages(struct compress_ctx *cc)
{
	...
	if (!level)
		len = f_LZ4_compress_default(cc->rbuf, cc->cbuf->cdata, cc->rlen,
						cc->clen, cc->private);
#ifdef CONFIG_AMLOGIC_F2FS_FS_LZ4HC
	else
		len = f_LZ4_compress_HC(cc->rbuf, cc->cbuf->cdata, cc->rlen,
...
{
	return lvl >= f_zstd_min_clevel() && lvl <= f_zstd_max_clevel();
}
...
static const struct f2fs_compress_ops *f2fs_cops[COMPRESS_MAX] = {
	...
	NULL,
#endif
#if defined(CONFIG_AMLOGIC_F2FS_FS_LZO) && defined(CONFIG_AMLOGIC_F2FS_FS_LZORLE)
	&f2fs_lzorle_ops,
#else
	NULL,
#endif
...
{
	compress_page_pool = f_mempool_create_page_pool(num_compress_pages, 0);
	return compress_page_pool ? 0 : -ENOMEM;
}

void f2fs_destroy_compress_mempool(void)
{
	f_mempool_destroy(compress_page_pool);
}
...
static struct page *f2fs_compress_alloc_page(void)
{
	...
	page = f_mempool_alloc(compress_page_pool, GFP_NOFS);
	lock_page(page);
...
static void f2fs_compress_free_page(struct page *page)
{
	...
	page->mapping = NULL;
	unlock_page(page);
	f_mempool_free(page, compress_page_pool);
}
...
{
#ifdef CONFIG_AMLOGIC_F2FS_CHECK_FS
...
static void set_cluster_writeback(struct compress_ctx *cc)
{
	...
	for (i = 0; i < cc->cluster_size; i++) {
		if (cc->rpages[i])
			f_set_page_writeback(cc->rpages[i]);
	}
}
			...
			struct compress_io_ctx *cic, int submitted)
{
	...
	for (i = 0; i < cc->cluster_size; i++) {
		...
		if (folio_test_writeback(page_folio(cc->rpages[i])))
			f_end_page_writeback(cc->rpages[i]);
	}
}
					...
					enum iostat_type io_type)
{
	...
	struct f2fs_io_info fio = {
		...
		.io_type = io_type,
		.io_wbc = wbc,
		.encrypted = f_fscrypt_inode_uses_fs_layer_crypto(cc->inode) ?
									1 : 0,
	};
	struct folio *folio;
	struct dnode_of_data dn;
	...
	if (unlikely(f2fs_cp_error(sbi))) {
		f_mapping_set_error(inode->i_mapping, -EIO);
		goto out_free;
	}
...
out_destroy_crypt:
	...
	for (--i; i >= 0; i--) {
		if (!cc->cpages[i])
			continue;
		f_fscrypt_finalize_bounce_page(&cc->cpages[i]);
	}
out_put_cic:
...
void f2fs_compress_write_end_io(struct bio *bio, struct page *page)
{
	...
	if (unlikely(bio->bi_status))
		f_mapping_set_error(cic->inode->i_mapping, -EIO);
	...
	for (i = 0; i < cic->nr_rpages; i++) {
		...
		f_end_page_writeback(cic->rpages[i]);
	}
...
continue_unlock:
		...
		if (!f_clear_page_dirty_for_io(cc->rpages[i]))
			goto continue_unlock;
...
static void f2fs_verify_cluster(struct work_struct *work)
{
	...
	for (i = 0; i < dic->cluster_size; i++) {
		...
		if (f_fsverity_verify_page(rpage))
				...
				bool in_task)
{
	...
	if (!failed && dic->need_verity) {
		...
		f_fsverity_enqueue_verify_work(&dic->verity_work);
		return;
	}
...
struct address_space_operations f2fs_compress_aops = {
				...
				block_t blkaddr, unsigned int len)
{
	if (!sbi->compress_inode)
		return;
	f_invalidate_mapping_pages(COMPRESS_MAPPING(sbi), blkaddr, blkaddr + len - 1);
}
...
void f2fs_invalidate_compress_pages(struct f2fs_sb_info *sbi, nid_t ino)
{
	...
	do {
		...
		nr = f_filemap_get_folios(mapping, &index, end - 1, &fbatch);
		if (!nr)
			break;
		...
		for (i = 0; i < nr; i++) {
			...
			f_generic_error_remove_folio(mapping, folio);
			f_folio_unlock(folio);
		}
