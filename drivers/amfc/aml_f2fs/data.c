...
{
	return f_bioset_init(&f2fs_bioset, F2FS_BIO_POOL_SIZE,
					0, BIOSET_NEED_BVECS);
}

void f2fs_destroy_bioset(void)
{
	f_bioset_exit(&f2fs_bioset);
}
...
static void f2fs_finish_read_bio(struct bio *bio, bool in_task)
{
	...
	bio_for_each_folio_all(fi, bio) {
		...
		f_folio_end_read(folio, bio->bi_status == 0);
	}

	if (ctx)
		f_mempool_free(ctx, bio_post_read_ctx_pool);
	bio_put(bio);
}
	...
	if (may_have_compressed_pages) {
		...
		bio_for_each_segment_all(bv, bio, iter_all) {
			...
			if (!f2fs_is_compressed_page(page) &&
			    !f_fsverity_verify_page(page)) {
				bio->bi_status = BLK_STS_IOERR;
				break;
			}
		}
	} else {
		f_fsverity_verify_bio(bio);
	}
...
static void f2fs_verify_and_finish_bio(struct bio *bio, bool in_task)
{
	...
	if (ctx && (ctx->enabled_steps & STEP_VERITY)) {
		...
		f_fsverity_enqueue_verify_work(&ctx->work);
	} else {
...
static void f2fs_post_read_work(struct work_struct *work)
{
	...
	if ((ctx->enabled_steps & STEP_DECRYPT) && !f_fscrypt_decrypt_bio(bio)) {
		f2fs_finish_read_bio(bio, true);
		return;
	}
...
static void f2fs_write_end_io(struct bio *bio)
{
	...
	bio_for_each_folio_all(fi, bio) {
		...
		if (fscrypt_is_bounce_folio(folio)) {
			...
			f_fscrypt_free_bounce_page(&io_folio->page);
		}
		...
		if (unlikely(bio->bi_status)) {
			f_mapping_set_error(folio->mapping, -EIO);
				  ...
				  gfp_t gfp_mask)
{
	...
	if (!fio || !fio->encrypted_page)
		f_fscrypt_set_bio_crypt_ctx(bio, inode, first_idx, gfp_mask);
				     ...
				     const struct f2fs_io_info *fio)
{
	...
	return f_fscrypt_mergeable_bio(bio, inode, next_idx);
}
...
int f2fs_submit_page_bio(struct f2fs_io_info *fio)
{
	...
	f_bio_add_folio_nofail(bio, data_folio, folio_size(data_folio), 0);

	if (fio->io_wbc && !is_read_io(fio->op))
		f_wbc_account_cgroup_owner(fio->io_wbc, fio_folio, PAGE_SIZE);
...
alloc_new:
	...
	if (fio->io_wbc)
		f_wbc_account_cgroup_owner(fio->io_wbc, folio, folio_size(folio));
...
next:
	...
	if (f2fs_sb_has_blkzoned(sbi) && btype < META && io->zone_pending_bio) {
		f_wait_for_completion_io(&io->zone_wait);
...
alloc_new:
	...
	if (fio->io_wbc)
		f_wbc_account_cgroup_owner(fio->io_wbc, page_folio(fio->page),
					 PAGE_SIZE);
				      ...
				      pgoff_t first_idx, bool for_write)
{
	...
	if (f_fscrypt_inode_uses_fs_layer_crypto(inode))
	...
	if (post_read_steps || f2fs_compressed_file(inode)) {
		...
		ctx = f_mempool_alloc(bio_post_read_ctx_pool, GFP_NOFS);
		ctx->bio = bio;
		ctx->sbi = sbi;
				 ...
				 bool for_write)
{
	...
	if (!f_bio_add_folio(bio, folio, PAGE_SIZE, 0)) {
		iostat_update_and_unbind_ctx(bio);
		if (bio->bi_private)
			f_mempool_free(bio->bi_private, bio_post_read_ctx_pool);
		bio_put(bio);
		return -EFAULT;
	}
					...
					pgoff_t *next_pgofs)
{
	...
	folio = f___filemap_get_folio(mapping, index, FGP_ACCESSED, 0);
	if (IS_ERR(folio))
		goto read;
	if (folio_test_uptodate(folio))
		return folio;
					...
					struct readahead_control *rac)
{
	...
	sector_t block_nr;
	pgoff_t index = f_folio_index(folio);
	int ret = 0;
...
got_it:
	if ((map->m_flags & F2FS_MAP_MAPPED)) {
		...
		if (!!folio_test_uptodate(folio) && (!folio_test_swapcache(folio) &&
					!f_cleancache_get_page(&folio->page))) {
			folio_mark_uptodate(folio);
			goto confused;
		}
...
submit_and_realloc:
	...
	if (!f_bio_add_folio(bio, folio, blocksize, 0))
...
int f2fs_encrypt_one_page(struct f2fs_io_info *fio)
{
	...
	if (f_fscrypt_inode_uses_inline_crypto(inode))
		return 0;

retry_encrypt:
	fio->encrypted_page = f_fscrypt_encrypt_pagecache_blocks(page,
...
got_it:
	...
	if (ipu_force ||
		...
		if (err) {
			if (f_fscrypt_inode_uses_fs_layer_crypto(inode))
				f_fscrypt_finalize_bounce_page(&fio->encrypted_page);
			f_folio_end_writeback(folio);
		} else {
...
redirty_out:
	f_folio_redirty_for_writepage(wbc, folio);
	/*
...
retry:
	retry = 0;
	if (wbc->sync_mode == WB_SYNC_ALL || wbc->tagged_writepages)
		f_tag_pages_for_writeback(mapping, index, end);
	done_index = index;
	while (!done && !retry && (index <= end)) {
		nr_pages = 0;
again:
		nr_folios = f_filemap_get_folios_tag(mapping, &index, end,
				tag, &fbatch);
		if (nr_folios == 0) {
			if (nr_pages)
				goto write;
...
continue_unlock:
			...
			if (!f_folio_clear_dirty_for_io(folio))
				goto continue_unlock;
...
next:
		...
		f_release_pages(pages, nr_pages);
		cond_resched();
	}
#ifdef CONFIG_AMLOGIC_F2FS_FS_COMPRESSION
...
repeat:
	 ...
	 */
	folio = f___filemap_get_folio(mapping, index,
		...
		struct folio *folio)
{
	...
	if (f_filemap_dirty_folio(mapping, folio)) {
...
retry:
		 ...
		 */
		ret = f_add_swap_extent(sis, cur_lblock, nr_pblocks, pblock);
		if (ret < 0)
			goto out;
		nr_extents += ret;
...
void f2fs_clear_page_cache_dirty_tag(struct folio *folio)
{
	...
	f___xa_clear_mark(&mapping->i_pages, folio->index,
						PAGECACHE_TAG_DIRTY);
...
int __init f2fs_init_post_read_processing(void)
{
	...
	bio_post_read_ctx_pool =
		f_mempool_create_slab_pool(NUM_PREALLOC_POST_READ_CTXS,
					 bio_post_read_ctx_cache);
...
{
	f_mempool_destroy(bio_post_read_ctx_pool);
	kmem_cache_destroy(bio_post_read_ctx_cache);
}
			    ...
			    struct iomap *srcmap)
{
	 ...
	 */
	map.m_len = f_fscrypt_limit_io_blocks(inode, map.m_lblk, map.m_len);
