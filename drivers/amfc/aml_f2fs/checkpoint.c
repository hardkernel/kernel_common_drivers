...
redirty_out:
	f_redirty_page_for_writepage(wbc, page);
				...
				long nr_to_write, enum iostat_type io_type)
{
	...
	while ((nr_folios = f_filemap_get_folios_tag(mapping, &index,
					(pgoff_t)-1,
...
static void __prepare_cp_block(struct f2fs_sb_info *sbi)
{
	...
	f_percpu_counter_set(&sbi->alloc_valid_block_count, 0);
	f_percpu_counter_set(&sbi->rf_node_block_count, 0);
}
...
void f2fs_wait_on_all_pages(struct f2fs_sb_info *sbi, int type)
{
	...
	for (;;) {
		...
		f_prepare_to_wait(&sbi->cp_wait, &wait, TASK_UNINTERRUPTIBLE);
		f_io_schedule_timeout(DEFAULT_IO_TIMEOUT);
	}
	...
	void *src, block_t blk_addr)
{
	...
	set_page_dirty(page);
	if (unlikely(!f_clear_page_dirty_for_io(page)))
		f2fs_bug_on(sbi, 1);
...
static int do_checkpoint(struct f2fs_sb_info *sbi, struct cp_control *cpc)
{
	...
	*((__le32 *)((unsigned char *)ckpt +
	...
	if (f2fs_sb_has_encrypt(sbi) || f2fs_sb_has_verity(sbi) ||
		...
		f2fs_bug_on(sbi,
			f_invalidate_inode_pages2_range(META_MAPPING(sbi),
...
static void __checkpoint_and_complete_reqs(struct f2fs_sb_info *sbi)
{
	...
	if (!dispatch_list)
		return;
	dispatch_list = f_llist_reverse_order(dispatch_list);
...
int f2fs_issue_checkpoint(struct f2fs_sb_info *sbi)
{
	...
	f_llist_add(&req.llnode, &cprc->issue_list);
...
int f2fs_start_ckpt_thread(struct f2fs_sb_info *sbi)
{
	...
	f_set_task_ioprio(cprc->f2fs_issue_ckpt, cprc->ckpt_thread_ioprio);
