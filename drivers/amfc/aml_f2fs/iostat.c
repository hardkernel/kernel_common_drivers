...
void iostat_update_and_unbind_ctx(struct bio *bio)
{
	...
	f_mempool_free(iostat_ctx, bio_iostat_ctx_pool);
}
		...
		struct bio *bio, struct bio_post_read_ctx *ctx)
{
	...
	iostat_ctx = f_mempool_alloc(bio_iostat_ctx_pool, GFP_NOFS);
	iostat_ctx->sbi = sbi;
...
int __init f2fs_init_iostat_processing(void)
{
	...
	if (!bio_iostat_ctx_cache)
		goto fail;
	bio_iostat_ctx_pool =
		f_mempool_create_slab_pool(NUM_PREALLOC_IOSTAT_CTXS,
					 bio_iostat_ctx_cache);
...
{
	f_mempool_destroy(bio_iostat_ctx_pool);
	kmem_cache_destroy(bio_iostat_ctx_cache);
}
