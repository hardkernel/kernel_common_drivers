...
extern struct address_space_operations f2fs_compress_aops;
#endif
...
void f2fs_set_inode_flags(struct inode *inode)
{
	...
	if (flags & F2FS_CASEFOLD_FL)
		new_fl |= S_CASEFOLD;
	f_inode_set_flags(inode, new_fl,
			...
			struct f2fs_inode *ri)
{
	...
	case COMPRESS_ZSTD:
		...
		if (clevel < f_zstd_min_clevel() || clevel > f_zstd_max_clevel())
			goto err_level;
#endif
		break;
	default:
...
#ifdef CONFIG_AMLOGIC_F2FS_FS_COMPRESSION
		f2fs_compress_aops.migrate_folio    = f_filemap_migrate_folio;
...
retry:
	...
	if (err) {
		f2fs_update_inode_page(inode);
		if (f_dquot_initialize_needed(inode))
			set_sbi_flag(sbi, SBI_QUOTA_NEED_REPAIR);
	}
	if (freeze_protected)
...
no_delete:
	...
	if (inode->i_ino)
		f_invalidate_mapping_pages(NODE_MAPPING(sbi), inode->i_ino,
							inode->i_ino);
	if (xnid)
		f_invalidate_mapping_pages(NODE_MAPPING(sbi), xnid, xnid);
	if (inode->i_nlink) {
...
out_clear:
	f_fscrypt_put_encryption_info(inode);
	f_fsverity_cleanup_inode(inode);
	clear_inode(inode);
}
