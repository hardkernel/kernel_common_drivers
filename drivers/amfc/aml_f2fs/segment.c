...
void f2fs_balance_fs(struct f2fs_sb_info *sbi, bool need)
{
	...
	if (test_opt(sbi, GC_MERGE) && sbi->gc_thread &&
		...
		f_prepare_to_wait(&sbi->gc_thread->fggc_wq, &wait,
...
{
	int ret = f_blkdev_issue_flush(bdev);
...
repeat:
	...
	if (!llist_empty(&fcc->issue_list)) {
		...
		fcc->dispatch_list = f_llist_reverse_order(fcc->dispatch_list);
...
int f2fs_issue_flush(struct f2fs_sb_info *sbi, nid_t ino)
{
	...
	f_llist_add(&cmd.llnode, &fcc->issue_list);
...
static void f2fs_submit_discard_endio(struct bio *bio)
{
	...
	spin_lock_irqsave(&dc->lock, flags);
	if (!dc->error)
		dc->error = f_blk_status_to_errno(bio->bi_status);
	dc->bio_ref--;
				...
				struct discard_cmd *dc, int *issued)
{
	...
	while (total_len && *issued < dpolicy->max_requests && !err) {
		...
		if (time_to_inject(sbi, FAULT_DISCARD)) {
			...
			err = f___blkdev_issue_discard(bdev,
							...
							struct discard_cmd *dc)
{
	...
	f_wait_for_completion_io(&dc->wait);
	mutex_lock(&dcc->cmd_lock);
...
next:
	...
	list_for_each_entry_safe(iter, tmp, wait_list, list) {
		...
		if (iter->state == D_DONE && !iter->ref) {
			f_wait_for_completion_io(&iter->wait);
			if (!iter->error)
				trimmed += iter->di.len;
		...
		struct block_device *bdev, block_t blkstart, block_t blklen)
{
	...
	if (f2fs_blkz_is_seq(sbi, devi, blkstart)) {
		...
		if (unlikely(is_sbi_flag_set(sbi, SBI_POR_DOING))) {
			...
			ret = f_blkdev_zone_mgmt(bdev, REQ_OP_ZONE_RESET,
						sector, nr_sects);
	...
	if (__is_large_section(sbi)) {
		if (f2fs_need_rand_seg(sbi)) {
			...
			if (GET_SEC_FROM_SEG(sbi, curseg->segno + 1) != hint)
				return curseg->segno;
			return f_get_random_u32_inclusive(curseg->segno + 1,
					GET_SEG_FROM_SEC(sbi, hint + 1) - 1);
		}
		return curseg->segno;
	} else if (f2fs_need_rand_seg(sbi)) {
		return f_get_random_u32_below(MAIN_SECS(sbi) * SEGS_PER_SEC(sbi));
	}
...
static int new_curseg(struct f2fs_sb_info *sbi, int type, bool new_sec)
{
	...
	if (F2FS_OPTION(sbi).fs_mode == FS_MODE_FRAGMENT_BLK)
		curseg->fragment_remained_chunk =
				f_get_random_u32_inclusive(1, sbi->max_fragment_chunk);
	return 0;
}
		...
		struct curseg_info *seg)
{
	...
	seg->fragment_remained_chunk =
		f_get_random_u32_inclusive(1, sbi->max_fragment_chunk);
	seg->next_blkoff +=
		f_get_random_u32_inclusive(1, sbi->max_fragment_hole);
}
...
static void do_write_page(struct f2fs_summary *sum, struct f2fs_io_info *fio)
{
	...
	if (f2fs_allocate_data_block(fio->sbi, fio->page, fio->old_blkaddr,
		...
		if (f_fscrypt_inode_uses_fs_layer_crypto(folio->mapping->host))
				    ...
				    struct blk_zone *zone)
{
	...
	f2fs_notice(sbi, "Valid blocks are not aligned with write "
		    ...
		    zone_segno, valid_block_cnt, f_blk_zone_cond_str(zone->cond));

	nofs_flags = memalloc_nofs_save();
	ret = f_blkdev_zone_mgmt(fdev->bdev, REQ_OP_ZONE_FINISH,
				zone->start, zone->len);
	memalloc_nofs_restore(nofs_flags);
	if (ret == -EOPNOTSUPP) {
		ret = f_blkdev_issue_zeroout(fdev->bdev, zone->wp,
...
static int do_fix_curseg_write_pointer(struct f2fs_sb_info *sbi, int type)
{
	...
	err = f_blkdev_report_zones(zbd->bdev, zone_sector, 1,
...
static int check_write_pointer(struct f2fs_sb_info *sbi)
{
	...
	for (i = 0; i < sbi->s_ndevs; i++) {
		...
		args.sbi = sbi;
		args.fdev = &FDEV(i);
		ret = f_blkdev_report_zones(FDEV(i).bdev, 0, BLK_ALL_ZONES,
