		...
		size_t size, int flags)
{
	...
	if (!f_inode_owner_or_capable(nop_mnt_idmap_t, inode))
		return -EPERM;
	if (value == NULL)
		return -EINVAL;
...
#ifdef CONFIG_AMLOGIC_F2FS_FS_SECURITY
...
{
	return f_security_inode_init_security(inode, dir, qstr,
					    ...
					    struct dentry *dentry)
{
	...
	f2fs_xattr_handler_map[F2FS_XATTR_INDEX_POSIX_ACL_ACCESS] = nop_posix_acl_access_t;
	f2fs_xattr_handler_map[F2FS_XATTR_INDEX_POSIX_ACL_DEFAULT] = nop_posix_acl_default_t;
