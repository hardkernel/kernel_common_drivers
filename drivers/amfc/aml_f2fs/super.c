...
#ifdef CONFIG_AMLOGIC_F2FS_FAULT_INJECTION
					  ...
					  bool is_remount)
{
	...
	err = f_fscrypt_parse_test_dummy_encryption(&param, policy);
	if (err) {
		if (err == -EEXIST)
			f2fs_warn(sbi,
...
#ifdef CONFIG_AMLOGIC_F2FS_FS_COMPRESSION
...
#ifdef CONFIG_AMLOGIC_F2FS_FS_LZ4
static int f2fs_set_lz4hc_level(struct f2fs_sb_info *sbi, const char *str)
{
#ifdef CONFIG_AMLOGIC_F2FS_FS_LZ4HC
	unsigned int level;
...
#ifdef CONFIG_AMLOGIC_F2FS_FS_ZSTD
...
static int parse_options(struct f2fs_sb_info *sbi, char *options, bool is_remount)
{
	...
	while ((p = strsep(&options, ",")) != NULL) {
		...
		token = f_match_token(p, f2fs_tokens, args);

		switch (token) {
		case Opt_gc_background:
			name = f_match_strdup(&args[0]);
		...
		case Opt_heap:
			...
			break;
#ifdef CONFIG_AMLOGIC_F2FS_FS_XATTR
		case Opt_user_xattr:
		...
		case Opt_inline_xattr_size:
			if (args->from && f_match_int(args, &arg))
				return -EINVAL;
		...
		case Opt_inline_xattr_size:
			...
			break;
#endif
#ifdef CONFIG_AMLOGIC_F2FS_FS_POSIX_ACL
		case Opt_acl:
		...
		case Opt_compress_algorithm:
			...
			if (!name)
				return -ENOMEM;
			if (!strcmp(name, "lzo")) {
#ifdef CONFIG_AMLOGIC_F2FS_FS_LZO
				F2FS_OPTION(sbi).compress_level = 0;
...
#ifdef CONFIG_AMLOGIC_F2FS_FS_LZORLE
				F2FS_OPTION(sbi).compress_level = 0;
				F2FS_OPTION(sbi).compress_algorithm =
								COMPRESS_LZORLE;
#else
...
static struct inode *f2fs_alloc_inode(struct super_block *sb)
{
	...
	fi = f_alloc_inode_sb(sb, f2fs_inode_cachep, GFP_F2FS_ZERO);
	if (!fi)
		return NULL;
...
static int f2fs_drop_inode(struct inode *inode)
{
	...
	if ((!inode_unhashed(inode) && inode->i_state & I_SYNC)) {
		if (!inode->i_nlink && !is_bad_inode(inode)) {
			...
			f_sb_start_intwrite(inode->i_sb);
	...
	if (!ret)
		ret = f_fscrypt_drop_inode(inode);
...
{
	f_fscrypt_free_inode(inode);
	kmem_cache_free(f2fs_inode_cachep, F2FS_I(inode));
}

static void destroy_percpu_info(struct f2fs_sb_info *sbi)
{
	f_percpu_counter_destroy(&sbi->total_valid_inode_count);
	f_percpu_counter_destroy(&sbi->rf_node_block_count);
	f_percpu_counter_destroy(&sbi->alloc_valid_block_count);
}
					   ...
					   struct super_block *sb)
{
	...
	if (F2FS_OPTION(sbi).s_qf_names[USRQUOTA])
		f_seq_show_option(seq, "usrjquota",
			F2FS_OPTION(sbi).s_qf_names[USRQUOTA]);

	if (F2FS_OPTION(sbi).s_qf_names[GRPQUOTA])
		f_seq_show_option(seq, "grpjquota",
			F2FS_OPTION(sbi).s_qf_names[GRPQUOTA]);

	if (F2FS_OPTION(sbi).s_qf_names[PRJQUOTA])
		f_seq_show_option(seq, "prjjquota",
			F2FS_OPTION(sbi).s_qf_names[PRJQUOTA]);
#endif
}
...
static int f2fs_show_options(struct seq_file *seq, struct dentry *root)
{
	...
	f_fscrypt_show_test_dummy_encryption(seq, ',', sbi->sb);
...
repeat:
		...
		if (IS_ERR(page)) {
			if (PTR_ERR(page) == -ENOMEM) {
				f_memalloc_retry_wait(GFP_NOFS);
				goto repeat;
			}
...
retry:
		...
		f_memcpy_to_folio(folio, offset_in_folio(folio, off), data, tocopy);
	...
	if (len == towrite)
		return err;
	inode_set_mtime_to_ts(inode, f_inode_set_ctime_current(inode));
...
int f2fs_dquot_initialize(struct inode *inode)
{
	...
	return f_dquot_initialize(inode);
}
...
static int f2fs_quota_on_mount(struct f2fs_sb_info *sbi, int type)
{
	...
	return f_dquot_quota_on_mount(sbi->sb, F2FS_OPTION(sbi).s_qf_names[type],
			     ...
			     unsigned int flags)
{
	...
	err = f_dquot_load_quota_inode(qf_inode, type, format_id, flags);
	iput(qf_inode);
	return err;
}
...
static int f2fs_quota_sync_file(struct f2fs_sb_info *sbi, int type)
{
	...
	ret = f_dquot_writeback_dquots(sbi->sb, type);
	if (ret)
		goto out;
	...
	f_truncate_inode_pages(&dqopt->files[type]->i_data, 0);
out:
	if (ret)
							...
							const struct path *path)
{
	...
	err = f_dquot_quota_on(sb, type, format_id, path);
	if (err)
		goto out;
...
static int __f2fs_quota_off(struct super_block *sb, int type)
{
	...
	if (!inode || !f_igrab(inode))
		return f_dquot_quota_off(sb, type);

	err = f2fs_do_quota_sync(sb, type);
	if (err)
		goto out_put;

	err = f_dquot_quota_off(sb, type);
...
void f2fs_quota_off_umount(struct super_block *sb)
{
	...
	for (type = 0; type < MAXQUOTAS; type++) {
		err = __f2fs_quota_off(sb, type);
		if (err) {
			int ret = f_dquot_quota_off(sb, type);
...
static int f2fs_dquot_mark_dquot_dirty(struct dquot *dquot)
{
	...
	struct f2fs_sb_info *sbi = F2FS_SB(sb);
	int ret = f_dquot_mark_dquot_dirty(dquot);
...
{
	struct f2fs_sb_info *sbi = F2FS_SB(sb);
	int ret = f_dquot_commit_info(sb, type);
...
static struct dquot_operations f2fs_quota_operations = {
...
static struct quotactl_ops f2fs_quotactl_ops = {
...
static struct export_operations f2fs_export_ops = {
						...
						pgoff_t index, bool update)
{
	...
	folio_lock(folio);
	f_folio_wait_writeback(folio);
	if (update)
		memcpy(F2FS_SUPER_BLOCK(folio, index), F2FS_RAW_SUPER(sbi),
					sizeof(struct f2fs_super_block));
	f_folio_mark_dirty(folio);
	f_folio_clear_dirty_for_io(folio);
	f_folio_start_writeback(folio);
	f_folio_unlock(folio);

	bio = bio_alloc(sbi->sb->s_bdev, 1, opf, GFP_NOFS);

	/* it doesn't need to set crypto context for superblock update */
	bio->bi_iter.bi_sector = SECTOR_FROM_BLOCK(f_folio_index(folio));

	if (!f_bio_add_folio(bio, folio, folio_size(folio), 0))
		f2fs_bug_on(sbi, 1);

	ret = submit_bio_wait(bio);
	f_folio_end_writeback(folio);
...
static int init_percpu_info(struct f2fs_sb_info *sbi)
{
	...
	err = f_percpu_counter_init(&sbi->alloc_valid_block_count, 0, GFP_KERNEL);
	if (err)
		return err;

	err = f_percpu_counter_init(&sbi->rf_node_block_count, 0, GFP_KERNEL);
	if (err)
		goto err_valid_block;

	err = f_percpu_counter_init(&sbi->total_valid_inode_count, 0,
								GFP_KERNEL);
	if (err)
		goto err_node_block;
	return 0;
...
static int init_blkz_info(struct f2fs_sb_info *sbi, int devi)
{
	...
	ret = f_blkdev_report_zones(bdev, 0, BLK_ALL_ZONES, f2fs_report_zone_cb,
				  &rep_zone_arg);
	if (ret < 0)
		return ret;
	return 0;
			...
			int *valid_super_block, int *recovery)
{
	...
	for (block = 0; block < 2; block++) {
		folio = f_read_mapping_folio(sb->s_bdev->bd_mapping, block, NULL);
		if (IS_ERR(folio)) {
...
int f2fs_commit_super(struct f2fs_sb_info *sbi, bool recover)
{
	...
	folio = f_read_mapping_folio(sbi->sb->s_bdev->bd_mapping, index, NULL);
	if (IS_ERR(folio))
...
static int f2fs_scan_devices(struct f2fs_sb_info *sbi)
{
	...
	for (i = 0; i < max_devices; i++) {
		...
		if (max_devices > 1) {
			...
			if (i == 0) {
				...
				FDEV(i).end_blk = FDEV(i).start_blk +
						SEGS_TO_BLKS(sbi,
						FDEV(i).total_segments) - 1;
				FDEV(i).bdev_file = f_bdev_file_open_by_path(
		...
		FDEV(i).bdev = f_file_bdev(FDEV(i).bdev_file);
...
static int f2fs_setup_casefold(struct f2fs_sb_info *sbi)
{
	...
	if (f2fs_sb_has_casefold(sbi) && !sbi->sb->s_encoding) {
		...
		encoding = f_utf8_load(encoding_info->version);
		if (IS_ERR(encoding)) {
			f2fs_err(sbi,
...
try_onemore:
	...
	f_percpu_counter_set(&sbi->total_valid_inode_count,
...
{
	return f_mount_bdev(fs_type, flags, dev_name, data, f2fs_fill_super);
}
...
static noinline void remove_f2fs(void)
{
#ifdef CONFIG_F2FS_FS
	struct file_system_type *old_fs = NULL;

	old_fs = get_fs_type("f2fs");
	if (!old_fs) {
		pr_info("No build in f2fs\n");
		return;
	}
	...
	if (f_f2fs_destroy_compress_mempool)
		f_f2fs_destroy_compress_mempool();
	f_f2fs_destroy_bioset();
	f_f2fs_destroy_bio_entry_cache();
	...
	f_f2fs_destroy_recovery_cache();
	f_f2fs_destroy_checkpoint_caches();
...
static int __init init_f2fs_fs(void)
{
	...
	f2fs_file_operations.iopoll         = f_iocb_bio_iopoll;
	f2fs_quota_operations.alloc_dquot   = f_dquot_alloc;
	f2fs_quota_operations.destroy_dquot = f_dquot_destroy;
	f2fs_quota_operations.get_next_id   = f_dquot_get_next_id;
	f2fs_quotactl_ops.set_info          = f_dquot_set_dqinfo;
	f2fs_quotactl_ops.get_dqblk         = f_dquot_get_dqblk;
	f2fs_quotactl_ops.set_dqblk         = f_dquot_set_dqblk;
	f2fs_quotactl_ops.get_nextdqblk     = f_dquot_get_next_dqblk;
	f2fs_quotactl_ops.get_state         = f_dquot_get_state;
	f2fs_dblock_aops.migrate_folio      = f_filemap_migrate_folio;
	f2fs_meta_aops.migrate_folio        = f_filemap_migrate_folio;
	f2fs_node_aops.migrate_folio        = f_filemap_migrate_folio;
	f2fs_export_ops.encode_fh           = f_generic_encode_ino32_fh;
