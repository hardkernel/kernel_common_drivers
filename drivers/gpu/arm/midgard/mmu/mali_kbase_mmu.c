...
void kbase_mmu_page_fault_worker(struct work_struct *data)
{
	...
	/* Grab the context that was already recounted in kbase_mmu_interrupt()
...
page_fault_retry:
	...
	if (grown) {
		 ...
		 * rather than trying to continue the context should be killed
			       ...
			       dma_addr_t old_pgd_dma_addr, dma_addr_t new_pgd_dma_addr)
{
	...
	if (ret == 0) {
		...
		/* Old page metadata pointer cleared as it now owned by the new page */
...
void kbase_mmu_bus_fault_worker(struct work_struct *data)
{
	...
	/* Grab the context, already recounted in kbase_mmu_interrupt() on
