...
static ssize_t total_dma_buf_gpu_mem_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct kbase_process *kprcs = to_kprcs(kobj);
	if (WARN_ON(!kprcs))
		return 0;
...
static struct kobj_attribute total_dma_buf_gpu_mem_attr = __ATTR_RO(total_dma_buf_gpu_mem);

static struct attribute *kprcs_attrs[] = {
	&process_name_attr.attr,
	&total_gpu_mem_attr.attr,
	&total_dma_buf_gpu_mem_attr.attr,
	NULL
};
