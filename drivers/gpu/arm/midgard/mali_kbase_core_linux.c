...
static ssize_t show_ctx_lp_mem_pool_size(struct device *dev, struct device_attribute *attr, char * const buf)
{
	struct list_head *entry;
	const struct list_head *kbdev_list;
	ssize_t ret = 0;
	...
	list_for_each(entry, kbdev_list) {
		...
		list_for_each_entry(kctx, &kbdev->kctx_list, kctx_list_link) {
			...
			* opened on this device */
				unsigned long cached_mem = 0;
				for (i = 0; i < MEMORY_GROUP_MANAGER_NR_GROUPS; i++)
					//pr_info("[%d]:kctx->mem_pools.large[%d] = %d", kctx->tgid, i, kctx->mem_pools.large[i].cur_size);
					cached_mem += kctx->mem_pools.large[i].cur_size;
				ret += scnprintf(buf + ret, PAGE_SIZE - ret,
				"%p %10u %10lu\n",
				kctx,
				kctx->tgid,
...
static ssize_t set_ctx_lp_mem_pool_size(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct kbase_device *kbdev;
	ssize_t err = count;
...
static DEVICE_ATTR(ctx_lp_mem_pool_size, S_IRUGO | S_IWUSR, show_ctx_lp_mem_pool_size, set_ctx_lp_mem_pool_size);
...
static ssize_t show_mem_pool_size(struct device *dev, struct device_attribute *attr, char * const buf)
{
	...
	list_for_each(entry, kbdev_list) {
		...
		if (kbdev) {
			ret += scnprintf(buf + ret, PAGE_SIZE - ret,
				"pgd_mem_pool        %10lu\n", kbdev->pgd_mem_pool.cur_size);
			ret += scnprintf(buf + ret, PAGE_SIZE - ret,
				"fw_mem_pool.small   %10lu\n", kbdev->fw_mem_pools.small.cur_size);
			ret += scnprintf(buf + ret, PAGE_SIZE - ret,
				"fw_mem_pool.large   %10lu\n", kbdev->fw_mem_pools.large.cur_size);
		}
	}
...
int kbase_pm_gpu_freq_init(struct kbase_device *kbdev)
{
	...
	if (IS_ERR(opp_ptr)) {
		...
		/* always use the lowest frequency from opp table */
...
static ssize_t total_gpu_mem_show(struct device *dev, struct device_attribute *attr,
				char *const buf)
{
	struct kbase_device *kbdev;
	kbdev = to_kbase_device(dev);
...
static ssize_t total_dma_buf_gpu_mem_show(struct device *dev, struct device_attribute *attr,
				char *const buf)
{
	struct kbase_device *kbdev;
	kbdev = to_kbase_device(dev);
...
static DEVICE_ATTR_RO(total_dma_buf_gpu_mem);

static ssize_t gpu_dma_buf_memory_show(struct device *dev, struct device_attribute *attr,
				char *const buf)
{
	struct kbase_device *kbdev;
	ssize_t ret = 0;
		...
		list_for_each_entry(kctx, &kbdev->kctx_list, kctx_list_link) {
			...
			* opened on this device */
			ret += scnprintf(buf + ret, PAGE_SIZE - ret,
				...
				kctx->kprcs->name,
				kctx->kprcs->tgid,
				(unsigned long) kctx->kprcs->dma_buf_pages << PAGE_SHIFT);
		}
		mutex_unlock(&kbdev->kctx_list_lock);
	}
...
static DEVICE_ATTR_RO(gpu_dma_buf_memory);
...
static struct attribute *kbase_attrs[] = {
	...
	&dev_attr_gpu_dma_buf_memory.attr,
	&dev_attr_mem_pool_size.attr,
	&dev_attr_total_gpu_mem.attr,
	&dev_attr_total_dma_buf_gpu_mem.attr,
	NULL
};
...
static struct attribute *ctx_attrs[] = {
	...
	&dev_attr_ctx_lp_mem_pool_size.attr,
	NULL
};
...
#endif
{
	...
	struct kbase_mem_dmabuf_list *entry, *tmp;
	if (likely(kbdev)) {
		mutex_lock(&kbdev->kbase_dmabuf_lock);
		list_for_each_entry_safe(entry, tmp, &kbdev->kbase_dmabuf_list.dmabuf_list, dmabuf_list) {
			list_del(&entry->dmabuf_list);
			kfree(entry);
		}
		mutex_unlock(&kbdev->kbase_dmabuf_lock);
